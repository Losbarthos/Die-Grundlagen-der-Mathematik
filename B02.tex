...
\directlua{
  thmlookup.registry_path = "_B02.registry.tsv"
  thmlookup.debug_path    = "_B02.debug.log"
}
...
\FormulaThmAuto{\forall x\, F(x)\rightarrow P(x)=Q(x)\dsep \forall x\, F(x)\rightarrow P(x)=R(x)\vdash \forall x\, F(x)\rightarrow Q(x)=R(x)}
\begin{tabproof}
