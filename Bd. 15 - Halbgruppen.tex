...
\[
  f(b_{i0})=f(a_i\MogStar a_0)
    =k\MogDiamond f(a_0)=0,
  \qquad
  f(b_{i1})=f(a_i\MogStar a_1)
    =k\MogDiamond f(a_1)=0,
\]
...
\[
  R(X,Y)\coloneqq
  \{b_{ij}\mid i,j\in\mathbb N\land
...
\(b_{13}\), liegt aber in der später definierten Menge \(D'\). Wäre ein solches
\(H\) ein Rechteck \(R(X,Y)\), so folgte aus
\(b_{00},b_{13}\in R(X,Y)\) stets \(b_{03}\in R(X,Y)\). Dies ist unmöglich,
...
Alle verwendeten Symbole seien paarweise verschieden.
