...
\section{Einheiten, Teilbarkeit und Primelemente}

\subsection{Einheiten in Halbgruppen}

Eine Einheit setzt ein globales neutrales Element voraus.  Die folgende
Definition ist dennoch für jede Halbgruppe sinnvoll: Besitzt die Halbgruppe
kein neutrales Element, so besitzt sie danach auch keine Einheiten.  Sobald
ein neutrales Element existiert, ist die Halbgruppe ein Monoid; die
ausführliche Einheitentheorie folgt daher in Band~16.

\FormulaDefDeltaK[Einheit einer Halbgruppe]{%
  \SemigroupUnit{A,\star}{u}%
}{SemigroupUnitDef}{%
  \DeltaRow{Mengen}{A\dsep e\dsep u\dsep v\dsep x}
  \DeltaRow{Funktionen}{\star}
  \DeltaPrem{Halbgruppe}{\SemigroupAlg{A}{\star}}
  \DeltaRow{Träger}{u\in A}
  \DeltaRow{Identität und Inverses}{%
    \begin{aligned}[t]
      &\exists e,v\in A\,\bigl(\forall x\in A\\[-2pt]
      &\quad(e\star x=x\land x\star e=x)\\[-2pt]
      &\quad{}\land u\star v=e\land v\star u=e\bigr)
    \end{aligned}}
  \DeltaRow{\textbf{Neues Symbol}}{}
  \DeltaPrem{Einheit}{\SemigroupUnit{A,\star}{u}}
}

\FormulaAxiomDeltaK[Zugriff auf die zugrunde liegende Halbgruppe]{%
  \SemigroupUnit{A,\star}{u}\vdash\SemigroupAlg{A}{\star}%
}{SemigroupUnitBaseAxiom}{%
  \DeltaRow{Mengen}{A\dsep u}
  \DeltaRow{Funktionen}{\star}
}

\FormulaAxiomDeltaK[Einheiten liegen im Träger]{%
  \SemigroupUnit{A,\star}{u}\vdash u\in A%
}{SemigroupUnitCarrierAxiom}{%
  \DeltaRow{Mengen}{A\dsep u}
  \DeltaRow{Funktionen}{\star}
}

\FormulaAxiomDeltaK[Zugriff auf Identität und Inverses]{%
  \SemigroupUnit{A,\star}{u}\vdash
  \begin{aligned}[t]
    \exists e,v\in A\,\bigl(&
      \forall x\in A\,(e\star x=x\land x\star e=x)\\[-2pt]
      &{}\land u\star v=e\land v\star u=e\bigr)
  \end{aligned}%
}{SemigroupUnitWitnessAxiom}{%
  \DeltaRow{Mengen}{A\dsep e\dsep u\dsep v\dsep x}
  \DeltaRow{Funktionen}{\star}
}
