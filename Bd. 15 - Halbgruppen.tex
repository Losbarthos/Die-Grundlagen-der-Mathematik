...
Wir benötigen aus der Mengenlehre nur eine einzige Instanz des allgemeinen
Grunddatensatzes
\FormulaRefAuto{CanonicalTaggedGroundDataWithFreshPoints}[thm], nämlich
\(I=\mathbb N\) und \(J=\mathbb N\times\mathbb N\). Zunächst führen wir
lediglich Namen für die kanonisch getaggten Elemente ein.
\FormulaDefDeltaK[Die Namen der getaggten Elemente]{%
  \begin{aligned}[t]
    i\in\mathbb N&\vdash
      a_i\coloneqq(\varnothing,i),\\[-2pt]
    i,j\in\mathbb N&\vdash
      b_{ij}\coloneqq(\{\varnothing\},(i,j))
  \end{aligned}
}{MogiljanskajaTaggedLayersDef}{%
  \DeltaRow{Mengen}{a_i\dsep b_{ij}\dsep i\dsep j}
}

Der Grunddatensatz bündelt bereits die gewöhnlichen Elementkriterien, die
Indexeindeutigkeit, die Disjunktheit und zwei verschiedene Außenpunkte.
Seine genannte Instanz lautet:
\FormulaThmDeltaK[Existenz des gesamten Grunddatenpakets]{%
  \exists L\,\exists D\,\exists u\,\exists v\,
    \mathsf{TGD}_{\varnothing,\{\varnothing\}}^{
      \mathbb N,\mathbb N\times\mathbb N}(L,D;u,v)%
}{MogiljanskajaFreshPointsExist}{%
  \DeltaRow{Mengen}{L\dsep D\dsep u\dsep v}
}
\begin{tabproofwide}
  \proofstepwidestar[]{%
    \exists L\,\exists D\,\exists u\,\exists v\,
      \mathsf{TGD}_{\varnothing,\{\varnothing\}}^{
        \mathbb N,\mathbb N\times\mathbb N}(L,D;u,v)}{%
    \FormulaRefAuto{CanonicalTaggedGroundDataWithFreshPoints}[thm]}
\end{tabproofwide}

Wir fixieren nun gleichzeitig Zeugen \(L,D,\mathbf0,k\) dieser
Existenzaussage. Dies ist eine konservative Zeugenfixierung: Die vier
neuen Konstanten werden ausschließlich mit den Eigenschaften des
Existenzsatzes verwendet und können in jedem späteren Beweis wieder durch
Existenzbeseitigung entladen werden.
\FormulaDefDeltaK[Konservative Fixierung des Grunddatenpakets]{%
  \mathsf{TGD}_{\varnothing,\{\varnothing\}}^{
    \mathbb N,\mathbb N\times\mathbb N}(L,D;\mathbf0,k)%
}{MogiljanskajaFreshPointsChoice}{%
  \DeltaRow{Mengen}{L\dsep D\dsep\mathbf0\dsep k}
}

\NewDocumentCommand{\MogiljanskajaGroundProjection}{m m}{%
  \begin{tabproofwide}
    \proofstepwidestar[]{%
      \mathsf{TGD}_{\varnothing,\{\varnothing\}}^{
        \mathbb N,\mathbb N\times\mathbb N}(L,D;\mathbf0,k)}{%
      \FormulaRefAuto{MogiljanskajaFreshPointsChoice}[def]}
    \proofstepwidestar[]{#1}{\FormulaRefAuto{#2}[thm]{1}}
  \end{tabproofwide}%
}
\NewDocumentCommand{\MogiljanskajaNamedGroundCorollary}{m m}{%
  \begin{tabproofwide}
    \proofstepwidestar[]{%
      \mathsf{TGD}_{\varnothing,\{\varnothing\}}^{
        \mathbb N,\mathbb N\times\mathbb N}(L,D;\mathbf0,k)}{%
      \FormulaRefAuto{MogiljanskajaFreshPointsChoice}[def]}
    \proofstepwidestar[]{#1}{%
      \FormulaRefAuto{#2}[thm]{1},
      \ \FormulaRefAuto{MogiljanskajaTaggedLayersDef}[def]}
  \end{tabproofwide}%
}
...
\MogiljanskajaNamedGroundCorollary{%
  i,j\in\mathbb N\dsep a_i=a_j\vdash i=j
...
\MogiljanskajaGroundProjection{L\cap D=\varnothing
...
\MogiljanskajaGroundProjection{\mathbf0\notin L\cup D
...
\MogiljanskajaGroundProjection{k\notin L\cup D
...
\FormulaThmDeltaK[Das Zusatzsymbol liegt nicht in der (a)-Schicht]{%
  k\notin L%
