...
\FormulaAxiomDeltaK[Symmetrie]{%
  x,y\in A \dsep x\sim y \vdash y\sim x%
}{EqRelSymmetry}{
...
\FormulaAxiomDeltaK[Transitivität]{%
  x,y,z\in A \dsep x\sim y \dsep y\sim z \vdash x\sim z%
}{EqRelTransitivity}{
...
\begin{tabproof}
  ...
  \proofstep{1}{( a \mathrel { \SameOccRel M } a \leftrightarrow \forall X \in M ( a \in X \leftrightarrow a \in X ) )}{%
      \FormulaRefAuto{SameOccRelChar}[thm]{1,1}}
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[2,3]{( a \mathrel { \SameOccRel M } b \leftrightarrow \forall X \in M ( a \in X \leftrightarrow b \in X ) )}{%
      \FormulaRefAuto{SameOccRelChar}[thm]{2,3}}
  ...
  \proofstepwidestar[1,2,3]{\forall X\in M\,(b\in X\leftrightarrow a\in X)}{%
    ...
    \forall x(Q(x)\leftrightarrow P(x))}{5}}
  \proofstepwidestar[2,3]{( b \mathrel { \SameOccRel M } a \leftrightarrow \forall X \in M ( b \in X \leftrightarrow a \in X ) )}{%
      \FormulaRefAuto{SameOccRelChar}[thm]{3,2}}
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[3,4]{( a \mathrel { \SameOccRel M } b \leftrightarrow \forall X \in M ( a \in X \leftrightarrow b \in X ) )}{%
      \FormulaRefAuto{SameOccRelChar}[thm]{3,4}}
  \proofstepwidestar[1,3,4]{%
    \forall X\in M\,(a\in X\leftrightarrow b\in X)}{%
    \FormulaRefAuto{P\leftrightarrow Q, P\vdash Q}{%
      6,1}}
  \proofstepwidestar[4,5]{( b \mathrel { \SameOccRel M } c \leftrightarrow \forall X \in M ( b \in X \leftrightarrow c \in X ) )}{%
      \FormulaRefAuto{SameOccRelChar}[thm]{4,5}}
  ...
  \proofstepwidestar[1,2,3,4,5]{%
    ...
    \forall x(P(x)\leftrightarrow R(x))}{7,9}}
  \proofstepwidestar[3,5]{( a \mathrel { \SameOccRel M } c \leftrightarrow \forall X \in M ( a \in X \leftrightarrow c \in X ) )}{%
      \FormulaRefAuto{SameOccRelChar}[thm]{3,5}}
...
\begin{tabproofsplit}
\proofpart{Reflexivität}
\proofstep{}{\forall a\in\bigcup M\,
    a\mathrel{\SameOccRel{M}}a}{%
...
\proofstep{}{%
    \forall a,b\in\bigcup M\,
    \bigl(a\mathrel{\SameOccRel{M}}b
    \rightarrow b\mathrel{\SameOccRel{M}}a\bigr)}{%
    \FormulaRefAuto{SameOccRelSymmetry}}

\closeproofpart

\proofpart{Transitivität}
  \setcounter{proofstepnr}{2}% Fortlaufende Schritte dieses Beweises.
\proofstep{}{%
    \forall a,b,c\in\bigcup M\,
    \bigl(\bigl(a\mathrel{\SameOccRel{M}}b\land
                 b\mathrel{\SameOccRel{M}}c\bigr)
    \rightarrow a\mathrel{\SameOccRel{M}}c\bigr)}{%
    \FormulaRefAuto{SameOccRelTransitivity}}

\closeproofpart

\proofpart{Äquivalenzrelation}
...
\begin{tabproof}
  ...
  \proofstep{3}{F^{-1}\colon B\bij A}{\FormulaRefAuto{InverseFunctionBijection}{3}}
...
\subsection{Kardinaler Größenvergleich}

Der funktionale Satz von Cantor--Schröder--Bernstein wurde in Band~08
bewiesen. Wir übersetzen ihn nun in die Sprache der Gleichmächtigkeit
und des durch Injektionen gegebenen Größenvergleichs.
...
\noindent\emph{Lesebeweis der Übertragung.}
Es gelte \(A\CardLeq B\) und \(B\CardLeq A\). Nach
\FormulaRefAuto{CardLeqDef}[def] gibt es dann eine Injektion
\(F\colon A\inj B\) und eine Injektion \(G\colon B\inj A\).
Für solche \(F\) und \(G\) liefert
\FormulaRefAuto{CantorBernsteinFixedInjections}[thm] aus Band~08
eine Bijektion \(H\colon A\bij B\). Damit gilt nach
\FormulaRefAuto{Gleichmächtigkeit}[def] bereits \(A\EqCard B\).
Da der Schluss für beliebige Zeugen der beiden Existenzannahmen gilt,
folgt er aus den beiden ursprünglichen Vergleichsaussagen.
Das Auswahlaxiom wird dafür nicht benötigt.

Die Konstruktion der Bijektion wird hier als bereits bewiesenes Resultat
aus Band~08 verwendet. Die \CSBReadingLink{} erklärt diese Konstruktion
und ihre Anwendung auf Gleichmächtigkeit im Zusammenhang; die
\CSBProofsLink{} enthalten die Ableitungen dieser Konstruktion. Die folgende
Tabelle führt die eben beschriebene Übertragung aus.
...
\input{tex/b11-migrated-graph-cardinality}
...
\begin{tabproofsplit}
\proofpart{Reflexivität}
\proofstep{}{\forall A\in M\,(A\EqCard A)}{\FormulaRefAuto{EqCardReflexive}}
...
\proofpart{Symmetrie}
  \setcounter{proofstepnr}{1}% Fortlaufende Schritte dieses Beweises.
\proofstep{}{%
    \forall A,B\in M\,(A\EqCard B\rightarrow B\EqCard A)}{%
...
\proofstep{}{%
    ...
    \FormulaRefAuto{EqCardTransitive}}
...
\begin{tabproofsplitwide}
  \proofpartwideindR[Transport entlang fester Bijektionen]{%
    ...
    \proofstepwidestar[2]{H^{-1}\colon M\bij A}{%
      \FormulaRefAuto{InverseFunctionBijection}{2}}
...
\begin{tabproofsplitwide}
  \proofpartwideindR[Teilmengenrichtung \(\EqClass{x}{\sim}{A}\subseteq\EqClass{y}{\sim}{A}\)]{%
    ...
    \proofstepwidestar[1,2,3,4]{y\sim x}{%
      \FormulaRefAuto{EqRelSymmetry}[ax]{2,3,4}}
    \proofstepwidestar[1,2,3,4,5]{y\sim z}{%
      \FormulaRefAuto{EqRelTransitivity}[ax]{3,2,6,8,7}}
    ...
    \proofstepwidestar[1,2,3,4]{\EqClass{x}{\sim}{A}\subseteq\EqClass{y}{\sim}{A}}{%
      \FormulaRefAuto{SubsetIntroductionRule}[thm]{[5]\vdots 12}}
  ...
  \proofpartwideindR[Teilmengenrichtung \(\EqClass{y}{\sim}{A}\subseteq\EqClass{x}{\sim}{A}\)]{%
    ...
    \proofstepwidestar[1,2,3,4,5]{x\sim z}{%
      \FormulaRefAuto{EqRelTransitivity}[ax]{2,3,6,4,7}}
...
\begin{tabproof}
  ...
  \proofstep{1,2,3,4}{x\sim y}{%
    \FormulaRefAuto{EqRelSymmetry}[ax]{3,2,7}}
\end{tabproof}
...
\section{Abstieg von Funktionen auf den Quotienten}

Eine Vorschrift auf Repräsentanten bestimmt genau dann eine Funktion auf
dem Quotienten, wenn äquivalente Repräsentanten denselben Wert liefern.
Die Existenz- und Eindeutigkeitsarbeit übernimmt der allgemeine
Surjektionsabstieg aus Band~7. In den Beweisen dieses Abschnitts sind
\(Q=\QuotientSet{A}{\sim}\) und
\(\pi=\QuotProj{A}{\sim}\) lediglich Abkürzungen für die bereits
definierten Mengenterme. Insbesondere wird keine Auswahl von
Repräsentanten vorausgesetzt.

\FormulaThmDeltaK[Verträglichkeit ist Konstanz auf Projektionsfasern]{%
  \begin{aligned}[t]
    &\EqRel{A,\sim}\dsep
      \forall a,b\in A\,(a\sim b\rightarrow h(a)=h(b))\vdash{}\\[-2pt]
    &\forall a,b\in A\,
      \bigl(\QuotProj{A}{\sim}(a)=\QuotProj{A}{\sim}(b)
        \rightarrow h(a)=h(b)\bigr)
  \end{aligned}
}{QuotientCompatibilityImpliesFiberCompatibility}{%
  \DeltaRow{Mengen}{A\dsep a\dsep b}
  \DeltaRow{Relationsoperatoren}{\sim}
  \DeltaRow{Funktionsterm}{h}
}
\begin{tabproofwide}
  \proofstepwidestar[1]{\EqRel{A,\sim}}{\rA}
  \proofstepwidestar[2]{\forall a,b\in A\,(a\sim b\rightarrow h(a)=h(b))}{\rA}
  \proofstepwidestar[3]{a\in A}{\rA}
  \proofstepwidestar[4]{b\in A}{\rA}
  \proofstepwidestar[5]{\pi(a)=\pi(b)}{\rA}
  \proofstepwidestar[1,3,4]{\pi(a)=\pi(b)\leftrightarrow a\sim b}{%
    \FormulaRefAuto{QuotProjEqualIffRel}[thm]{1,3,4}}
  \proofstepwidestar[1,3,4,5]{a\sim b}{%
    \FormulaRefAuto{P\leftrightarrow Q,P\vdash Q}{6,5}}
  \proofstepwidestar[2,3]{\forall b\in A\,(a\sim b\rightarrow h(a)=h(b))}{%
    \rRE{\rUE{2},3}}
  \proofstepwidestar[2,3,4]{a\sim b\rightarrow h(a)=h(b)}{\rRE{\rUE{8},4}}
  \proofstepwidestar[1,2,3,4,5]{h(a)=h(b)}{\rRE{9,7}}
  \proofstepwidestar[1,2,3,4]{\pi(a)=\pi(b)\rightarrow h(a)=h(b)}{\rRI{5,10}}
  \proofstepwidestar[1,2,3]{b\in A\rightarrow(\pi(a)=\pi(b)\rightarrow h(a)=h(b))}{\rRI{4,11}}
  \proofstepwidestar[1,2,3]{\forall b\in A\,(\pi(a)=\pi(b)\rightarrow h(a)=h(b))}{\rUI{12}}
  \proofstepwidestar[1,2]{a\in A\rightarrow\forall b\in A\,(\pi(a)=\pi(b)\rightarrow h(a)=h(b))}{\rRI{3,13}}
  \proofstepwidestar[1,2]{\forall a,b\in A\,(\pi(a)=\pi(b)\rightarrow h(a)=h(b))}{\rUI{14}}
\end{tabproofwide}

\FormulaThmDeltaK[Eindeutiger Funktionsabstieg auf den Quotienten]{%
  \begin{aligned}[t]
    &\EqRel{A,\sim}\dsep h\colon A\to B\dsep
      \forall a,b\in A\,(a\sim b\rightarrow h(a)=h(b))\vdash{}\\[-2pt]
    &\exists!g\,\bigl(g\colon\QuotientSet{A}{\sim}\to B
      \land g\circ\QuotProj{A}{\sim}=h\bigr)
  \end{aligned}
}{QuotientFunctionDescent}{%
  \DeltaRow{Mengen}{A\dsep B\dsep a\dsep b}
  \DeltaRow{Relationsoperatoren}{\sim}
  \DeltaRow{Funktionen}{h\dsep g}
}
\begin{tabproofwide}
  \proofstepwidestar[1]{\EqRel{A,\sim}}{\rA}
  \proofstepwidestar[2]{h\colon A\to B}{\rA}
  \proofstepwidestar[3]{\forall a,b\in A\,(a\sim b\rightarrow h(a)=h(b))}{\rA}
  \proofstepwidestar[1]{\pi\colon A\sur Q}{\FormulaRefAuto{QuotProjSurjective}[thm]{1}}
  \proofstepwidestar[1,3]{\forall a,b\in A\,(\pi(a)=\pi(b)\rightarrow h(a)=h(b))}{%
    \FormulaRefAuto{QuotientCompatibilityImpliesFiberCompatibility}[thm]{1,3}}
  \proofstepwidestar[1,2,3]{\exists!g\,(g\colon Q\to B\land g\circ\pi=h)}{%
    \FormulaRefAuto{SurjectionFactorization}[thm]{4,2,5}}
\end{tabproofwide}

\FormulaThmDeltaK[Eindeutiger Wert auf einer Quotientenklasse]{%
  \begin{aligned}[t]
    &\EqRel{A,\sim}\dsep h\colon A\to B\dsep
      \forall a,b\in A\,(a\sim b\rightarrow h(a)=h(b))\\[-2pt]
    &\dsep C\in\QuotientSet{A}{\sim}\vdash
      \exists!z\in B\,\exists a\in A\,
      \bigl(C=\QuotProj{A}{\sim}(a)\land z=h(a)\bigr)
  \end{aligned}
}{QuotientValueDescent}{%
  \DeltaRow{Mengen}{A\dsep B\dsep C\dsep a\dsep b\dsep z}
  \DeltaRow{Relationsoperatoren}{\sim}
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[2]{h\colon A\to B}{\rA}
  \proofstepwidestar[3]{\forall a,b\in A\,(a\sim b\rightarrow h(a)=h(b))}{\rA}
  \proofstepwidestar[4]{C\in Q}{\rA}
  \proofstepwidestar[1]{\pi\colon A\sur Q}{\FormulaRefAuto{QuotProjSurjective}[thm]{1}}
  \proofstepwidestar[1,3]{\forall a,b\in A\,(\pi(a)=\pi(b)\rightarrow h(a)=h(b))}{%
    \FormulaRefAuto{QuotientCompatibilityImpliesFiberCompatibility}[thm]{1,3}}
  \proofstepwidestar[1,2,3,4]{\exists!z\in B\,\exists a\in A\,(C=\pi(a)\land z=h(a))}{%
    \FormulaRefAuto{SurjectionFiberValueUnique}[thm]{5,2,6,4}}
\end{tabproofwide}

\FormulaThmDeltaK[Zweistellige Verträglichkeit auf Projektionsfasern]{%
  \begin{aligned}[t]
    &\EqRel{A,\sim}\dsep
      \forall a,b,c,d\in A\,
      \bigl((a\sim c\land b\sim d)\rightarrow h(a,b)=h(c,d)\bigr)
      \vdash{}\\[-2pt]
    &\forall a,b,c,d\in A\,
      \Bigl(\bigl(\QuotProj{A}{\sim}(a)=\QuotProj{A}{\sim}(c)
      \land\QuotProj{A}{\sim}(b)=\QuotProj{A}{\sim}(d)\bigr)\\[-2pt]
    &\hspace{42mm}\rightarrow h(a,b)=h(c,d)\Bigr)
  \end{aligned}
}{QuotientBinaryCompatibilityImpliesFiberCompatibility}{%
...
\begin{tabproofwide}
  \proofstepwidestar[1]{\EqRel{A,\sim}}{\rA}
  \proofstepwidestar[2]{\begin{aligned}[t]&\forall a,b,c,d\in A\,((a\sim c\land b\sim d)\\[-2pt]&\quad\rightarrow h(a,b)=h(c,d))\end{aligned}}{\rA}
  \proofstepwidestar[3]{a\in A}{\rA}
  \proofstepwidestar[4]{b\in A}{\rA}
  \proofstepwidestar[5]{c\in A}{\rA}
  \proofstepwidestar[6]{d\in A}{\rA}
  \proofstepwidestar[7]{\pi(a)=\pi(c)\land\pi(b)=\pi(d)}{\rA}
  \proofstepwidestar[7]{\pi(a)=\pi(c)}{\rAEa{7}}
  \proofstepwidestar[7]{\pi(b)=\pi(d)}{\rAEb{7}}
  \proofstepwidestar[1,3,5]{\pi(a)=\pi(c)\leftrightarrow a\sim c}{%
    \FormulaRefAuto{QuotProjEqualIffRel}[thm]{1,3,5}}
  \proofstepwidestar[1,4,6]{\pi(b)=\pi(d)\leftrightarrow b\sim d}{%
    \FormulaRefAuto{QuotProjEqualIffRel}[thm]{1,4,6}}
  \proofstepwidestar[1,3,5,7]{a\sim c}{\FormulaRefAuto{P\leftrightarrow Q,P\vdash Q}{10,8}}
  \proofstepwidestar[1,4,6,7]{b\sim d}{\FormulaRefAuto{P\leftrightarrow Q,P\vdash Q}{11,9}}
  \proofstepwidestar[1,3,4,5,6,7]{a\sim c\land b\sim d}{\rAI{12,13}}
  \proofstepwidestar[2,3]{\forall b,c,d\in A\,((a\sim c\land b\sim d)\rightarrow h(a,b)=h(c,d))}{\rRE{\rUE{2},3}}
  \proofstepwidestar[2,3,4]{\forall c,d\in A\,((a\sim c\land b\sim d)\rightarrow h(a,b)=h(c,d))}{\rRE{\rUE{15},4}}
  \proofstepwidestar[2,3,4,5]{\forall d\in A\,((a\sim c\land b\sim d)\rightarrow h(a,b)=h(c,d))}{\rRE{\rUE{16},5}}
  \proofstepwidestar[2,3,4,5,6]{(a\sim c\land b\sim d)\rightarrow h(a,b)=h(c,d)}{\rRE{\rUE{17},6}}
  \proofstepwidestar[1,2,3,4,5,6,7]{h(a,b)=h(c,d)}{\rRE{18,14}}
  \proofstepwidestar[1,2,3,4,5,6]{\begin{aligned}[t]&(\pi(a)=\pi(c)\land\pi(b)=\pi(d))\\[-2pt]&\quad\rightarrow h(a,b)=h(c,d)\end{aligned}}{\rRI{7,19}}
  \proofstepwidestar[1,2,3,4,5]{\begin{aligned}[t]&d\in A\rightarrow\\[-2pt]&\quad((\pi(a)=\pi(c)\land\pi(b)=\pi(d))\\[-2pt]&\quad\rightarrow h(a,b)=h(c,d))\end{aligned}}{\rRI{6,20}}
  \proofstepwidestar[1,2,3,4,5]{\begin{aligned}[t]&\forall d\in A\,\\[-2pt]&\quad((\pi(a)=\pi(c)\land\pi(b)=\pi(d))\\[-2pt]&\quad\rightarrow h(a,b)=h(c,d))\end{aligned}}{\rUI{21}}
  \proofstepwidestar[1,2,3,4]{\begin{aligned}[t]&c\in A\rightarrow\forall d\in A\,\\[-2pt]&\quad((\pi(a)=\pi(c)\land\pi(b)=\pi(d))\\[-2pt]&\quad\rightarrow h(a,b)=h(c,d))\end{aligned}}{\rRI{5,22}}
  \proofstepwidestar[1,2,3,4]{\begin{aligned}[t]&\forall c,d\in A\,\\[-2pt]&\quad((\pi(a)=\pi(c)\land\pi(b)=\pi(d))\\[-2pt]&\quad\rightarrow h(a,b)=h(c,d))\end{aligned}}{\rUI{23}}
  \proofstepwidestar[1,2,3]{\begin{aligned}[t]&b\in A\rightarrow\forall c,d\in A\,\\[-2pt]&\quad((\pi(a)=\pi(c)\land\pi(b)=\pi(d))\\[-2pt]&\quad\rightarrow h(a,b)=h(c,d))\end{aligned}}{\rRI{4,24}}
  \proofstepwidestar[1,2,3]{\begin{aligned}[t]&\forall b,c,d\in A\,\\[-2pt]&\quad((\pi(a)=\pi(c)\land\pi(b)=\pi(d))\\[-2pt]&\quad\rightarrow h(a,b)=h(c,d))\end{aligned}}{\rUI{25}}
  \proofstepwidestar[1,2]{\begin{aligned}[t]&a\in A\rightarrow\forall b,c,d\in A\,\\[-2pt]&\quad((\pi(a)=\pi(c)\land\pi(b)=\pi(d))\\[-2pt]&\quad\rightarrow h(a,b)=h(c,d))\end{aligned}}{\rRI{3,26}}
  \proofstepwidestar[1,2]{\begin{aligned}[t]&\forall a,b,c,d\in A\,\\[-2pt]&\quad((\pi(a)=\pi(c)\land\pi(b)=\pi(d))\\[-2pt]&\quad\rightarrow h(a,b)=h(c,d))\end{aligned}}{\rUI{27}}
\end{tabproofwide}

\FormulaThmDeltaK[Eindeutiger Wert einer zweistelligen Quotientenvorschrift]{%
  \begin{aligned}[t]
    &\EqRel{A,\sim}\dsep h\colon A\times A\to B\dsep{}\\[-2pt]
    &\forall a,b,c,d\in A\,((a\sim c\land b\sim d)\rightarrow h(a,b)=h(c,d))\\[-2pt]
    &\dsep C,D\in\QuotientSet{A}{\sim}\vdash{}\\[-2pt]
    &\exists!z\in B\,\exists a,b\in A\,
      \bigl(C=\QuotProj{A}{\sim}(a)\land
      D=\QuotProj{A}{\sim}(b)\land z=h(a,b)\bigr)
  \end{aligned}
}{QuotientBinaryValueDescent}{%
  \DeltaRow{Mengen}{A\dsep B\dsep C\dsep D\dsep a\dsep b\dsep c\dsep d\dsep z}
  \DeltaRow{Relationsoperatoren}{\sim}
  \DeltaRow{Funktionen}{h}
}
\begin{tabproofwide}
  \proofstepwidestar[1]{\EqRel{A,\sim}}{\rA}
  \proofstepwidestar[2]{h\colon A\times A\to B}{\rA}
  \proofstepwidestar[3]{\begin{aligned}[t]&\forall a,b,c,d\in A\,((a\sim c\land b\sim d)\\[-2pt]&\quad\rightarrow h(a,b)=h(c,d))\end{aligned}}{\rA}
  \proofstepwidestar[4]{C\in Q}{\rA}
  \proofstepwidestar[5]{D\in Q}{\rA}
  \proofstepwidestar[1]{\pi\colon A\sur Q}{\FormulaRefAuto{QuotProjSurjective}[thm]{1}}
  \proofstepwidestar[1,3]{\begin{aligned}[t]&\forall a,b,c,d\in A\,\\[-2pt]&\quad((\pi(a)=\pi(c)\land\pi(b)=\pi(d))\\[-2pt]&\quad\rightarrow h(a,b)=h(c,d))\end{aligned}}{%
    \FormulaRefAuto{QuotientBinaryCompatibilityImpliesFiberCompatibility}[thm]{1,3}}
  \proofstepwidestar[1,2,3,4,5]{\begin{aligned}[t]&\exists!z\in B\,\exists a,b\in A\,(C=\pi(a)\\[-2pt]&\quad\land D=\pi(b)\land z=h(a,b))\end{aligned}}{%
    \FormulaRefAuto{SurjectionBinaryFiberValueUnique}[thm]{6,2,7,4,5}}
\end{tabproofwide}

Nach \FormulaRefAuto{QuotProjValue}[thm] sind die Projektionswerte
genau die Äquivalenzklassen ihrer Argumente. Der letzte Satz kann daher
unmittelbar zur Definition einer zweistelligen Operation auf Klassen
verwendet werden, sobald ihre Werte im gewünschten Zielträger liegen.
...
\begin{tabproofsplitwide}
  ...
  \proofpartwideindR[Vom Urbildzeugen zum Klassenzeugen]{%
    ...
    \proofstepwidestar[1,6]{\QuotProj{A}{\sim}(a)=\EqClass{a}{\sim}{A}}{%
      \FormulaRefAuto{QuotProjValue}{1,10}}
    \proofstepwidestar[1,6]{C=\EqClass{a}{\sim}{A}}{%
      \FormulaRefAuto{a = b,\, b = c \vdash a = c}{\rAEb{6},11}}
    \proofstepwidestar[1,6]{a\in X\land C=\EqClass{a}{\sim}{A}}{\rAI{7,12}}
    \proofstepwidestar[1,6]{\exists a\in X\,C=\EqClass{a}{\sim}{A}}{\rEI{13}}
...
\begin{tabproofsplitwide}
  ...
  \proofpartwideindR[\(\leftarrow\)]{%
    ...
    \proofstepwidestar[1,2,3]{\QuotFamMap A \sim M ( X ) = \QuotProj A \sim [ X ]}{%
      \FormulaRefAuto{QuotFamMapValue}{1,2,3}}
...
\begin{tabproofsplitwide}
  ...
  \proofpartwideindR[Auswertung auf den beiden Familiengliedern]{%
    ...
    \proofstepwidestar[1,2,4]{\QuotFamMap A \sim M ( X ) \cup \QuotProj A \sim [ Y ] = \QuotFamMap A \sim M ( X ) \cup \QuotFamMap A \sim M ( Y )}{%
      \FormulaRefAuto{A=B\vdash C\cup A = C\cup B}{8}}
    \proofstepwidestar[1,2,3,4]{%
      \QuotProj{A}{\sim}[X]\cup\QuotProj{A}{\sim}[Y]=
      \QuotFamMap{A}{\sim}{M}(X)\cup\QuotFamMap{A}{\sim}{M}(Y)}{%
  ...
  \proofpartwide{Schluss}
    ...
    \proofstepwidestar[1,2,3,4]{\QuotProj A \sim [ X \cup Y ] = \QuotFamMap A \sim M ( X ) \cup \QuotFamMap A \sim M ( Y )}{%
...
\begin{tabproofsplitwide}
  ...
  \proofpartwideindR[Rückrichtung]{%
    ...
    \proofstepwidestar[1,2,7]{( a \mathrel { \SameOccRel M } b \leftrightarrow \forall Z \in M ( a \in Z \leftrightarrow b \in Z ) )}{%
      \FormulaRefAuto{SameOccRelChar}[thm]{2,9}}
    ...
    \proofstepwidestar[1,2,7]{a\in X}{%
      \FormulaRefAuto{P\leftrightarrow Q,\,Q\vdash P}{14,8}}
...
\begin{tabproofsplitwide}
  \proofpartwideindR[Teilmengenrichtung \(X\subseteq Y\)]{%
    ...
    \proofstepwidestar[1,2,3]{X\subseteq Y}{%
      \FormulaRefAuto{SubsetIntroductionRule}[thm]{[4]\vdots 10}}
  \closeproofpartwideindR
...
\begin{tabproofsplitwide}
  \proofpartwideindR[Transport fester Urbilder]{%
    ...
    \proofstepwidestar[2,3]{%
      ...
      \rIE{\rAEb{2},\rAEb{3}}}
    \proofstepwidestar[1,2,3]{\OccQuotMap M ( X \cup Y ) = \OccQuotMap M ( X ) \cup \OccQuotMap M ( Y )}{%
...
\begin{tabproofsplitwide}
  \proofpartwideindR[Die Vereinigung liegt im Quotiententräger]{%
    ...
    \proofstepwidestar[]{%
      \bigcup\OccQuotFam{M}\subseteq\OccQuotCarrier{M}}{%
      \FormulaRefAuto{SubsetIntroductionRule}[thm]{[1]\vdots 13}}
  \closeproofpartwideindR
  ...
  \proofpartwideindR[Jede Quotientenklasse liegt in der Vereinigung]{%
    ...
    \proofstepwidestar[]{%
      \OccQuotCarrier{M}\subseteq\bigcup\OccQuotFam{M}}{%
      \FormulaRefAuto{SubsetIntroductionRule}[thm]{[1]\vdots 17}}
  \closeproofpartwideindR
...
\begin{tabproofsplitwide}
  ...
  \proofpartwideindR[Ununterscheidbarkeit der Repräsentanten]{%
    ...
    \proofstepwidestar[1,2,3,4,5,6,7]{%
      ...
      \end{aligned}}{%
      \rAI{4,\rAI{6,10}}}
    \proofstepwidestar[4,6]{( a \mathrel { \SameOccRel M } b \leftrightarrow \forall X \in M ( a \in X \leftrightarrow b \in X ) )}{%
      \FormulaRefAuto{SameOccRelChar}[thm]{4,6}}
