...
\]
vor. Der entsprechende Block beginnt analog mit den Annahmezeilen
\[
  A_1,\dots,A_r
\]
; die Menge \(I_s\) enthält dann alle zu diesen Annahmen gehörigen
Indizes, die beim Entladen gemeinsam aus \(I_s\) entfernt werden.
...
\begin{DeltaContext}{In diesem Abschnitt verwendete Symbole}
  \DeltaRow{Aussagen}{A \dsep B \dsep \varphi}
...
\begin{enumerate}
  ...
  \item kurze \textbf{Kontextzeilen}, in denen wir angeben, wie die
        ...
        \]
\end{enumerate}
Die Kontextzeilen gehören nicht zur formalen Aussage selbst; sie
sollen nur deutlich machen, welche Art von Objekten mit den
Symbolen gemeint ist (Mengen, Funktionen u.\,Ä.).
...
\]
gekennzeichnet. Es folgt der Kontext, z.\,B.\ in der Form
\[
  \text{Mengen: } M,N
  \qquad
  \text{Relationen/Funktionen: } F \subseteq M \times N.
\]
und die betreffende Formel \(\varphi\).
...
\begin{DeltaContext}{In diesem Abschnitt verwendete Symbole}
  ...
  \DeltaRow{Terme}{t_1 \dsep \dots \dsep t_n}
  \DeltaRow{Neue Symbole}{Q \dsep \sigma \dsep \iota}
\end{DeltaContext}
...
Sei \(\varphi(t_1,\dots,t_n)\) eine wohldefinierte Aussage in der
Sprache \(\mathcal L\), in der Terme \(t_1,\dots,t_n\) vorkommen
dürfen, und sei \(Q\) ein neues \(n\)-stelliges Symbol (z.\,B.\ ein
neues Prädikatensymbol oder ein neues Funktionssymbol).

Eine \textbf{explizite Definition} von \(Q\) schreiben wir in
metasprachlicher Notation als
\[
  Q(t_1,\dots,t_n) \coloneqq \varphi(t_1,\dots,t_n).
\]
Damit vereinbaren wir: Überall dort, wo der Ausdruck
\(Q(t_1,\dots,t_n)\) vorkommt, darf er als Abkürzung für
...
\[
  \forall t_1,\dots,t_n\,
    \bigl( Q(t_1,\dots,t_n) \leftrightarrow
...
\[
  \varphi(t_1,\dots,t_n) \vdash Q(t_1,\dots,t_n)
  \qquad\text{und}\qquad
  Q(t_1,\dots,t_n) \vdash \varphi(t_1,\dots,t_n).
\]
Das bedeutet: In Beweisen dürfen wir \(Q(t_1,\dots,t_n)\)
...
\(t_1,\dots,t_n\) vorkommen dürfen, und sei \(Q\) wieder ein neues
\(n\)-stelliges Symbol.

Eine \textbf{partielle Definition} von \(Q\) schreiben wir
metasprachlich als
\[
  C(t_1,\dots,t_n) \rightarrow
  \bigl(Q(t_1,\dots,t_n) \coloneqq \varphi(t_1,\dots,t_n)\bigr).
\]
Damit wird \(Q(t_1,\dots,t_n)\) nur \emph{unter der Bedingung}
...
\[
  C(t_1,\dots,t_n) \rightarrow
  \bigl(Q(t_1,\dots,t_n) \leftrightarrow \varphi(t_1,\dots,t_n)\bigr)
\]
ergeben sich etwa die beiden Regeln
\[
  C(t_1,\dots,t_n),\; \varphi(t_1,\dots,t_n)
    \vdash Q(t_1,\dots,t_n)
\]
\[
  C(t_1,\dots,t_n),\; Q(t_1,\dots,t_n)
...
\textbf{Metadefinitionen} (auch \emph{meta-sprachliche Definitionen})
...
\emph{Nicht-Gleichheitszeichen} als meta-sprachliche Abkürzung ein durch
\[
  t \neq u \;\coloneqq\; \neg\bigl(t = u\bigr).
\]
