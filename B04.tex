...
\FormulaThmDeltaK[Ersetzungsschema (äquivalente Funktionsform)]{%
  ...
  \DeltaRow{Mengen}{A\dsep B\dsep x\dsep y\dsep z\dsep\vec p}
  \DeltaRow{Formel der Mengensprache}{\varphi(x,y,\vec p)}
  \DeltaRow{Wertterm (Termschema)}{t_{\varphi}}
}

Für den Beweis verwenden wir im funktionsfreien Ersetzungsschema die Formel
\[
  \psi(x,y,\vec p)\coloneqq \bigl(y=t_{\varphi}(x)\bigr).
\]
Sobald der Wertterm unter der Eindeutigkeitsprämisse definiert ist, hat diese
Gleichung für jedes \(x\in A\) genau eine Lösung.
...
\begin{tabproof}
  ...
  \proofstep{2}{x\in A}{\rA}
  \proofstep{1,2}{t_{\varphi}(x)=t_{\varphi}(x)}{\rII}
  \proofstep{1,2}{%
    \exists y\,\bigl(y=t_{\varphi}(x)\bigr)
  }{\rEI{3}}
  \proofstep{5}{y=t_{\varphi}(x)}{\rA}
  \proofstep{6}{z=t_{\varphi}(x)}{\rA}
  \proofstep{5,6}{y=z}{%
  ...
  \proofstep{1}{%
    \forall x\in A\,\exists!y\,\bigl(y=t_{\varphi}(x)\bigr)
  }{\rUI{\rRI{2,8}}}
  \proofstep{1}{%
    \exists B\,\forall y\,
    \Bigl(
      ...
      \exists x\in A\,\bigl(y=t_{\varphi}(x)\bigr)
    \Bigr)
  }{\FormulaRefAuto{ReplacementSchema}[ax]{9}}
\end{tabproof}
