...
\begin{proof}
Dies ist das Termschema \FormulaRefAuto{TermImageSetUniqueExists}[thm]
für den Mengenterm \(t(j)=(r+j,v(j))\).
\end{proof}
...
\begin{proof}
Nach \FormulaRefAuto{ShiftedWordGraphDef}[def] gibt es
\(j,k\in\NatSeg s\) mit
\((q,a)=(r+j,v(j))\) und \((q,b)=(r+k,v(k))\).
Die Gleichheit geordneter Paare liefert
\(q=r+j=r+k\), \(a=v(j)\) und \(b=v(k)\).
Aus \FormulaRefAuto{PeanoNatSegElementNatural}[thm] folgen
\(j,k\in\mathbb N\); mit \(r\in\mathbb N\) ergibt
\FormulaRefAuto{PeanoAddLeftCancellation}[thm] daher \(j=k\).
Somit gilt \(a=v(j)=v(k)=b\).
\end{proof}
...
\(\Pred(j)<n\). Insbesondere liegt ein solches \(j\) in
\(\mathbb N\setminus\{0\}\), sodass \(\Pred(j)\) definiert ist. Nachfolger und
Vorgänger sind auf diesen Bereichen invers.
\end{proof}
...
\begin{proof}
Dies ist der Rekursionssatz für die natürlichen Zahlen aus Band~10,
...
\FormulaThmDeltaK[Additive Verschachtelung einer wachsenden Mengenfolge]{%
  \begin{aligned}[t]
    &D\colon\mathbb N\to\powerset(A)\dsep
      \forall j\in\mathbb N\,D(j)\subseteq D(\Succ(j))\dsep
...
\begin{tabproofsplitwide}
  \proofpartind[Induktionsanfang]{%
    n\in\mathbb N\vdash D(n)\subseteq D(n+0)}
    \proofstep{1}{n\in\mathbb N}{\rA}
    \proofstep{1}{n+0=n}{\FormulaRefAuto{PeanoAddRightZero}[thm]{1}}
    \proofstep{}{D(n)\subseteq D(n)}{\FormulaRefAuto{A\subseteq A}[thm]}
    \proofstep{1}{D(n)\subseteq D(n+0)}{\rIE{2,3}}
  \closeproofpartind
  \proofpartind[Induktionsschritt]{%
    \begin{aligned}[t]
      &\forall j\in\mathbb N\,D(j)\subseteq D(\Succ(j))\dsep
        n,k\in\mathbb N\dsep D(n)\subseteq D(n+k)\vdash{}\\[-2pt]
      &D(n)\subseteq D(n+\Succ(k))
    \end{aligned}}
    \proofstep{1}{\forall j\in\mathbb N\,D(j)\subseteq D(\Succ(j))}{\rA}
    \proofstep{2}{n,k\in\mathbb N}{\rA}
    \proofstep{3}{D(n)\subseteq D(n+k)}{\rA}
    \proofstep{2}{n+k\in\mathbb N}{\FormulaRefAuto{PeanoAddClosure}[thm]{2}}
    \proofstep{1,2}{D(n+k)\subseteq D(\Succ(n+k))}{\rRE{\rUE{1},4}}
    \proofstep{1,2,3}{D(n)\subseteq D(\Succ(n+k))}{%
      \FormulaRefAuto{A\subseteq B\dsep B\subseteq C\vdash A\subseteq C}[thm]{3,5}}
    \proofstep{2}{n+\Succ(k)=\Succ(n+k)}{%
      \FormulaRefAuto{PeanoAddSuccRight}[thm]{2}}
    \proofstep{1,2,3}{D(n)\subseteq D(n+\Succ(k))}{\rIE{7,6}}
  \closeproofpartind
  \proofpart{Induktionsschluss}
    \proofstep{1}{D\colon\mathbb N\to\powerset(A)}{\rA}
    \proofstep{2}{\forall j\in\mathbb N\,D(j)\subseteq D(\Succ(j))}{\rA}
    \proofstep{3}{n,k\in\mathbb N}{\rA}
    \proofstep{1,2,3}{D(n)\subseteq D(n+k)}{%
      \FormulaRefAuto{PeanoInductionRule}[ax]{IA,IS,2,3}}
  \closeproofpart
...
\begin{tabproof}
  \proofstep{1}{D\colon\mathbb N\to\powerset(A)}{\rA}
  \proofstep{2}{\forall j\in\mathbb N\,D(j)\subseteq D(\Succ(j))}{\rA}
  \proofstep{3}{m,n\in\mathbb N}{\rA}
  \proofstep{4}{m\leq n}{\rA}
  \proofstep{3,4}{\exists k\in\mathbb N\,(m+k=n)}{%
    \FormulaRefAuto{m\in\mathbb{N}\dsep n\in\mathbb{N}\vdash
      m\leq n \leftrightarrow \exists k\in\mathbb{N}\,(m+k=n)}[thm]{3,4}}
  \proofstep{6}{k\in\mathbb N\land m+k=n}{\rA}
  \proofstep{1,2,3,6}{D(m)\subseteq D(m+k)}{%
    \FormulaRefAuto{SubsetSequenceAdditiveNesting}[thm]{1,2,3,\rAEa{6}}}
  \proofstep{1,2,3,6}{D(m)\subseteq D(n)}{\rIE{\rAEb{6},7}}
  \proofstep{1,2,3,4}{D(m)\subseteq D(n)}{\rEE{5,6,8}}
\end{tabproof}
...
\FormulaRefAuto{TypedFunctionDefinitionPrinciple}[thm] liefert daraus
...
\FormulaRefAuto{RealCutMultiplicativeClosure}[thm] und
\FormulaRefAuto{TypedFunctionDefinitionPrinciple}[thm] liefern beide
Funktionen.
\end{proof}
