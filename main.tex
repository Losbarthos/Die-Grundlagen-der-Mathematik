\documentclass[a4paper,11pt]{report}

\usepackage[german]{babel}
\usepackage{csquotes}
\usepackage{amsmath,amssymb}
\usepackage{hyperref}
\usepackage{subfiles}
\usepackage{xparse}
\usepackage{mathtools} % für \coloneqq
\usepackage{booktabs}  % für \toprule, \midrule, \bottomrule
\usepackage{tabularx}  % für tabularx-Umgebung
\usepackage{makecell}

% erst amsthm (falls formula-envs.tex es nicht selbst lädt; doppelt ist meist ok, aber sauber ist einmal)
\usepackage{amsthm}
\usepackage{xparse}
\usepackage{multirow}
\usepackage{microtype}
\hypersetup{hidelinks}


\input{tex/impl/thmlookup.tex}
\input{tex/impl/formula-envs.tex}
\input{tex/impl/proof-tables.tex} 
\input{tex/impl/commands.tex}

% Verhindert Seitenumbruch innerhalb eines Regelschemas
\newenvironment{RuleSchema}{\begin{samepage}}{\end{samepage}}

\newcounter{file}

% --- thmlookup: pro Band in eigene Dateien schreiben ---
\directlua{
  function thmlookup.set_band_output(tag)
    local base = "\luaescapestring{\detokenize{registry/_}}" .. tag
    local reg  = base .. ".registry.tsv"
    local dbg  = base .. ".debug.log"

    thmlookup.registry_path = reg
    thmlookup.debug_path    = dbg
    thmlookup.prepare_run()

    texio.write_nl("thmlookup: output -> " .. thmlookup.registry_path)
  end
}

\newcommand{\ThmLookupBand}[1]{%
  \directlua{thmlookup.set_band_output([==[\luaescapestring{\detokenize{#1}}]==])}%
}

\newcommand{\ThmLookupLoadBandRegistries}[1]{%
  \directlua{
    local list = [==[\luaescapestring{\detokenize{#1}}]==]
    for tag in string.gmatch(list, [==[[^, ]+]==]) do
      thmlookup.load_registry_file([==[\luaescapestring{\detokenize{registry/_}}]==] .. tag .. [==[.registry.tsv]==])
    end
  }%
}



% --- Warnschwellen ---
\hbadness=10000
\vbadness=10000
\hfuzz=1pt
\vfuzz=1pt


\begin{document}

\ThmLookupBand{B01}
\subfile{B01}

\clearpage
\ThmLookupBand{B02}
\ThmLookupLoadBandRegistries{B01}
\subfile{B02}

\clearpage
\ThmLookupBand{B03}
\ThmLookupLoadBandRegistries{B01,B02}
\subfile{B03}

\clearpage
\ThmLookupBand{B04}
\ThmLookupLoadBandRegistries{B01,B02,B03}
\subfile{B04}

\clearpage
\ThmLookupBand{B05}
\ThmLookupLoadBandRegistries{B01,B02,B03,B04}
\subfile{B05}


\end{document}
