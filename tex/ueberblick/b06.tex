...
\addlinespace[.8ex]
\textbf{Differenzen unter einer Injektion}
Für \(F\colon M\inj N\), \(C\subseteq A\subseteq M\).
&
\textit{Bild der Differenz}
\UebFormel{F[A\setminus C]=F[A]\setminus F[C].}
\UebQuellen{\FormulaRefAuto{InjectiveImageDifference}[thm]}\\
\addlinespace[.8ex]
