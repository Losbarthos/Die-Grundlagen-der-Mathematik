...
injektiv. W3 ist das Induktionsaxiom in Mengenform (Minimalität): Jede
Teilmenge mit Anfang und Abschluss unter allen Anfügungen ist der
ganze Träger.
\BXXVIIBlickQuellen{\FormulaRefAuto{WordStructureDef}[def]}
&
\textit{Axiomatische Induktion und ihre Zerlegungsfolge}
Eine Eigenschaft, die für den Anfang gilt und bei Anfügung erhalten
bleibt, gilt für sämtliche Wörter der Struktur. Daraus folgt: Jedes
Wort ist der Anfang oder ein Nachfolger; W2 sichert dessen Eindeutigkeit.
\BXXVIIBlickQuellen{%
  \FormulaRefAuto{WordStructureDecomposition}[thm];
  \FormulaRefAuto{WordStructureInduction}[ax];
  \FormulaRefAuto{WordStructureInductionRule}[ax]}
Mengenform, Prädikatsform und lokale Regel sind gleichwertige Formen
von W3, keine zusätzlichen unabhängigen Annahmen.\\
\addlinespace[.8ex]

\textbf{Rekursionsdaten einer Wortstruktur}
\BXXVIIBlickFormel{%
  x_0\in X,\qquad r\colon X\times A\to X.}
Die Zahlenrekursion aus Band~10 erzeugt aufeinander aufbauende
Wertzuordnungen. W0 bis W3 sichern, dass ihre Vereinigung eine
Funktion auf allen Wörtern ist.
&
...
Die Rechengesetze werden mit dem axiomatischen Induktionsprinzip W3
bewiesen.\\
\addlinespace[.8ex]
...
Eigenschaften als W0 bis W3 für beliebige Wortstrukturen. W3 ist das
Induktionsaxiom in Mengenform: Anfang und Abschluss unter allen
Buchstabenanfügungen erfassen den ganzen Träger. Wie bei den
Peano-Axiomen stehen die gleichwertige Prädikatsform und die lokale
Induktionsregel für nachfolgende Beweise ausdrücklich bereit. Diese
Formen führen keine neuen unabhängigen Annahmen ein.
...
\widowpenalty=10000
Die axiomatische Wortinduktion liefert die Existenz der
Nachfolgerzerlegung; W2 sichert ihre Eindeutigkeit. Bei Bäumen folgen
Zerlegung und Induktion entsprechend aus den frühen Strukturaxiomen.
Für die Wortrekursion erzeugt der Dedekindsche Rekursionssatz eine Folge
von Stufen. Zahleninduktion sichert eindeutige Werte und aufsteigende
Stufen; Wortinduktion zeigt, dass ihre Vereinigung alle Wörter erfasst
(\FormulaRefAuto{WordStructureRecursion}[thm]).
Die Baumrekursion verwendet weiterhin den Schnitt der unter ihren
Rekursionsregeln abgeschlossenen Relationen
(\FormulaRefAuto{BinaryTreeStructureRecursion}[thm]).
