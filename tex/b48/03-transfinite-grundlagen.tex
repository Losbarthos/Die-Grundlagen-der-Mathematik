...
\noindent
Wähle für die beiden vorausgesetzten Existenzaussagen Injektionen
\(f\) und \(g\). Für diese Zeugen gilt:
\begin{tabproof}
 \proofstep{1}{f\colon A\inj B}{\rA}
 \proofstep{2}{g\colon B\inj A}{\rA}
 \proofstep{1,2}{\exists h\colon A\bij B}
 {\FormulaRefAuto{CantorBernsteinFixedInjections}[thm]{1,2}}
 \proofstep{1,2}{A\approx B}
 {\BXLVIIIWhy{Definition von \(\approx\)}}
\end{tabproof}
\noindent
Die Schlussaussage enthält weder \(f\) noch \(g\). Zweimalige
Existenzbeseitigung entlässt deshalb die gewählten Zeugen.
Dies ist der Satz aus Band~8 in der Notation dieses Bandes:
\(A\approx B\) bedeutet ebenso wie \(A\EqCard B\) die Existenz einer
Bijektion zwischen den Mengen \(A\) und \(B\). Der dortige Beweis
benötigt weder Auswahl noch Rekursion. Wird der Satz innerhalb eines
Modells von ZF benutzt, wird die gesamte mengentheoretische Herleitung
in diesem Modell interpretiert; ihre Injektionen und die gewonnene
Bijektion sind dann modellintern.
...
\begin{tabproof}
 ...
 {\BXLVIIIWhy{Ordnungstyp; \FormulaRefAuto{B48CSB}[thm]}}
 \proofstep{1}{\BXLVIIIText{Da \(\omega\le\kappa\), folgt
 \(\kappa\cdot\omega=\kappa\). Positive endliche Potenzen von
 \(\kappa\) haben nach Induktion ebenfalls Mächtigkeit \(\kappa\).
 Wähle für jede positive Länge \(n\) eine Injektion
 \(i_n:\kappa^n\to\kappa\). Ein Tupel \(x\) der Länge \(n>0\)
 wird durch \((i_n(x),n)\) codiert, das leere Tupel durch \((0,0)\).
 Dies injiziert \(\kappa^{<\omega}\) in \(\kappa\times\omega\).
 Die Tupel der Länge eins liefern die Gegeninjektion von \(\kappa\);
 also hat \(\kappa^{<\omega}\) Mächtigkeit \(\kappa\).}}
 {\BXLVIIIWhy{Produktgleichung; Auswahl von Injektionen;
 \FormulaRefAuto{B48CSB}[thm]}}
\end{tabproof}
\noindent
