  ...
  if #M.by_key[key] > 0 then
    ...
    if not has_fresh then
      ...
      if M.duplicate_policy == "error" then
        local first = M.by_key[key][1]
        tex.error("ThmLookup: Duplikat erkannt (gleiche Struktur/Key).", {
          "Key: " .. tostring(key),
          "Existing: " .. tostring(first.env) .. " " .. tostring(first.number) .. " label=" .. tostring(first.label),
          "New:      " .. tostring(env) .. " " .. tostring(number) .. " label=" .. tostring(label),
          "Siehe Debug: " .. tostring(M.debug_path),
        })
        return
      elseif M.duplicate_policy == "warn" then
        local first = M.by_key[key][1]
        texio.write_nl("thmlookup WARNING: duplicate key=" .. tostring(key) ..
          " existing=" .. tostring(first.label) .. " new=" .. tostring(label) ..
          " (skipping new)")
        return
      end
      -- policy == "keep" => weiter
