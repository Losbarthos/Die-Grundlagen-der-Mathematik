...
Bei der Zahlenrekursion wird der neue Wert aus einem Vorgängerwert
berechnet. Die Wortrekursion nimmt zusätzlich den angefügten Buchstaben
auf. Ein Baumknoten \(k(u,v)\) entsteht dagegen aus zwei bereits
gebildeten Teilbäumen \(u,v\). Deshalb benötigt die Baumrekursion
beide Werte \(F(u),F(v)\); die Funktion \(g\colon X\times X\to X\)
verknüpft sie zum Wert des neuen Baums. Den Anfang bilden die Blätter:
Die Funktion \(f\colon A\to X\) gibt für jedes Blatt \(\ell(a)\)
den Wert \(f(a)\) vor.

Die folgende Abkürzung hält genau diese Anfangs- und Knotenregel fest.
Zu zeigen ist, dass sie gemeinsam genau eine Funktion \(F\colon T\to X\)
bestimmen. Wie bei den Wörtern dürfen dabei verschiedene Bäume denselben
Wert erhalten.

\input{tex/b27-tree-recursion-specification}

Für den Existenzbeweis benutzen wir hier die Schnittkonstruktion,
die bereits dem Zahlenrekursionssatz in Band~10 zugrunde lag.
Ein Paar \((t,x)\) hält die Zuordnung eines Werts zu einem Baum fest.
Wir betrachten zunächst alle Teilmengen von \(T\times X\), die die
Blattwerte enthalten und unter der Knotenregel abgeschlossen sind.
Solche Hilfsrelationen müssen noch keine Funktionen sein. Ihr Schnitt
liefert die kleinste abgeschlossene Relation. Die folgenden
Faserfilterbeweise schließen zusätzliche Werte am selben Baum aus;
Bauminduktion zeigt damit, dass jeder Baum genau einen Wert besitzt.
Erst dann ist die Relation als Funktion erwiesen. Die Hilfsrelationen
dienen somit der Konstruktion der gesuchten Funktion; die
Rekursionsvorschrift selbst steht bereits oben.
