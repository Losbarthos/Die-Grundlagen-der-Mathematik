Wie bei einer Zahlenrekursion, die bei \(1\) statt bei \(0\) beginnt,
setzen hier die Anfangswerte erst nach dem ersten Schritt ein. Für
jeden Buchstaben \(a\) ist ein eigener Anfangswert \(f(a)\) am
Einbuchstabenwort vorgesehen. Danach berechnet \(s\) den nächsten Wert
aus dem bisherigen Wert und dem angefügten Buchstaben.

Um den bereits bewiesenen Rekursionssatz vom Leerwort aus anzuwenden,
verwenden wir Hilfswerte in \(\powerset(X)\). Die leere Menge bedeutet:
Es wurde noch kein Buchstabe gelesen. Eine einelementige Menge
\(\{x\}\) bedeutet: Der bisher berechnete Wert ist \(x\).
Beim ersten Buchstaben \(a\) entsteht \(\{f(a)\}\), bei jedem
weiteren aus \(\{x\}\) die Menge \(\{s(x,a)\}\). So benötigen
wir keinen zusätzlich gewählten Anfangswert \(x_0\in X\).

Der folgende Term ist auf allen Teilmengen von \(X\) definiert, damit
die Hilfsschrittfunktion den vom Rekursionssatz geforderten Typ besitzt.
Wortinduktion zeigt anschließend, dass die Hilfsfunktion \(H\) an jedem
nichtleeren Wort eine einelementige Menge liefert. Aus ihr gewinnen wir
den gesuchten Wert zurück. Wir verwenden den Term
\[
