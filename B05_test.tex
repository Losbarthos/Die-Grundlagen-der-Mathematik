...
Peano-Anfangsabschnitte beschrieben. Zunaechst werden nur die
Ausgangszeichen festgelegt. Das Praedikat \(\Finite{M}\) und der Term
\(\tCard(M)\) werden hier nicht definitorisch auf Anfangsabschnittszeugen
zurueckgefuehrt; ihre Bedeutung wird durch die anschliessenden Axiome
festgelegt.

\FormulaDefDeltaK[Definitionelle Ausgangsdaten der endlichen Mengen]{%
  \mathsf{FinSig}(\NatSeg,\Finite,\tCard)%
  ...
  \DeltaRow{\textbf{Primitive Zeichen}}{}
  \DeltaRow{Anfangsabschnitt}{\NatSeg{n}}
  \DeltaRow{Endlichkeitspraedikat}{\Finite{M}}
  \DeltaRow{Kardinalzahlterm}{\tCard(M)}
  \DeltaRow{\textbf{Definitionelle Abkuerzung}}{}
...
Die axiomatische Struktur beginnt danach. Der folgende Block enthaelt nur
Axiomenschemata; Definitionen und neue Symbolnamen sind hier nicht mehr
Bestandteil der Struktur.

\FormulaDefDeltaK[Axiomatische Struktur der endlichen Mengen]{%
  \mathsf{FinAx}(\NatSeg,\Finite,\tCard)%
}{Endliche Mengen}{%
  \DeltaRow{\textbf{Anfangsabschnittsaxiome}}{}
  ...
  \DeltaRow{Ordnungsschnitt}
           {\begin{aligned}[t]
              m\in\mathbb{N}\dsep n\in\mathbb{N}\vdash{}&\\[-2pt]
  ...
  \DeltaRow{Bilder}
           {\begin{aligned}[t]
              C\subseteq A\dsep F\colon A\to B\dsep{}&\\[-2pt]
              \Finite{C}\vdash{}&\Finite{F[C]}
            \end{aligned}}
  \DeltaRow{Transport}
           {\begin{aligned}[t]
              \Finite{M}\dsep M\EqCard N\vdash{}&
              \Finite{N}
            \end{aligned}}
  \DeltaRow{\textbf{Kardinalzahlaxiome}}{}
  \DeltaRow{Wohldefiniertheit}
           {\begin{aligned}[t]
              \Finite{M}\vdash{}&\\[-2pt]
              \exists! n\,\bigl(&n\in\mathbb{N}\land{}\\[-2pt]
              &\NatSeg{n}\EqCard M\bigr)
            \end{aligned}}
  ...
  \DeltaRow{Eindeutigkeit}
           {\begin{aligned}[t]
              n\in\mathbb{N}\dsep m\in\mathbb{N}\dsep{}&\\[-2pt]
              \NatSeg{n}\EqCard M\dsep
              \NatSeg{m}\EqCard M\vdash{}&n=m
            \end{aligned}}
}
...
\noindent
In diesem Testband ist \(\Finite{M}\) ein primitives Praedikat der
axiomatischen Struktur. Die fruehere definitorische Charakterisierung aus
Band~04 wird in B05 durch die Zeugen- und Einfuehrungsaxiome ersetzt.
