%============================================================
%  Bd. 05 Test - Axiomatischer Aufbau der endlichen Mengen
%============================================================
...
\title{Bd. 05 Test - Axiomatischer Aufbau der endlichen Mengen}
\author{Martin Kunze}
\date{}
\setcounter{file}{5}
...
\ifSubfilesClassLoaded{
  \directlua{
    thmlookup.registry_path = "\luaescapestring{\detokenize{registry/_B05_test.registry.tsv}}";
    thmlookup.debug_path    = "\luaescapestring{\detokenize{registry/_B05_test.debug.log}}";
    ...
    texio.write_nl("thmlookup: loaded B01-B04 registries for B05_test")
  }
}{
}
...
% Auch beim Einbinden ueber main.tex wird die B05-Test-Praeambel nicht ausgewertet.
...
\directlua{
  ...
  thmlookup.load_registry_file([==[\luaescapestring{\detokenize{registry/_B03.registry.tsv}}]==]);
  thmlookup.load_registry_file([==[\luaescapestring{\detokenize{registry/_B04.registry.tsv}}]==]);
  texio.write_nl([==[thmlookup: B05_test loaded B01-B04 registries]==])
}

\chapter{Einleitung}

Dieser Testband stellt die endlichen Mengen in axiomatischer Form voran und
uebernimmt danach den in Band~04 ausgearbeiteten Resultatteil. Band~04 bleibt
unveraendert; die folgenden Kapitel sind eine Arbeitskopie fuer den separaten
Aufbau der Theorie der endlichen Mengen.
...
Die neuen Ausgangsdaten dieses Testbands betreffen die Anfangsabschnitte
\(\NatSeg{n}\), den Endlichkeitsbegriff \(\Finite{M}\) und die daraus
abgeleitete endliche Kardinalzahl.

\chapter{Definitionen und Axiome der endlichen Mengen}

Die endlichen Mengen werden in diesem Testband axiomatisch ueber
Peano-Anfangsabschnitte beschrieben. Der folgende Block spielt dieselbe Rolle
wie der Peano-Axiomenblock in Band~04: Er nennt zuerst die definierenden
Symbole und darunter die Axiome, aus denen die spaeteren Beweise in Band~05
arbeiten.

\FormulaDefDeltaK[Allgemeine Definition der endlichen Mengen]{%
  \mathsf{FinAx}(\mathbb{N},0,\Succ,\NatSeg,\Finite,\tCard)%
}{Endliche Mengen}{%
  \DeltaRow{\textbf{Grundlage}}{}
  \DeltaRow{Peano-Struktur}
           {(\mathbb{N},0,\Succ)}
           [\FormulaRefAuto{Peano-Axiome}]
  \DeltaRow{Natuerliche Ordnung}
           {<_{\mathbb{N}}\dsep \leq_{\mathbb{N}}}
           [\FormulaRefAuto{Ordnungsrelation der natuerlichen Zahlen}]
  \DeltaRow{\textbf{Definierende Begriffe}}{}
  \DeltaRow{Anfangsabschnitt}
           {n\in\mathbb{N}\vdash
            \NatSeg{n}\coloneqq \{i\in\mathbb{N}\mid i<n\}}
           [\FormulaRefAuto{PeanoNatSeg}]
  \DeltaRow{Endliche Menge}
           {\Finite{M}\coloneqq
            \exists n\in\mathbb{N}\,(\NatSeg{n}\EqCard M)}
           [\FormulaRefAuto{Begriff der endlichen Menge}]
  \DeltaRow{Endliche Kardinalzahl}
           {\Finite{M}\vdash
            \tCard(M)\coloneqq
            \iota n\,(n\in\mathbb{N}\land \NatSeg{n}\EqCard M)}
           [\FormulaRefAuto{Kardinalzahl einer endlichen Menge}]
  \DeltaRow{\textbf{Axiome der Anfangsabschnitte}}{}
  \DeltaRow{Nullabschnitt}
           {\NatSeg{0}=\varnothing}
           [\FormulaRefAuto{PeanoNatSegZero}[ax]]
  \DeltaRow{Nachfolgerabschnitt}
           {n\in\mathbb{N}\vdash
            \NatSeg{\Succ(n)}=\NatSeg{n}\cup\{n\}}
           [\FormulaRefAuto{PeanoNatSegSucc}[ax]]
  \DeltaRow{Ordnungsschnitt}
           {m\in\mathbb{N}\dsep n\in\mathbb{N}\vdash
            m<n\leftrightarrow m\in\NatSeg{n}}
           [\FormulaRefAuto{PeanoNatSegStrictMembership}[ax]]
  \DeltaRow{\textbf{Axiome der Endlichkeit}}{}
  \DeltaRow{Zeuge}
           {\Finite{M}\vdash
            \exists n\in\mathbb{N}\,(\NatSeg{n}\EqCard M)}
           [\FormulaRefAuto{FiniteWitnessAxiom}[ax]]
  \DeltaRow{Zeuge in Konjunktionsform}
           {\Finite{M}\vdash
            \exists n\,\bigl(n\in\mathbb{N}\land \NatSeg{n}\EqCard M\bigr)}
           [\FormulaRefAuto{FiniteWitnessConjunctionAxiom}[ax]]
  \DeltaRow{Einführung}
           {n\in\mathbb{N}\dsep \NatSeg{n}\EqCard M\vdash \Finite{M}}
           [\FormulaRefAuto{FiniteIntroAxiom}[ax]]
  ...
  \DeltaRow{Endliche Vereinigung}
           {\Finite{A}\dsep \Finite{B}\vdash \Finite{A\cup B}}
           [\FormulaRefAuto{FiniteUnionAxiom}[ax]]
  \DeltaRow{Teilmengen}
           {T\subseteq M\dsep \Finite{M} \vdash \Finite{T}}
           [\FormulaRefAuto{FiniteSubsetAxiom}[ax]]
  \DeltaRow{Bilder}
           {C\subseteq A\dsep F\colon A\to B\dsep \Finite{C} \vdash \Finite{F[C]}}
           [\FormulaRefAuto{FiniteImageAxiom}[ax]]
  \DeltaRow{Gleichmaechtigkeitstransport}
           {\Finite{M}\dsep M\EqCard N\vdash \Finite{N}}
           [\FormulaRefAuto{FiniteEqCardTransportAxiom}[ax]]
  \DeltaRow{\textbf{Axiome der endlichen Kardinalzahl}}{}
  \DeltaRow{Wohldefiniertheit}
           {\Finite{M}\vdash
            \exists! n\,(n\in\mathbb{N}\land \NatSeg{n}\EqCard M)}
           [\FormulaRefAuto{FiniteCardWellDefinedAxiom}[ax]]
  \DeltaRow{Natuerlichkeit}
           {\Finite{M}\vdash \tCard(M)\in\mathbb{N}}
           [\FormulaRefAuto{FiniteCardNaturalAxiom}[ax]]
  \DeltaRow{Zaehlt die Menge}
           {\Finite{M}\vdash \NatSeg{\tCard(M)}\EqCard M}
           [\FormulaRefAuto{FiniteCardCountsAxiom}[ax]]
  \DeltaRow{Eindeutigkeit}
           {n\in\mathbb{N}\dsep m\in\mathbb{N}\dsep
            \NatSeg{n}\EqCard M\dsep \NatSeg{m}\EqCard M\vdash n=m}
           [\FormulaRefAuto{FiniteCardUniqueAxiom}[ax]]
  \DeltaRow{\textbf{Neue Symbole}}{}
  \DeltaRow{Anfangsabschnitt}{\NatSeg{n}}
  \DeltaRow{Endlichkeitspraedikat}{\Finite{M}}
  \DeltaRow{Endliche Kardinalzahl}{\tCard(M)}
}

\FormulaAxiomAutoR[Null-Anfangsabschnitt]{%
  \NatSeg{0}=\varnothing%
}{PeanoNatSegZero}

\FormulaAxiomAutoR[Nachfolger-Anfangsabschnitt]{%
  n\in\mathbb{N}\vdash \NatSeg{\Succ(n)}=\NatSeg{n}\cup\{n\}%
}{PeanoNatSegSucc}

\FormulaAxiomAutoR[Strikte Ordnung als Mitgliedschaft im Anfangsabschnitt]{%
  m\in\mathbb{N}\dsep n\in\mathbb{N}\vdash
  m<n\leftrightarrow m\in\NatSeg{n}%
...
\FormulaAxiomAutoR[Endlichkeit hat einen Anfangsabschnittszeugen in Konjunktionsform]{%
  \Finite{M}\vdash
  \exists n\,\bigl(n\in\mathbb{N}\land \NatSeg{n}\EqCard M\bigr)%
}{FiniteWitnessConjunctionAxiom}

\FormulaAxiomAutoR[Einführung der Endlichkeit aus einem Anfangsabschnittszeugen]{%
  n\in\mathbb{N}\dsep \NatSeg{n}\EqCard M\vdash \Finite{M}%
}{FiniteIntroAxiom}

\FormulaAxiomAutoR[Die leere Menge ist endlich]{%
  \Finite{\varnothing}%
}{FiniteEmptyAxiom}

\FormulaAxiomAutoR[Adjunktion eines neuen Elements erhält Endlichkeit]{%
  a\notin A \dsep \Finite{A} \vdash \Finite{A\cup\{a\}}%
}{FiniteAdjunctionAxiom}

\FormulaAxiomAutoR[Vereinigung endlicher Mengen ist endlich]{%
  \Finite{A}\dsep \Finite{B}\vdash \Finite{A\cup B}%
}{FiniteUnionAxiom}

\FormulaAxiomAutoR[Teilmengen endlicher Mengen sind endlich]{%
  T\subseteq M\dsep \Finite{M} \vdash \Finite{T}%
}{FiniteSubsetAxiom}

\FormulaAxiomAutoR[Bilder endlicher Mengen sind endlich]{%
  C\subseteq A\dsep F\colon A\to B\dsep \Finite{C} \vdash \Finite{F[C]}%
}{FiniteImageAxiom}

\FormulaAxiomAutoR[Transport der Endlichkeit entlang der Gleichmächtigkeit]{%
  \Finite{M}\dsep M \EqCard N \vdash \Finite{N}%
}{FiniteEqCardTransportAxiom}

\FormulaAxiomAutoR[Wohldefiniertheit der endlichen Kardinalzahl]{%
  \Finite{M}\vdash
  \exists! n\,\bigl(n\in\mathbb{N}\land \NatSeg{n} \EqCard M\bigr)%
}{FiniteCardWellDefinedAxiom}

\FormulaAxiomAutoR[Kardinalzahl einer endlichen Menge ist natürlich]{%
  \Finite{M}\vdash \tCard(M)\in\mathbb{N}%
}{FiniteCardNaturalAxiom}

\FormulaAxiomAutoR[Kardinalzahl einer endlichen Menge zählt die Menge]{%
  \Finite{M}\vdash \NatSeg{\tCard(M)}\EqCard M%
}{FiniteCardCountsAxiom}

\FormulaAxiomAutoR[Eindeutigkeit der endlichen Kardinalzahl]{%
  n\in\mathbb{N}\dsep m\in\mathbb{N}\dsep
  \NatSeg{n}\EqCard M\dsep \NatSeg{m}\EqCard M\vdash n=m%
}{FiniteCardUniqueAxiom}

\paragraph{Status dieses Testbands.}
Der folgende Teil ist der Resultatteil aus Band~04 zum Thema endliche Mengen.
Er beweist die oben aufgefuehrten Charakterisierungssaetze im tabellarischen
Sinn und bleibt als Beweis- und Konsistenztest fuer die axiomatische Neuordnung
erhalten.
...
\begin{tabproof}
  \proofstep{1}{\Finite{M}}{\rA}
  \proofstep{1}{\exists n\,\bigl(n\in\mathbb{N}\land \NatSeg{n} \EqCard M\bigr)}{%
    \FormulaRefAuto{FiniteWitnessConjunctionAxiom}[ax]{1}}
