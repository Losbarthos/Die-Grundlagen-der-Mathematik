%============================================================
%  Bd. 47 - Metrische Räume und Vollständigkeit
%============================================================

\documentclass[main.tex]{subfiles}

\ifSubfilesClassLoaded{
  \BandSetupStandalone{B47}
}{
}

\title{Bd. 47 - Metrische Räume und Vollständigkeit}
\author{Martin Kunze}
\date{}
\setcounter{file}{47}

\hypersetup{
  pdftitle={Bd. 47 - Metrische Räume und Vollständigkeit},
...
\FormulaDefDeltaK[Metrik und metrischer Raum]{%
  \operatorname{Metrik}_{X}(d)%
}{MetricSpaceDef}{%
  \DeltaRow{Mengen}{X\dsep x\dsep y\dsep z}
  \DeltaRow{Funktionen}{d}
  \DeltaRow{\textbf{Axiome}}{}
  \DeltaPrem{Abstandsfunktion}{d\colon X\times X\to\mathbb R}
  \DeltaRow{Positivität}{x,y\in X\vdash0\leq d(x,y)}
  \DeltaRow{Definitheit}{x,y\in X\vdash d(x,y)=0\leftrightarrow x=y}
  \DeltaRow{Symmetrie}{x,y\in X\vdash d(x,y)=d(y,x)}
  \DeltaRow{Dreiecksungleichung}{%
    x,y,z\in X\vdash d(x,z)\leq d(x,y)+d(y,z)}
  \DeltaRow{\textbf{Neues Symbol}}{}
  \DeltaPrem{Metrik}{\operatorname{Metrik}_{X}(d)}
}

\FormulaAxiomDeltaK[Typisierung einer Metrik]{%
  \operatorname{Metrik}_{X}(d)\vdash d\colon X\times X\to\mathbb R%
}{MetricFunctionAxiom}{%
  \DeltaRow{Mengen}{X}
  \DeltaRow{Funktionen}{d}
}

\FormulaAxiomDeltaK[Positivität einer Metrik]{%
  \operatorname{Metrik}_{X}(d)\dsep x,y\in X\vdash0\leq d(x,y)%
}{MetricPositivityAxiom}{%
  \DeltaRow{Mengen}{X\dsep x\dsep y}
  \DeltaRow{Funktionen}{d}
}

\FormulaAxiomDeltaK[Definitheit einer Metrik]{%
  \operatorname{Metrik}_{X}(d)\dsep x,y\in X
  \vdash d(x,y)=0\leftrightarrow x=y%
}{MetricDefinitenessAxiom}{%
  \DeltaRow{Mengen}{X\dsep x\dsep y}
  \DeltaRow{Funktionen}{d}
}

\FormulaAxiomDeltaK[Symmetrie einer Metrik]{%
  \operatorname{Metrik}_{X}(d)\dsep x,y\in X\vdash d(x,y)=d(y,x)%
}{MetricSymmetryAxiom}{%
  \DeltaRow{Mengen}{X\dsep x\dsep y}
  \DeltaRow{Funktionen}{d}
}

\FormulaAxiomDeltaK[Dreiecksungleichung einer Metrik]{%
  \operatorname{Metrik}_{X}(d)\dsep x,y,z\in X
  \vdash d(x,z)\leq d(x,y)+d(y,z)%
}{MetricTriangleAxiom}{%
  \DeltaRow{Mengen}{X\dsep x\dsep y\dsep z}
  \DeltaRow{Funktionen}{d}
}
...
\textbf{metrischer Raum}. Die vier Abstandsgesetze heißen Positivität,
...
\begin{proof}
Aus \FormulaRefAuto{MetricTriangleAxiom}[ax] und
\FormulaRefAuto{MetricSymmetryAxiom}[ax] folgen
\[
...
\FormulaRefAuto{RealDistanceLaws}[thm]. Damit sind sämtliche Axiome aus
\FormulaRefAuto{MetricSpaceDef}[def] erfüllt.
\end{proof}
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Positivität, Definitheit und Symmetrie.}\par
Die Werte \(0\) und \(1\) sind nichtnegativ. Nach Definition gilt
\(\delta_X(x,y)=0\) genau für \(x=y\), und die Fallunterscheidung ist
symmetrisch in \(x\) und \(y\). \par\medskip\noindent\textbf{(ii) Dreiecksungleichung.}\par
Für die Dreiecksungleichung ist nur der Fall
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Positivität des kleineren Radius.}\par
Aus \(y\in B_r^d(x)\) folgt \(d(x,y)<r\), also
\(r-d(x,y)>0\). \par\medskip\noindent\textbf{(ii) Inklusion der kleineren Kugel.}\par
Ist
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Von der Folge zur Restfolge.}\par
Konvergiert \(a\), so genügt für die Restfolge derselbe Index, denn
\(n+r\geq n\). \par\medskip\noindent\textbf{(ii) Von der Restfolge zur Folge.}\par
Konvergiert umgekehrt die Restfolge ab dem Index \(N\), so
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Nichtleerheit der Wertemenge.}\par
...
\par\medskip\noindent\textbf{(ii) Von einer reellen Schranke zur Kugel.}\par
Ist \(|a_n|\leq C\) für alle \(n\), so liegt die Wertemenge in
\(B_{C+1}^{d_{\mathbb R}}(0)\). \par\medskip\noindent\textbf{(iii) Von einer Kugel zur reellen Schranke.}\par
Liegt sie umgekehrt in
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Stetigkeit der Identität.}\par
Für die Identität kann zu \(\varepsilon>0\) derselbe Radius
\(\delta=\varepsilon\) verwendet werden. \par\medskip\noindent\textbf{(ii) Stetigkeit der konstanten Abbildung.}\par
Bei der konstanten Abbildung ist
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Stetigkeit erhält Folgengrenzwerte.}\par
Die Hinrichtung ist
\FormulaRefAuto{MetricContinuousMapsPreserveLimits}[thm].

\par\medskip\noindent\textbf{(ii) Das Folgenkriterium erzwingt Stetigkeit.}\par
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Stetigkeit.}\par
Für jedes \(\varepsilon>0\) wählen wir \(\delta=\varepsilon\).
Aus \(d_X(u,x)<\delta\) folgt wegen der Isometrie
\[
 d_Y(f(u),f(x))=d_X(u,x)<\varepsilon.
\]
Damit ist \(f\) an jedem Punkt stetig.

\par\medskip\noindent\textbf{(ii) Erhaltung der Cauchy-Eigenschaft.}\par
Für eine Cauchy-Folge \(a\) gilt für alle \(m,n\in\mathbb N\)
\[
 d_Y(f(a_m),f(a_n))=d_X(a_m,a_n).
\]
Zu jedem \(\varepsilon>0\) ist deshalb jeder Cauchy-Index der Folge
\(a\) auch ein Cauchy-Index der Bildfolge \(f\circ a\).
\end{proof}
...
\FormulaDefDeltaK[Folgenkompakte Teilmenge]{%
  ...
  &\quad\forall a\colon\mathbb N\to K\,
    ...
    \Bigl(\forall n\in\mathbb N\,
      \bigl(\varphi(n)<\varphi(n+1)\bigr)\\[-2pt]
  &\hspace{64mm}\land
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Nichtleerheit und Unbeschränktheit.}\par
...
\(J\subseteq\NatSeg{p+1}\) und damit nach
\FormulaRefAuto{FiniteNatSegSubset}[thm] endlich, ein Widerspruch.

\par\medskip\noindent\textbf{(ii) Wohldefinierter Nachfolgerschritt.}\par
...
\par\medskip\noindent\textbf{(iii) Rekursion und Eigenschaften der Aufzählung.}\par
Wähle ferner \(j_0=\min J\). Der Dedekindsche Rekursionssatz
\FormulaRefAuto{DedekindRecursionTheoremAddOne}[thm] liefert eine Funktion
\(\varphi\colon\mathbb N\to\mathbb N\) mit
\[
 \varphi(0)=j_0,
 \qquad
 \varphi(n+1)=s_J(\varphi(n)).
\]
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Konvergenz erzwingt schließlich konstante Werte.}\par
Konvergiert \(a\) gegen \(x\), so gilt ab einem Index
\(\delta_X(a_n,x)<1/2\). Die diskrete Metrik kann dann nicht den Wert \(1\)
haben, also ist \(a_n=x\). \par\medskip\noindent\textbf{(ii) Schließlich konstante Werte liefern Konvergenz.}\par
Die Rückrichtung folgt unmittelbar, weil eine
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Beschränktheit.}\par
Die Kugel \(B_2^{\delta_{\mathbb N}}(0)\) ist ganz \(\mathbb N\), also ist
der Raum beschränkt. \par\medskip\noindent\textbf{(ii) Fehlende Folgenkompaktheit.}\par
Die kanonische Folge \(a_n=n\) besitzt jedoch keine
...
\FormulaThmDeltaK[Satz von der monotonen Teilfolge]{%
  ...
  &\quad\forall n\in\mathbb N\,
      \bigl(\varphi(n)<\varphi(n+1)\bigr)\land{}\\[-2pt]
...
\]
\par\medskip\noindent\textbf{(i) Unendlich viele Spitzenindizes.}\par
...
folgt
\(a_{\varphi(n+1)}\leq a_{\varphi(n)}\). Die Teilfolge ist monoton
fallend.

\par\medskip\noindent\textbf{(ii) Endlich viele Spitzenindizes.}\par
