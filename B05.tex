...
\chapter{Axiomatische Grundlage}

Band~03 führt das Prädikat \(\Finite{\cdot}\) als primitives Symbol ein. Für den gesamten
vorliegenden Band gelten genau die dort registrierten drei
Endlichkeitsaxiome:
\[
\begin{array}{rcll}
 &\Finite{\varnothing},
 &\qquad&\FormulaRefAuto{FiniteEmptyAxiom}[ax],\\[2pt]
 a\notin A,\ \Finite A
 &\vdash \Finite{A\cup\{a\}},
 &&\FormulaRefAuto{FiniteAdjunctionAxiom}[ax],\\[2pt]
 P(\varnothing),\
 [\Finite A,P(A),a\notin A]\vdots P(A\cup\{a\}),\
 \Finite M
 &\vdash P(M),
...
Als Brücke zur üblichen Darstellung steht aus Band~03 ausschließlich die dort
aus den drei Axiomen bewiesene Bijektionscharakterisierung zur Verfügung:
\[
  \Finite M\eqvdash
  \exists n\in\mathbb N\,\exists F\colon n\bij M.
\]
Dies ist \FormulaRefAuto{FiniteBijectionEquiv}[thm].
Dabei ist \(n\) die von-Neumann-Zahl selbst. Die später verwendeten
Peano-Anfangsabschnitte \(\NatSeg{n}\) sind davon zu unterscheiden und werden
nur in ausdrücklich entsprechend bezeichneten Sätzen benutzt.
...
\FormulaThmDeltaK[Charakterisierung durch Gleichmächtigkeit]{%
  \Finite M\eqvdash
...
\begin{tabproofsplit}
  \proofpart{\(\vdash\)}
    ...
    \proofstep{1}{\exists n\in\mathbb N\,\exists F\colon n\bij M}{%
      \FormulaRefAuto{FiniteBijectionEquiv}[thm]{1}}
    \proofstep{3}{n\in\mathbb N\land \exists F\colon n\bij M}{\rA}
    \proofstep{3}{n\in\mathbb N}{\rAEa{3}}
    \proofstep{3}{\exists F\colon n\bij M}{\rAEb{3}}
    \proofstep{3}{n\EqCard M}{%
      \FormulaRefAuto{A\EqCard B\coloneqq \exists F\colon A\bij B}{5}}
    \proofstep{3}{n\in\mathbb N\land n\EqCard M}{\rAI{4,6}}
    \proofstep{3}{\exists n\in\mathbb N\,(n\EqCard M)}{\rEI{7}}
    \proofstep{1}{\exists n\in\mathbb N\,(n\EqCard M)}{\rEE{2,3,8}}
  \closeproofpart

  \proofpart{\(\dashv\)}
    \proofstep{1}{\exists n\in\mathbb N\,(n\EqCard M)}{\rA}
    \proofstep{2}{n\in\mathbb N\land n\EqCard M}{\rA}
    \proofstep{2}{n\in\mathbb N}{\rAEa{2}}
    \proofstep{2}{n\EqCard M}{\rAEb{2}}
    \proofstep{2}{\exists F\colon n\bij M}{%
      \FormulaRefAuto{A\EqCard B\coloneqq \exists F\colon A\bij B}{4}}
    \proofstep{2}{n\in\mathbb N\land\exists F\colon n\bij M}{\rAI{3,5}}
    \proofstep{2}{\exists n\in\mathbb N\,\exists F\colon n\bij M}{\rEI{6}}
    \proofstep{2}{\Finite M}{\FormulaRefAuto{FiniteBijectionEquiv}[thm]{7}}
    \proofstep{1}{\Finite M}{\rEE{1,2,8}}
  \closeproofpart
\end{tabproofsplit}
...
\begin{tabproof}
  ...
  \proofstep{1}{\exists n\in\mathbb N\,(n\EqCard M)}{%
  ...
  \proofstep{2,4}{\exists n\in\mathbb N\,(n\EqCard N)}{\rEI{8}}
  \proofstep{2,4}{\Finite N}{\FormulaRefAuto{FiniteEqCardEquiv}[thm]{9}}
  \proofstep{1,2}{\Finite N}{\rEE{3,4,10}}
\end{tabproof}
...
\begin{tabproof}
  ...
  \proofstep{1,2}{\Finite N}{\FormulaRefAuto{FiniteEqCardTransport}[thm]{1,3}}
\end{tabproof}
...
Die Transportaussagen wurden oben ausschließlich aus
\FormulaRefAuto{FiniteBijectionEquiv}[thm] und den allgemeinen Regeln der
Gleichmächtigkeit hergeleitet.
...
\begin{tabproof}
  ...
  \proofstep{3}{A=A\cup\{a\}}{\FormulaRefAuto{a=b\vdash b=a}{4}}
  \proofstep{1,3}{\Finite{A\cup\{a\}}}{\rIE{5,1}}
  \proofstep{7}{a\notin A}{\rA}
  \proofstep{1,7}{\Finite{A\cup\{a\}}}{%
    \FormulaRefAuto{FiniteAdjunction}[thm]{7,1}}
...
% Der folgende von-Neumann-Induktionsblock ist aktiv.
...
\begin{tabproof}
  \proofstep{}{0\in\mathbb{N}}{\FormulaRefAuto{PeanoZero}[ax]}
  \proofstep{}{0=\varnothing}{\FormulaRefAuto{PeanoNatSegZero}}
  \proofstep{}{\varnothing \EqCard \varnothing}{\FormulaRefAuto{A \EqCard A}}
  \proofstep{}{0 \EqCard \varnothing}{\rIE{2,3}}
  \proofstep{}{0\in\mathbb{N}\land 0 \EqCard \varnothing}{\rAI{1,4}}
  \proofstep{}{\exists n\in\mathbb{N}\,n \EqCard \varnothing}{\rEI{5}}
  \proofstep{}{\Finite{\varnothing}}{%
    \FormulaRefAuto{\Finite{M}\coloneqq \exists n\in\mathbb{N}\,\bigl(n \EqCard M\bigr)}{6}}
\end{tabproof}
\fi
...
\begin{tabproof}
  ...
  \proofstep{1,8}{\bot}{\rBI{9,\FormulaRefAuto{a\not\in a}}}
...
% Ende des von-Neumann-Induktionsblocks
...
\begin{tabproof}
  ...
  \proofstep{2}{\exists n\in\mathbb{N}\,\exists F\colon n\bij M}{%
    \FormulaRefAuto{FiniteBijectionEquiv}[thm]{2}}
...
\FormulaThmDeltaK[Bilder endlicher Mengen sind endlich]{%
...
\begin{tabproof}
  ...
  \proofstep{1}{\neg\exists H\colon \mathbb{N}\inj n}{%
    \FormulaRefAuto{VonNeumannNoNatInjection}[thm]{1}}
...
\FormulaThmDelta[Selbstinjektionen natürlicher Zahlen sind bijektiv]{%
  n\in\mathbb{N}\dsep F\colon n\inj n
  \vdash F\colon n\bij n
}{%
  \DeltaRow{Mengen}{n}
  \DeltaRow{Funktionen}{F\dsep G}
}
...
\FormulaThmDelta[Keine Injektion in kleinere natürliche Zahl]{%
  ...
  \dsep F\colon n\inj m \vdash \bot
}{%
  \DeltaRow{Mengen}{m\dsep n}
  \DeltaRow{Funktionen}{F}
  \DeltaRow{Relationen}{<_{\mathbb{N}}}
...
\FormulaDefDeltaK[Kardinalzahl einer endlichen Menge]{%
  \Finite{M}\vdash
  \tCard(M) \coloneqq
  \iota n\,\bigl(n\in\mathbb{N}\land n \EqCard M\bigr)
}{FiniteCardinality}{%
  \DeltaRow{Mengen}{M\dsep n}
...
\FormulaDefDeltaK[Voraussetzung der Frankl-Vermutung]{%
  ...
  \DeltaRow{Endlichkeit}
           {\Finite{M}}
           [\parbox[t]{3cm}{%
             \FormulaRefAuto{FiniteBijectionEquiv}[thm]\\
