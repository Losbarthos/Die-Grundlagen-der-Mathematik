...
zurückgeführt werden kann.  Ein abschließendes Kapitel fragt dann, welche
Informationen sich aus einer Potenzhalbgruppe wieder über ihre
Ausgangshalbgruppe gewinnen lassen.  Die dabei sichtbare Schwierigkeit,
Einermengen rein multiplikativ zu erkennen, führt schrittweise zur
Potenzhalbgruppenvermutung.  Kurze Kopfübungen mit getrennten Hinweisen und
Lösungen schließen den Band ab.
...
Tafeln, aber noch kein allgemeiner Beweis der später formulierten
...
\[
  ...
  \{a^r\mid r\geq1\}
\]
die von \(a\) erzeugte \emph{Unterhalbgruppe}.  Wegen
\(a^ra^s=a^{r+s}\) ist diese Menge unter der Multiplikation abgeschlossen.
Eine Halbgruppe heißt \emph{monogen}, wenn ihr Träger gleich
\(\langle a\rangle\) für ein Element \(a\) ist.  Im Folgenden wird also
\(\langle0\rangle\) untersucht.  Der Großbuchstabe \(N\) bezeichnet dabei
stets die Trägergröße und entspricht der Länge der zuvor verwendeten
Nummerierung \(\NatSeg{n}\).
...
\]
Die Assoziativität ist damit nicht nur behauptet, sondern direkt ablesbar:
\[
\begin{aligned}
  (x_r x_s)x_t
  &=x_{\rho(\rho(r+s)+t)}
...
verschiedene Vorlauf- beziehungsweise Periodenlängen und sind daher nicht
isomorph.
...
Wiederholungspunkten.  Nachdem \(x_1=0,x_2=2,x_3=1\) feststehen, zeigt der
gesamte Rekursionsbaum auf \(S_3\) nur noch drei Äste:
\[
\begin{array}{c|c|c}
...
Der erste Ast ist das Leitbeispiel; der zweite besitzt denselben Typ
\((\mu,\lambda)=(2,2)\) wie das folgende Kontrastbeispiel.
...
Sie analysiert zunächst nur \(\langle0\rangle\) und liefert im monogenen Fall
...
\end{array}.
\]
Hier ist \(1\) neutral, \(0\) absorbierend und \(2\star2=0\).  Für die drei
...
\]
ein Halbgruppenisomorphismus.  Falls bereits bekannt ist, dass
\[
...
\end{aligned}
\]
Somit ist \(f\) ein Isomorphismus der Ausgangshalbgruppen.  Der offene Kern
des Problems kann deshalb in einem Satz zusammengefasst werden:
\[
  \boxed{\text{Wie erkennt man die Einermengen nur an der Multiplikation?}}
\]
...
etwa Idempotenz sowie Zeilen- und Spaltenprofile.
\end{remark}
...
Potenzhalbgruppenvermutung}.  Für endliche Halbgruppen sind alle Teilmengen
endlich; die große und die finitäre Potenzhalbgruppe fallen daher zusammen.
...
Die in Band~15 bewiesene umgekehrte Richtung gilt sogar ohne Endlichkeit:
...
werden darf.  Eine \emph{einseitige Starrheitsaussage} ist nochmals stärker:
Dabei wird nur eine Grundhalbgruppe einer speziellen Klasse vorausgesetzt,
während die andere zunächst beliebig sein darf.
...
\begin{itemize}
  \item Ohne Endlichkeitsvoraussetzung ist die allgemeine Aussage falsch.
    Das in Band~15 vollständig ausgeführte Gegenbeispiel geht zurück auf
  ...
  \item Sind beide Halbgruppen idempotent, so ist ihre Klasse global bestimmt
    \href{https://doi.org/10.1080/00927872.2017.1319474}%
      {(Yu--Zhao--Gan, 2018)}.
  \item Sind beide Halbgruppen vollständig regulär, so ist auch diese Klasse
    global bestimmt
    \href{https://doi.org/10.1080/00927872.2026.2659846}%
      {(Yu--Zhao, 2026)}.
  \item Ist eine Grundhalbgruppe eine Gruppe, darf die andere zunächst eine
    beliebige Halbgruppe sein; aus isomorphen großen Potenzhalbgruppen folgt
...
Die folgenden Aufgaben sind bewusst so kurz gehalten, dass keine vollständige
Tafel mitgeschrieben werden muss.  Die Aufgaben \(1\)--\(4\) sind zum
Aufwärmen, \(5\)--\(8\) bilden den mittleren Aufguss und \(9\)--\(12\) führen
bis an die Rekonstruktionsfrage.  Hinweise und Kurzlösungen stehen getrennt
hinter den Aufgaben.

\section{Aufgaben}

\begin{enumerate}
  \item Eine Halbgruppe \(A\) habe vier Elemente.  Wie viele Elemente besitzt
    \(\PowNE{A}\)?  Wie viele davon liegen in \(P_1,P_2,P_3,P_4\)?
  ...
  \item Auf \(S_5=\{0,1,2,3,4\}\) beginne die Potenzfolge mit
    \[
      0,2,4,2,4,\ldots.
    \]
    Bestimme \(\mu,\lambda\) und \(\langle0\rangle\).  Legt die Folge die
...
\begin{enumerate}
  ...
  \item Suche die erste Wiederholung und notiere, welche Elemente von
    \(S_5\) nie auftreten.
  \item Das Tripel \((0,0,0)\) erzwingt
    \(p(1,0)=p(0,1)\).  Nenne den gemeinsamen Wert \(u\) und unterscheide
    \(u=0\) und \(u=1\).
...
\begin{enumerate}
  \item Es gilt \(|\PowNE{A}|=2^4-1=15\).  Die Blockgrößen sind
    \(4,6,4,1\).
  ...
  \item Die erste Wiederholung ist \(x_4=x_2\), also
    \((\mu,\lambda)=(2,2)\) und
    \(\langle0\rangle=\{0,2,4\}\).  Weil \(1\) und \(3\) fehlen, legt die
    Folge die ganze Tafel im Allgemeinen nicht fest.

  \item Setze \(u=p(1,0)=p(0,1)\).  Für \(u=0\) und \(u=1\) wird jeweils
    \(p(1,1)=1\) erzwungen.  Die beiden Tafelwörter lauten \(1001\) und
    \(1111\).
