...
\FormulaDefDeltaK[Die beiden Träger und ihre Multiplikationen]{%
  \begin{gathered}
    \begin{aligned}
      ...
      T&\coloneqq S\cup\{k\},\\[2pt]
      \MogStar&\colon S\times S\to S,&
      \MogDiamond&\colon T\times T\to T
    \end{aligned}\\[3pt]
    \begin{aligned}
      i,j\in\mathbb N
        &\vdash a_i\MogStar a_j\coloneqq b_{ij},\\[-2pt]
      x,y\in S\dsep(x\notin L\lor y\notin L)
        &\vdash x\MogStar y\coloneqq\mathbf0,\\[2pt]
      i,j\in\mathbb N
        &\vdash a_i\MogDiamond a_j\coloneqq b_{ij},\\[-2pt]
      x,y\in T\dsep(x\notin L\lor y\notin L)
...
niemals als Produkt auftritt; das zeigt \textup{(M4)}. Gäbe es einen
Isomorphismus \(f\), so hätte \(k\) wegen der Surjektivität ein Urbild
\(s=a_i\in L\). Dann wären aber
\[
  f(b_{i0})=k\MogDiamond f(a_0)=\mathbf0
  \qquad\text{und}\qquad
  f(b_{i1})=k\MogDiamond f(a_1)=\mathbf0.
\]
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[]{%
    ...
    (M4)--(M5) Produktstatus und Kollaps}
  \proofstepwidestar[1,2,3]{%
    \begin{aligned}[t]
      &\SemigroupAlg{S}{\MogStar}\land
        \SemigroupAlg{T}{\MogDiamond}\land{}\\[-2pt]
...
Für die Reserve verwenden wir die in Band~17 definierten Mengen und
Bijektionen
\[
  \begin{gathered}
...
\FormulaThmDeltaK[Argument (M5): Kollaps eines angenommenen Isomorphismus]{%
  ...
  \DeltaRow{Mengen}{L\dsep D\dsep S\dsep T\dsep\mathbf0\dsep k
    \dsep s\dsep a_i\dsep a_0\dsep a_1\dsep b_{i0}\dsep b_{i1}
    \dsep i}
...
\(k\) ein Urbild \(s\in S\). Läge \(s\notin L\), so wäre \(s\) nach
\FormulaRefAuto{MogiljanskajaArgumentM4ProductStatus}[thm] ein Produkt
\(u\MogStar v\). Die Homomorphie von \(f\) würde dann
\[
  k=f(s)=f(u)\MogDiamond f(v)
\]
liefern, obwohl \(k\) nach demselben Satz in \(T\) kein Produkt ist. Also
muss \(s\in L\) gelten.
...
\begin{tabproofwide}
  ...
  \proofstepwidestarbreak[4,6]{s\notin L\vdash
    \exists u\in T\,\exists v\in T\,(k=u\MogDiamond v)}{%
    \FormulaRefAuto{MogiljanskajaArgumentM4ProductStatus}[thm]{6}}
  \proofstepwidestar[7]{s\in L}{%
    \FormulaRefAuto{MogiljanskajaArgumentM4ProductStatus}[thm]}
  \proofstepwidestar[8]{\exists i\in\mathbb N\,(s=a_i)}{%
    \FormulaRefAuto{MogiljanskajaLayerLMembership}[thm]{8}}
  \proofstepwidestar[9]{s=a_i}{\rA}
  \proofstepwidestar[4,6,10]{f(b_{i0})=\mathbf0}{%
    \ensuremath{b_{i0}=a_i\MogStar a_0\dsep
      k\MogDiamond f(a_0)=\mathbf0}}
  \proofstepwidestar[4,6,10]{f(b_{i1})=\mathbf0}{%
    \ensuremath{b_{i1}=a_i\MogStar a_1\dsep
      k\MogDiamond f(a_1)=\mathbf0}}
  \proofstepwidestar[]{b_{i0}\neq b_{i1}}{%
    \FormulaRefAuto{MogiljanskajaBIndexUnique}[thm],
    \FormulaRefAuto{PeanoZeroNeOne}[thm]}
  \proofstepwidestar[3,11,12,13]{\bot}{%
    Injektivität von \(f\)}
  \proofstepwidestar[2]{\bot}{\rEE{5,6,14}}
  \proofstepwidestar[1]{\bot}{\rEE{1,2,15}}
  \proofstepwidestar[]{\neg\exists f\,
    \SemigroupIso{f}{S,\MogStar}{T,\MogDiamond}}{\rCI{1,16}}
\end{tabproofwide}
...
\FormulaRefAuto{MogiljanskajaReserveMapsDef}[def] ist
\(\mathcal E=\mathfrak I[\{k\},D\cup\{k\}]\). Weil \(k\notin D\),
...
\(R(\Phi(X),\Phi(Y))=R(X,Y)\). Der Nullanteil
