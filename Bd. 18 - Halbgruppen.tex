...
Die Beweise dieses Gegenbeispiels werden zunächst nur metasprachlich
zusammengefasst. Jede Tabellenzeile nennt genau das tragende Argument; die
Kürzel \textup{(M1)} bis \textup{(M17)} verweisen auf die gleich bezeichneten
Einzelsätze im formalen Anhang.
...
Der Haupttext verwendet die Argumente \textup{(M1)} bis \textup{(M17)} nur
als knappe metasprachliche Begründungen. In diesem Anhang wird jedes dieser
Argumente als eigener Satz formuliert und bewiesen. Die Reihenfolge stimmt
mit den Beweistabellen des Haupttextes überein; insbesondere bleibt der dort
verwendete direkte Weg über \(R\) und \(\varepsilon\) erhalten.
