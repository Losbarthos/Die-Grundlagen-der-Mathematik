...
Tarski-Maximum, dann laesst sich jedes \(X\in B\) echt vergroessern.
Eine Wahlfunktion liefert daraus Schritte \(S(X)\), eine zweite Wahl
waehlt je Schritt ein frisches Element \(D(X)\in S(X)\setminus X\). Die
Rekursion entlang \(S\) waechst monoton; die jeweils frisch gewaehlten
Elemente bleiben an verschiedenen Stufen verschieden. So entsteht eine
Injektion \(\mathbb{N}\inj M\). Dedekind-Endlichkeit schliesst dieses
Gegenbeispiel aus, und aus \(\Finite{M}\) folgt die benoetigte
