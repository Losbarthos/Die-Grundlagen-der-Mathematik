...
\begin{AxTable}
  ...
  \AxLine{Funktionale Eindeutigkeit}{(x,y)\in R\dsep (x,z)\in R \vdash y=z}[mit \(R\coloneq F\)]
  \AxLine{Totalität auf \(A\)}{x\in A \vdash \exists y\,(x,y)\in R}[mit \(R\coloneq F\)]
  \AxLine{Injektivität}{F(x)=F(y)\vdash x=y}
\end{AxTable}
\end{definition}
...
\proofstep{1,2}{x_1=x_2}{\FormulaRefAuto{F(x)=F(y)\vdash x=y}{7}}
\end{tabproof}
...
\FormulaThmDelta{F \textrm{ bijektiv} \vdash F^{-1}\colon B\to A \textrm{ ist eine Funktion}}
{
\DeltaRow{Mengen}{A\dsep B}
\DeltaRow{Funktionen}{F\colon A\to B}
}
\begin{tabproof}
  \proofstep{1}{F \textrm{ bijektiv}}{\rA}
\end{tabproof}


\FormulaAxiomDelta[Injektivität von \(F^{-1}\)]%
{F^{-1}(x)=F^{-1}(y)\vdash x=y}%
{
\DeltaRow{Mengen}{x\dsep y\dsep A\dsep B}
...
\FormulaThmAuto[Totalität von \(F^{-1}\)]{%
\bigl(F \text{ ist surjektiv}\bigr) \;\rightarrow\;
\bigl(\forall y\in B\;\exists x\in A\; (y,x)\in F^{-1}\bigr)
}
[Unter der Voraussetzung, dass \(F\) surjektiv ist, gilt: \(F^{-1}\) ist total (auf ganz \(B\)).]

\FormulaThmAuto[Umkehrfunktion bei Bijektivität]{%
\bigl(F \text{ ist bijektiv}\bigr) \;\rightarrow\; F^{-1} \text{ ist eine Funktion } B\to A
}
[Wenn \(F\colon A \to B\) bijektiv ist, dann ist die inverse Relation \(F^{-1}\) eine Umkehrfunktion von \(B\) nach \(A\).]

\FormulaThmAuto[Rückgängigmachung]{%
\forall x \in A\; \bigl(F^{-1}(F(x)) = x\bigr)\;\land\;\forall y \in B\; \bigl(F(F^{-1}(y)) = y\bigr)
}
