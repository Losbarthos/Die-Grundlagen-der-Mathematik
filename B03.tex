...
Eine \textbf{paarweise disjunkte Familie nichtleerer Mengen} ist eine Menge \(A\) von Mengen, die die folgenden der \textbf{Nichtleerheit} und \textbf{Paarweise Disjunktheit} erfüllt.
\end{definition}
...
\FormulaThmAuto{(x,y)\in F\eqvdash (y,x)\in F^{-1}}[Seien \(A,\,B\) Mengen, \(F \colon A \to B\), \(x\in A\) und \(y\in B\), dann gilt:]
\begin{tabproofsplit}
\proofpart{\(\vdash\)}
...
\FormulaThmAuto[Eindeutigkeit von \(F^{-1}\)]{F\colon A\hookrightarrow B, F^{-1}(y,x_1)\in F^{-1} ,\, (y,x_2)\in F^{-1} \vdash x_1 = x_2}[
Seien \(A,\,B\) Mengen, \(x_1,x_2\in A\), \(y\in B\). Dann gilt:]
\begin{tabproof}
\prooftext{Seien \(A,\,B\) Mengen, \(x_1,x_2\in A\) und \(y\in B\), }
\prooftext{Sei ferner \(F\colon A\hookrightarrow B\) eine injektive Funktion, dann gilt:}
\proofstep{1}{(y,x_1)\in F^{-1}}{\rA}
\proofstep{2}{(y,x_2)\in F^{-1}}{\rA}
\proofstep{1}{(x_1,y)\in F}{\FormulaRefAuto{x \in \{x \in A \mid P(x)\}\vdash P(x)}{1}}
\proofstep{2}{(x_2,y)\in F}{\FormulaRefAuto{x \in \{x \in A \mid P(x)\}\vdash P(x)}{2}}
...
\proofstep{1,2}{x_1=x_2}{\FormulaRefAuto{F(x_1)=F(x_2)\ \vdash\ x_1=x_2}{7}}
\end{tabproof}


\FormulaThmAuto[Totalität von \(F^{-1}\)]%
{F\colon A\twoheadrightarrow B \vdash \exists x\in A\; (y,x)\in F^{-1}}%
[Seien \(A,\,B\) Mengen und \(y\in B\). Dann gilt:]
\begin{tabproof}
  \prooftext{Seien \(A,B\) Mengen und \(y\in B\). }
  \prooftext{Sei ferner \(F\colon A\twoheadrightarrow B\) surjektiv, dann gilt:}
  \proofstep{}{\exists x\in A\; y=F(x)}{\FormulaRefAuto{\exists x\in A\; F(x)=y}}
  \proofstep{2}{x\in A\land y=F(x)}{\rA}
  \proofstep{2}{x\in A}{\rAEa{2}}
  \proofstep{2}{(x,F(x))\in F}{\FormulaRefAuto{(x,F(x))\in F}{3}}
  \proofstep{2}{(F(x),x)\in F^{-1}}{\FormulaRefAuto{(x,F(x))\in F}{3}}
