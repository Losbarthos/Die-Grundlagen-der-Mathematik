...
\FormulaDefDeltaK[Paarweise verschiedene Viererfolge]{%
  \mathsf{PairDiff}_4(p,q,r,s)\coloneqq
  p\neq q\land p\neq r\land p\neq s\land
  q\neq r\land q\neq s\land r\neq s
}{Paarweise verschiedene Viererfolge}{%
  \DeltaRow{Mengen}{p\dsep q\dsep r\dsep s}
  \DeltaRow{Neue Symbole}{\mathsf{PairDiff}_4}
}

\FormulaDefDeltaK[Vierpunktige Grundmenge]{%
  \mathsf{FourPts}(U;p,q,r,s)\coloneqq
  U=\{p,q,r,s\}\land \mathsf{PairDiff}_4(p,q,r,s)
}{Vierpunktige Grundmenge}{%
  \DeltaRow{Mengen}{U\dsep p\dsep q\dsep r\dsep s}
  \DeltaRow{Neue Symbole}{\mathsf{FourPts}}
}

\FormulaDefDeltaK[Fränkelsche Vierpunktfamilie]{%
  \mathsf{F}_2(U)\coloneqq
  \{\varnothing\}\cup
  \{X\in\powerset(U)\mid \exists x\in X\,\exists y\in X\,(x\neq y)\}
}{Fränkelsche Vierpunktfamilie}{%
  \DeltaRow{Mengen}{U\dsep X\dsep x\dsep y}
  \DeltaRow{Neue Symbole}{\mathsf{F}_2}
}

\FormulaThmDelta[Vierpunktfamilie mit doppelter Vereinigung]{%
  \mathsf{FourPts}(U;p,q,r,s)\vdash \exists M\,\mathsf{DupUCF}(M)
}{%
  \DeltaRow{Mengen}{U\dsep M\dsep M_0\dsep A\dsep a\dsep b\dsep c\dsep d}
  \DeltaRow{Teilmengenfamilien}{M_{\bar a}}
}
Setze im Beweis \(M_0\coloneqq \mathsf{F}_2(U)\).  Die
Vereinigungsschließung ist unmittelbar: Die Vereinigung mit
\(\varnothing\) ändert nichts, und die Vereinigung zweier Teilmengen von
\(U\), die jeweils mindestens zwei verschiedene Elemente besitzen, besitzt
wieder mindestens zwei verschiedene Elemente.

Für \(\mathsf{DupCov}(M_0)\) sei \(A\in M_0\) und \(a\in A\). Dann ist
\(A\neq\varnothing\), also liegt \(A\) nicht im linken Summanden
\(\{\varnothing\}\); folglich enthält \(A\) mindestens zwei verschiedene
Elemente. Falls \(A\setminus\{a\}\) mindestens zwei verschiedene Elemente
\(b,c\) enthält, wähle
\[
  I_1=\varnothing,\qquad I_2=\{b,c\}.
\]
Dann liegen \(I_1,I_2\in (M_0)_{\bar a}\), sie sind verschieden, und
\(I_1\cup A=I_2\cup A=A\). Bleibt der Fall, dass
\(A\setminus\{a\}=\{b\}\). Wegen der vier paarweise verschiedenen Punkte
von \(U\) gibt es dann zwei verschiedene Elemente \(c,d\in U\setminus A\).
Mit
\[
  I_1=\{c,d\},\qquad I_2=\{b,c,d\}
\]
gilt wieder \(I_1,I_2\in (M_0)_{\bar a}\), \(I_1\neq I_2\), und nun
\(I_1\cup A=I_2\cup A=U\).
\begin{tabproof}
  \proofstep{1}{\mathsf{FourPts}(U;p,q,r,s)}{\rA}
  \proofstep{1}{M_0=\mathsf{F}_2(U)}{\text{Setze }M_0\coloneqq\mathsf{F}_2(U)}
  \proofstep{1}{\varnothing\in M_0}{\text{Df. }\mathsf{F}_2}
  \proofstep{1}{M_0\neq\varnothing}{\text{aus Zeile }3}
  \proofstep{1}{M_0\subseteq\powerset(U)}{\text{Df. }\mathsf{F}_2}
  \proofstep{1}{\Finite{U}}{\text{aus der vierpunktigen Darstellung von }U}
  \proofstep{1}{\Finite{\powerset(U)}}{%
    \text{Potenzmenge einer endlichen Menge}}
  \proofstep{1}{\Finite{M_0}}{%
    \text{Teilmenge einer endlichen Menge}}
  \proofstep{1}{\UnionClosedFam(M_0)}{%
    \text{Vereinigungsschließung wie oben}}
  \proofstep{1}{\mathsf{DupCov}(M_0)}{%
    \text{die beiden oben konstruierten Zeugen }I_1,I_2}
  \proofstep{1}{\mathsf{DupUCF}(M_0)}{%
    \FormulaRefAuto{Doppelt überdeckende Vereinigungsfamilie}[def]{4,8,9,10}}
  \proofstep{1}{\exists M\,\mathsf{DupUCF}(M)}{\rEI{11}}
\end{tabproof}

Da es vier paarweise verschiedene Mengen gibt, etwa die ersten vier
von-Neumann-Zahlen, liefert der Satz insbesondere die Existenzformel
\[
  \begin{aligned}
    \exists M\Bigl(&
      M\neq\varnothing\land \Finite{M}\land \UnionClosedFam(M)\land{}\\
