...
\FormulaDefDeltaK[Begriff der Menge (Zermelo-Fraenkel-Axiome)]{\in \text{(zweistelliges Prädikat)}}{Menge}{
  ...
  \DeltaRow{Vereinigung}
           ...
           [\FormulaRefAutoFwd{\exists C\;\forall x\;\bigl(x \in C \;\leftrightarrow\;\exists B\,(B \in A \,\land\, x \in B)\bigr)}]
  %
  ...
  \DeltaRow{Unendlichkeit}
           ...
           [\FormulaRefAutoFwd{\exists A\,(\emptyset \in A \land \forall x \in A\,(x \cup \{x\} \in A))}]
%
    \DeltaRow{\textbf{Neue Symbole}}{}
    \DeltaRow{Mengen}{A\dsep B\dsep C\dsep O\dsep x}
    %%begin novalidate
    \DeltaRow{Leere Menge}{\emptyset}[\FormulaRefAutoFwd{\emptyset := \iota O\bigl(\forall x\,(x \not\in O)\bigr)}]
    %%end novalidate
    \DeltaRow{Funktionen}{F\colon A\to B}[\FormulaRefAutoFwd{Funktion}]
    \DeltaRow{Surjektive Funktionen}{G\colon B\sur A}[\FormulaRefAutoFwd{Surjektive Funktion}]
...
\FormulaThmDelta{B\in A\dsep x\in B\vdash x \in \bigcup A}%
{%
\DeltaRow{Mengen}{x\dsep A\dsep B}%
}
\begin{tabproof}
  ...
  \proofstep{1,2}{ \exists B\,B\in A\land x\in B }{\rEI{3}}
  \proofstep{1,2}{ x \in \bigcup A }{\FormulaRefAuto{x \in \bigcup A \;\eqvdash\;\exists B\,(B \in A \,\land\, x \in B)}{4}}
\end{tabproof}
...
\section{Hilfsbegriffe}

\subsection{Axiome zur disjunkten Familie}
...
% --- Totale Relation als Hilfsbegriff (Axiome existieren schon beim Funktionsbegriff) ---
% WICHTIG: In der Referenzspalte muss die Formel exakt so stehen, wie das Axiom damals eingeführt wurde.
% (Falls dein Totalitätsaxiom damals mit F formuliert wurde, bleibt hier F im \FormulaRefAuto stehen.)
...
% --- Familie paarweise disjunkter, nichtleerer Mengen ---
...
\section{Äquivalente Formulierungen des Auswahlprinzips}

\FormulaDefDelta[Auswahlprinzip (AC – Surjektionsform)]{%
  \ACsur \coloneqq
  \exists F\colon A \to B\,\bigl(G \circ F = \Id_A\bigr)
}{
  \DeltaRow{Mengen}{A \dsep B}
  \DeltaRow{Surjektive Funktion}{G\colon B \sur A}
}

\FormulaDefDelta[Auswahlprinzip (AC – Relationsform)]{%
  \ACrel \coloneqq
  \exists F\colon A \to B\,\forall x\in A\,\bigl((x,F(x))\in R\bigr)
}{
  \DeltaRow{Mengen}{A \dsep B}
  \DeltaRow{Totale Relation}{\TotRel{R,A,B}}
}

\FormulaDefDelta[Auswahlprinzip (AC – Familienform)]{%
  \ACfam \coloneqq
  \exists L\,\forall X\in M\,\exists! a\,\bigl(a\in X \land a\in L\bigr)
}{
  \DeltaRow{Mengen}{M}
  \DeltaRow{Paarweise disjunkte nichtleere Familie}{\DisjFam(M)}
}


\section{Surjektionsform nach Relationsform}
...
\begin{tabproof}
  ...
  \proofstep{}{\pi_1\restriction_R\colon R\to A}{%
    \FormulaRefAuto{C\subseteq A \vdash F\restriction_C\colon C\to B}{1,2}}
\end{tabproof}

\FormulaThmDelta
{\pi_2\restriction_R\colon R\to B}
{
  \DeltaRow{Mengen}{A\dsep B}
  \DeltaRow{Totale Relation}{\TotRel{R,A,B}}
}
\begin{tabproof}
  ...
  \proofstep{}{\pi_2\restriction_R\colon R\to B}{%
    \FormulaRefAuto{C\subseteq A \vdash F\restriction_C\colon C\to B}{1,2}}
\end{tabproof}
...
\FormulaThmDelta{%
  \pi_1\restriction_R\circ G = \Id_A\dsep x\in A
  \vdash \pi_1(G(x))=x
}{
  \DeltaRow{Mengen}{A\dsep B}
  \DeltaRow{Totale Relation}{\TotRel{R,A,B}}
  \DeltaRow{Funktion}{G\colon A\to R}
  \DeltaRow{Projektion auf erste Komponente}{\pi_1:A\times B\to A}
}
\begin{tabproofwide}
  \proofstepwidestar[1]{\pi_1\restriction_R\circ G = \Id_A}{\rA}
  \proofstepwidestar[2]{x\in A}{\rA}
  \proofstepwidestar[]{R\subseteq A\times B}{\FormulaRefAuto{F \subseteq A \times B}}
  \proofstepwidestar[2]{G(x)\in R}{\FormulaRefAuto{x\in A\vdash F(x)\in B}{2}}

  \proofstepwide[2]{\pi_1(G(x))}{=}{\pi_1\restriction_R(G(x))}{%
      \FormulaRefAuto{C\subseteq A \dsep x\in C \vdash F\restriction_C(x)=F(x)}{3,4}}

  \proofstepwide[2]{}{=}{(\pi_1\restriction_R\circ G)(x)}{%
  ...
  \proofstepwide[1,2]{\pi_1(G(x))}{=}{x}{\rChain{6,9}}
\end{tabproofwide}

\FormulaThmDelta{%
  \pi_1\restriction_R\circ G = \Id_A
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[3]{G(x)\in R}{\FormulaRefAuto{x\in A\vdash F(x)\in B}{3}}
  \proofstepwidestar[3]{G(x)\in A\times B}{\FormulaRefAuto{A\subseteq B, x\in A\vdash x\in B}{2,4}}

  \proofstepwidestar[3]{G(x) = \bigl(\pi_1(G(x)),\pi_2(G(x))\bigr)}{%
    \FormulaRefAuto{z\in A\times B \vdash z=(\pi_1(z),\pi_2(z))}{5}}

  \proofstepwidestar[1,3]{\pi_1(G(x))=x}{%
    \FormulaRefAuto{\pi_1\restriction_R\circ G = \Id_A \dsep x\in A \vdash \pi_1(G(x))=x}{1,3}}

  \proofstepwidestar[1,3]{\bigl(\pi_1(G(x)),\pi_2(G(x))\bigr)=\bigl(x,\pi_2(G(x))\bigr)}{%
    \rIE{7,6}}

  \proofstepwidestar[1,3]{G(x)=\bigl(x,\pi_2(G(x))\bigr)}{%
    \FormulaRefAuto{a=b, b=c\vdash a=c}{7,8}}

  \proofstepwidestar[1,3]{\bigl(x,\pi_2(G(x))\bigr)\in R}{%
    \rIE{9,4}}
...
\FormulaThmDelta{%
  \pi_1\restriction_R\circ G = \Id_A
  \vdash \forall x\in A\,\bigl(x,\pi_2\restriction_R(G(x))\bigr)\in R
}{
  \DeltaRow{Mengen}{A\dsep B}
  \DeltaRow{Totale Relation}{\TotRel{R,A,B}}
  \DeltaRow{Funktion}{G\colon A\to R}
...
\begin{tabproof}
  \proofstep{1}{\pi_1\restriction_R\circ G = \Id_A}{\rA}
  \proofstep{}{R\subseteq A\times B}{\FormulaRefAuto{R \subseteq A \times B}}

  \proofstep{1}{\forall x\in A\,\bigl(x,\pi_2(G(x))\bigr)\in R}{%
    \FormulaRefAuto{\pi_1\restriction_R\circ G = \Id_A \vdash \forall x\in A\,\bigl(x,\pi_2(G(x))\bigr)\in R}{1}}

  \proofstep{}{x\in A}{\rA}
  \proofstep{}{G(x)\in R}{\FormulaRefAuto{x\in A\vdash F(x)\in B}{4}}

  \proofstep{}{ \pi_2\restriction_R(G(x))=\pi_2(G(x))}{%
    \FormulaRefAuto{C\subseteq A \dsep x\in C \vdash F\restriction_C(x)=F(x)}{2,5}}

  \proofstep{}{ \pi_2(G(x))=\pi_2\restriction_R(G(x))}{%
    \FormulaRefAuto{a=b\vdash b=a}{6}}

  \proofstep{}{(x,\pi_2(G(x)))\in R}{\rUE{3}}
  \proofstep{}{(x,\pi_2\restriction_R(G(x)))\in R}{\rIE{7,8}}

  \proofstep{1}{\forall x\in A\,\bigl(x,\pi_2\restriction_R(G(x))\bigr)\in R}{%
    \rUI{\rRI{4,9}}}
\end{tabproof}
...
\begin{tabproof}
  ...
  \proofstep{2}{\forall x\in A\,\bigl(x,\pi_2\restriction_R(G(x))\bigr)\in R}{%
    \FormulaRefAuto{\pi_1\restriction_R\circ G = \Id_A
      \vdash \forall x\in A\,\bigl(x,\pi_2\restriction_R(G(x))\bigr)\in R}{1}}

  \proofstep{3}{\pi_2\restriction_R\circ G\colon A\to B}{%
    \FormulaRefAuto{G\circ F\colon A\to C}{}}

  \proofstep{4}{x\in A}{\rA}

  \proofstep{4}{\bigl(x,\pi_2\restriction_R(G(x))\bigr)\in R}{\rUE{2}}

  \proofstep{4}{(\pi_2\restriction_R\circ G)(x)=\pi_2\restriction_R(G(x))}{%
    \FormulaRefAuto{x\in A \vdash (G\circ F)(x)=G(F(x))}{4}}

  \proofstep{4}{\pi_2\restriction_R(G(x))=(\pi_2\restriction_R\circ G)(x)}{%
    \FormulaRefAuto{a=b\vdash b=a}{6}}

  \proofstep{4}{\bigl(x,(\pi_2\restriction_R\circ G)(x)\bigr)\in R}{\rIE{7,5}}

  \proofstep{4}{\forall x\in A\,\bigl(x,(\pi_2\restriction_R\circ G)(x)\bigr)\in R}{%
    \rUI{\rRI{4,8}}}
...
\FormulaThmDelta{%
  \exists G\colon A\to R\,\bigl(\pi_1\restriction_R\circ G = \Id_A\bigr)
...
\begin{tabproof}
  \proofstep{1}{\exists G\colon A\to R\,(\pi_1\restriction_R\circ G = \Id_A)}{\rA}
  \proofstep{2}{G\colon A\to R\land \pi_1\restriction_R\circ G = \Id_A}{\rA}

  \proofstep{2}{\exists F\colon A\to B\,\forall x\in A\,((x,F(x))\in R)}{%
    \FormulaRefAuto{\pi_1\restriction_R\circ G = \Id_A
      \vdash \exists F\colon A\to B\,\forall x\in A\,(x,F(x))\in R}{2}}
...
\FormulaDefDelta[Mitgliedschaftsrelation]{%
  R_M \coloneqq \{\, (X,a)\in M\times \bigcup M \mid a\in X \,\}
}{
  \DeltaRow{Mengen}{M \dsep X \dsep a}
}

% ------------------------------------------------------------
% 1) Lemma: R_M ist Teilmenge des Produkts
% ------------------------------------------------------------
\FormulaThmDelta
{R_M \subseteq M\times \bigcup M}
{
  \DeltaRow{Mengen}{M \dsep X \dsep a}
  \DeltaRow{Relation}{R_M}
}
\begin{tabproof}
  \proofstep{}{R_M=\{\, (X,a)\in M\times \bigcup M \mid a\in X \,\}}{\rA}
  \proofstep{}{R_M \subseteq M\times \bigcup M}{%
    \FormulaRefAuto{\{\,x\in A \mid P(x)\,\}\subseteq A}{}}
\end{tabproof}

% ------------------------------------------------------------
% 2) Lemma: Aus (X,a)\in R_M folgt X\in M, a\in \bigcup M und a\in X
% ------------------------------------------------------------
\FormulaThmDelta
{(X,a)\in R_M \vdash X\in M \land a\in \bigcup M \land a\in X}
{
  \DeltaRow{Mengen}{M \dsep X \dsep a}
  \DeltaRow{Relation}{R_M}
}
\begin{tabproof}
  \proofstep{1}{(X,a)\in R_M}{\rA}
  \proofstep{1}{(X,a)\in M\times \bigcup M \land a\in X}{%
    \FormulaRefAuto{x \in \{x \in A \mid P(x)\} \eqvdash x \in A \land P(x)}{1}}
...
% ------------------------------------------------------------
% 3) Lemma: Aus X\in M und a\in X folgt (X,a)\in R_M
% ------------------------------------------------------------
\FormulaThmDelta
{X\in M \dsep a\in X \vdash (X,a)\in R_M}
{
  \DeltaRow{Mengen}{M \dsep X \dsep a}
  \DeltaRow{Relation}{R_M}
}
\begin{tabproof}
  ...
  \proofstep{1,3}{(X,a)\in M\times \bigcup M}{%
    \FormulaRefAuto{(a,b)\in A\times B \eqvdash a\in A \land b\in B}{1,3}}
  \proofstep{1,2,4}{(X,a)\in R_M}{%
    \FormulaRefAuto{x\in A \dsep P(x) \vdash x\in \{\,x\in A \mid P(x)\,\}}{4,2}}
\end{tabproof}

% ============================================================
% (1) Totalität von R_M (als eigenes Lemma)
% ============================================================

\FormulaThmDeltaK[Totalität von $R_M$]{%
  X\in M \vdash \exists a\,\bigl((X,a)\in R_M\bigr)
}{%
  \DisjFam(M)\vdash X\in M \vdash \exists a\,\bigl((X,a)\in R_M\bigr)
}{
  \DeltaRow{Mengen}{M \dsep X \dsep a}
  \DeltaRow{\makecell[l]{Paarweise disjunkte\\ nichtleere Familie}}{\DisjFam(M)}
  \DeltaRow{Relation}{R_M}
}
\begin{tabproof}
  \proofstep{1}{X\in M}{\rA}
  ...
  \proofstep{1}{\exists a\,(a\in X)}{%
    \FormulaRefAuto{A\neq\emptyset \eqvdash \exists x\,(x\in A)}{2}}

  \proofstep{4}{a\in X}{\rA}

  \proofstep{1,4}{(X,a)\in R_M}{%
    \FormulaRefAuto{X\in M \dsep a\in X \vdash (X,a)\in R_M}{1,4}}

  \proofstep{1,4}{\exists a\,\bigl((X,a)\in R_M\bigr)}{\rEI{5}}

  \proofstep{1}{\exists a\,\bigl((X,a)\in R_M\bigr)}{\rEE{3,4,6}}

  \proofstep{}{X\in M \vdash \exists a\,\bigl((X,a)\in R_M\bigr)}{\rRI{1,7}}
\end{tabproof}
xxx

% ============================================================
% (2) TotRel(R_M,M,⋃M) als "Definition-Style"-Theorem (ohne tabproof)
% ============================================================

\FormulaThmDeltaK[Totale Relation]{\TotRel{R_M,M,\bigcup M}}{%
  \DisjFam(M)\vdash \TotRel{R_M,M,\bigcup M}
}{
  \DeltaRow{Mengen}{M \dsep X \dsep a}
  \DeltaRow{\makecell[l]{Paarweise disjunkte\\ nichtleere Familie}}{\DisjFam(M)}
  \DeltaRow{Relation}{R_M}
  \DeltaRow{\textbf{Begründung}}{}
  \DeltaRow{Menge geordneter Paare}{R_M \subseteq M\times \bigcup M}
    [\FormulaRefAuto{R_M \subseteq M\times \bigcup M}]
  \DeltaRow{Totalität}{X\in M \vdash \exists a\,\bigl((X,a)\in R_M\bigr)}
    [\FormulaRefAuto{X\in M \vdash \exists a\,\bigl((X,a)\in R_M\bigr)}]
}{}

% ------------------------------------------------------------
% 5) Lemma: ACrel liefert eine Auswahlfunktion F auf M für R_M
% ------------------------------------------------------------
\FormulaThmDelta
{\ACrel \dsep \DisjFam(M)\vdash \exists F\colon M\to \bigcup M\,\forall X\in M\,(F(X)\in X)}
{
  \DeltaRow{Mengen}{M \dsep X \dsep a}
  \DeltaRow{Familie}{\DisjFam(M)}
}
\begin{tabproof}
  \proofstep{1}{\ACrel}{\rA}
  \proofstep{2}{\DisjFam(M)}{\rA}
  \proofstep{2}{\TotRel{R_M,M,\bigcup M}}{\FormulaRefAuto{\DisjFam(M)\vdash \TotRel{R_M,M,\bigcup M}}{2}}
  \proofstep{1,2}{\exists F\colon M\to \bigcup M\,\forall X\in M\,((X,F(X))\in R_M)}{%
    \rUE{\FormulaRefAuto{\ACrel}{}}}
  \proofstep{5}{F\colon M\to \bigcup M \land \forall X\in M\,((X,F(X))\in R_M)}{\rA}
  \proofstep{6}{X\in M}{\rA}
  \proofstep{5,6}{(X,F(X))\in R_M}{\rUE{5}}
  \proofstep{5,6}{X\in M \land F(X)\in \bigcup M \land F(X)\in X}{%
    \FormulaRefAuto{(X,a)\in R_M \vdash X\in M \land a\in \bigcup M \land a\in X}{7}}
  \proofstep{5,6}{F(X)\in X}{\rAEb{\rAEb{8}}}
  \proofstep{5}{\forall X\in M\,(F(X)\in X)}{\rUI{\rRI{6,9}}}
  \proofstep{1,2}{\exists F\colon M\to \bigcup M\,\forall X\in M\,(F(X)\in X)}{\rEI{\rAI{10,\rAEa{5}}}}
\end{tabproof}
xxx
% ------------------------------------------------------------
% 6) Lemma: Disjunktheit macht die Wahl eindeutig (über L = Bild(F))
% ------------------------------------------------------------
\FormulaThmDelta
{\DisjFam(M) \dsep \forall X\in M\,(F(X)\in X) \vdash
  \forall X\in M\,\exists! a\,\bigl(a\in X \land a\in \{F(Y)\mid Y\in M\}\bigr)}
{
...
\begin{tabproof}
  ...
  \proofstep{2}{\forall X\in M\,(F(X)\in X)}{\rA}

  \proofstep{3}{X\in M}{\rA}
  \proofstep{2,3}{F(X)\in X}{\rUE{2}}

  \proofstep{2,3}{F(X)\in \{F(Y)\mid Y\in M\}}{%
    \FormulaRefAuto{X\in M \vdash F(X)\in \{F(Y)\mid Y\in M\}}{3}}

  \proofstep{2,3}{\exists a\,(a\in X \land a\in \{F(Y)\mid Y\in M\})}{%
    \rEI{\rAI{4,5}}}

  \proofstep{7}{a\in X \land a\in \{F(Y)\mid Y\in M\}}{\rA}
  \proofstep{7}{a\in X}{\rAEa{7}}
  \proofstep{7}{a\in \{F(Y)\mid Y\in M\}}{\rAEb{7}}
  \proofstep{7}{\exists Y\in M\,(a=F(Y))}{%
    \FormulaRefAuto{a\in \{F(Y)\mid Y\in M\} \vdash \exists Y\in M\,(a=F(Y))}{10}}

  \proofstep{12}{Y\in M \land a=F(Y)}{\rA}
  \proofstep{12}{Y\in M}{\rAEa{12}}
  \proofstep{2,12}{F(Y)\in Y}{\rUE{2}}
  \proofstep{7,12}{a\in Y}{\rIE{12,14}}

  \proofstep{3,12}{X=Y}{%
    \FormulaRefAuto{%
      \DisjFam(M) \dsep X\in M \dsep Y\in M \dsep a\in X \dsep a\in Y \vdash X=Y}{1,3,13,9,15}}

  \proofstep{7,12}{a=F(X)}{%
    \FormulaRefAuto{X=Y \dsep a=F(Y) \vdash a=F(X)}{16,12}}

  \proofstep{7}{\forall b\,\bigl((b\in X \land b\in \{F(Y)\mid Y\in M\})\rightarrow b=a\bigr)}{%
    \rUI{\rRI{7,17}}}

  \proofstep{3}{\exists! a\,(a\in X \land a\in \{F(Y)\mid Y\in M\})}{%
    \FormulaRefAuto{\exists a\,P(a) \dsep \forall b\,(P(b)\rightarrow b=a) \vdash \exists! a\,P(a)}{6,18}}

  \proofstep{1,2}{\forall X\in M\,\exists! a\,(a\in X \land a\in \{F(Y)\mid Y\in M\})}{%
    \rUI{\rRI{3,19}}}
\end{tabproof}

% ------------------------------------------------------------
% 7) Abschluss: ACrel => ACfam (unter DisjFam(M))
% ------------------------------------------------------------
\FormulaThmDelta
{\ACrel \dsep \DisjFam(M) \vdash \ACfam}
{
  \DeltaRow{Mengen}{M \dsep X \dsep Y \dsep a}
  \DeltaRow{Familie}{\DisjFam(M)}
}
\begin{tabproof}
  ...
  \proofstep{3}{\exists F\colon M\to \bigcup M\,\forall X\in M\,(F(X)\in X)}{%
    \FormulaRefAuto{\ACrel \dsep \DisjFam(M)\vdash \exists F\colon M\to \bigcup M\,\forall X\in M\,(F(X)\in X)}{1,2}}

  \proofstep{4}{F\colon M\to \bigcup M \land \forall X\in M\,(F(X)\in X)}{\rA}

  \proofstep{2,4}{\forall X\in M\,\exists! a\,(a\in X \land a\in \{F(Y)\mid Y\in M\})}{%
    \FormulaRefAuto{\DisjFam(M) \dsep \forall X\in M\,(F(X)\in X) \vdash
      \forall X\in M\,\exists! a\,(a\in X \land a\in \{F(Y)\mid Y\in M\})}{2,4}}

  \proofstep{2,4}{\exists L\,\forall X\in M\,\exists! a\,(a\in X \land a\in L)}{%
    \rEI{\rAI{5,\rA}}}

  \proofstep{1,2}{\ACfam}{\rUE{6}}
\end{tabproof}

\section{Familienform nach Surjektionsform}

% ------------------------------------------------------------
% 0) Fasern und die zugehörige Familie
% ------------------------------------------------------------

\FormulaDefDelta[Faser]{%
  \Fib(G,x) \coloneqq \{\, b\in B \mid G(b)=x \,\}
}{
  \DeltaRow{Mengen}{A \dsep B \dsep x \dsep b}
  \DeltaRow{Funktion}{G\colon B\to A}
}

\FormulaDefDelta[Faserfamilie]{%
  M_G \coloneqq \{\, X \mid \exists x\in A\,(X=\Fib(G,x)) \,\}
}{
...
% ------------------------------------------------------------
% 1) Lemma: Jede Faser liegt in B
% ------------------------------------------------------------
\FormulaThmDelta
{b\in \Fib(G,x) \vdash b\in B}
{
  \DeltaRow{Mengen}{A\dsep B\dsep x\dsep b}
  \DeltaRow{Funktion}{G\colon B\to A}
}
\begin{tabproof}
  \proofstep{1}{b\in \Fib(G,x)}{\rA}
  \proofstep{1}{b\in B \land G(b)=x}{%
    \FormulaRefAuto{z\in \{\,t\in B \mid P(t)\,\} \vdash z\in B \land P(z)}{1}}
  \proofstep{1}{b\in B}{\rAEa{2}}
\end{tabproof}

% ------------------------------------------------------------
% 2) Lemma: Surjektivität liefert nichtleere Fasern
% ------------------------------------------------------------
\FormulaThmDelta
{G\colon B\sur A \dsep x\in A \vdash \Fib(G,x)\neq\varnothing}
{
  \DeltaRow{Mengen}{A\dsep B\dsep x}
  \DeltaRow{Surjektive Funktion}{G\colon B\sur A}
}
\begin{tabproof}
  ...
  \proofstep{2}{x\in A}{\rA}
  \proofstep{1,2}{\exists b\in B\,(G(b)=x)}{\FormulaRefAuto{Surjektive Funktion}{1,2}}
  \proofstep{4}{b\in B \land G(b)=x}{\rA}
  \proofstep{4}{b\in B}{\rAEa{4}}
  \proofstep{4}{G(b)=x}{\rAEb{4}}
  \proofstep{4}{b\in \Fib(G,x)}{%
    \FormulaRefAuto{z\in B \land P(z) \vdash z\in \{\,t\in B \mid P(t)\,\}}{5,6}}
  \proofstep{2}{\exists b\,(b\in \Fib(G,x))}{\rEE{3,4,7}}
  \proofstep{1,2}{\Fib(G,x)\neq\varnothing}{\FormulaRefAuto{\exists z\,(z\in X)\eqvdash X\neq\varnothing}{8}}
\end{tabproof}

% ------------------------------------------------------------
% 3) Lemma: Aus X∈M_G folgt X ist eine Faser
% ------------------------------------------------------------
\FormulaThmDelta
{X\in M_G \vdash \exists x\in A\,(X=\Fib(G,x))}
{
  \DeltaRow{Mengen}{A\dsep B\dsep X\dsep x}
  \DeltaRow{Funktion}{G\colon B\to A}
}
\begin{tabproof}
  \proofstep{1}{X\in M_G}{\rA}
  \proofstep{1}{\exists x\in A\,(X=\Fib(G,x))}{%
    \FormulaRefAuto{z\in \{\,t \mid Q(t)\,\}\vdash Q(z)}{1}}
\end{tabproof}

% ------------------------------------------------------------
% 4) Lemma: X∈M_G ⇒ X≠∅ (unter Surjektivität)
% ------------------------------------------------------------
\FormulaThmDelta
{G\colon B\sur A \dsep X\in M_G \vdash X\neq\varnothing}
{
  \DeltaRow{Mengen}{A\dsep B\dsep X}
  \DeltaRow{Surjektive Funktion}{G\colon B\sur A}
}
\begin{tabproof}
  \proofstep{1}{G\colon B\sur A}{\rA}
  \proofstep{2}{X\in M_G}{\rA}
  \proofstep{2}{\exists x\in A\,(X=\Fib(G,x))}{\FormulaRefAuto{X\in M_G \vdash \exists x\in A\,(X=\Fib(G,x))}{2}}
  \proofstep{4}{x\in A \land X=\Fib(G,x)}{\rA}
  \proofstep{4}{x\in A}{\rAEa{4}}
  \proofstep{4}{X=\Fib(G,x)}{\rAEb{4}}
  \proofstep{1,5}{\Fib(G,x)\neq\varnothing}{\FormulaRefAuto{G\colon B\sur A \dsep x\in A \vdash \Fib(G,x)\neq\varnothing}{1,5}}
  \proofstep{1,4}{X\neq\varnothing}{\rIE{6,7}}
  \proofstep{1,2}{X\neq\varnothing}{\rEE{3,4,8}}
\end{tabproof}

% ------------------------------------------------------------
% 5) Lemma: Verschiedene Fasern sind disjunkt
% ------------------------------------------------------------
\FormulaThmDelta
{G\colon B\to A \dsep x\in A \dsep y\in A \dsep x\neq y \vdash \Fib(G,x)\cap \Fib(G,y)=\varnothing}
{
  \DeltaRow{Mengen}{A\dsep B\dsep x\dsep y\dsep b}
  \DeltaRow{Funktion}{G\colon B\to A}
}
\begin{tabproof}
  ...
  \proofstep{5}{b\in \Fib(G,x)\cap \Fib(G,y)}{\rA}
  \proofstep{5}{b\in \Fib(G,x) \land b\in \Fib(G,y)}{%
    \FormulaRefAuto{b\in X\cap Y \eqvdash (b\in X \land b\in Y)}{5}}
  \proofstep{5}{b\in \Fib(G,x)}{\rAEa{6}}
  \proofstep{5}{b\in \Fib(G,y)}{\rAEb{6}}
  \proofstep{5}{G(b)=x}{%
    \rAEb{\FormulaRefAuto{z\in \{\,t\in B \mid P(t)\,\} \vdash z\in B \land P(z)}{7}}}
  \proofstep{5}{G(b)=y}{%
    \rAEb{\FormulaRefAuto{z\in \{\,t\in B \mid P(t)\,\} \vdash z\in B \land P(z)}{8}}}
  \proofstep{5}{x=y}{\FormulaRefAuto{a=b \dsep a=c \vdash b=c}{9,10}}
  \proofstep{5}{\bot}{\rBI{4,11}}
  \proofstep{}{b\in \Fib(G,x)\cap \Fib(G,y) \rightarrow \bot}{\rRI{5,12}}
  \proofstep{}{ \Fib(G,x)\cap \Fib(G,y)=\varnothing}{%
    \FormulaRefAuto{\forall b\,(b\in X \rightarrow \bot)\eqvdash X=\varnothing}{13}}
\end{tabproof}

% ------------------------------------------------------------
% 6) Lemma: X,Y ∈ M_G, X≠Y ⇒ X∩Y=∅
% ------------------------------------------------------------
\FormulaThmDelta
{G\colon B\to A \dsep X\in M_G \dsep Y\in M_G \dsep X\neq Y \vdash X\cap Y=\varnothing}
{
  \DeltaRow{Mengen}{A\dsep B\dsep X\dsep Y\dsep x\dsep y}
  \DeltaRow{Funktion}{G\colon B\to A}
}
\begin{tabproof}
  \proofstep{1}{G\colon B\to A}{\rA}
  \proofstep{2}{X\in M_G}{\rA}
  \proofstep{3}{Y\in M_G}{\rA}
  \proofstep{4}{X\neq Y}{\rA}

  \proofstep{2}{\exists x\in A\,(X=\Fib(G,x))}{\FormulaRefAuto{X\in M_G \vdash \exists x\in A\,(X=\Fib(G,x))}{2}}
  \proofstep{3}{\exists y\in A\,(Y=\Fib(G,y))}{\FormulaRefAuto{X\in M_G \vdash \exists x\in A\,(X=\Fib(G,x))}{3}}

  \proofstep{7}{x\in A \land X=\Fib(G,x)}{\rA}
  \proofstep{8}{y\in A \land Y=\Fib(G,y)}{\rA}
  \proofstep{7}{x\in A}{\rAEa{7}}
  \proofstep{7}{X=\Fib(G,x)}{\rAEb{7}}
  \proofstep{8}{y\in A}{\rAEa{8}}
  \proofstep{8}{Y=\Fib(G,y)}{\rAEb{8}}

  \proofstep{7,8}{x\neq y}{%
    \FormulaRefAuto{X=\Fib(G,x)\dsep Y=\Fib(G,y)\dsep X\neq Y \vdash x\neq y}{10,13,4}}

  \proofstep{1,9,12,14}{\Fib(G,x)\cap \Fib(G,y)=\varnothing}{%
    \FormulaRefAuto{G\colon B\to A \dsep x\in A \dsep y\in A \dsep x\neq y \vdash \Fib(G,x)\cap \Fib(G,y)=\varnothing}{1,9,12,15}}

  \proofstep{1,7,8}{X\cap Y=\varnothing}{\rIE{16,\rIE{10,13}}}
  \proofstep{1,2,3,4}{X\cap Y=\varnothing}{\rEE{6,8,\rEE{5,7,17}}}
\end{tabproof}

% ------------------------------------------------------------
% 7) Lemma: Aus Surjektivität folgt DisjFam(M_G)
% ------------------------------------------------------------
\FormulaThmDelta
{G\colon B\sur A \vdash \DisjFam(M_G)}
{
  \DeltaRow{Mengen}{A\dsep B\dsep X\dsep Y}
  \DeltaRow{Surjektive Funktion}{G\colon B\sur A}
}
\begin{tabproof}
  ...
  \proofstep{2}{X\in M_G}{\rA}
  \proofstep{1,2}{X\neq\varnothing}{\FormulaRefAuto{G\colon B\sur A \dsep X\in M_G \vdash X\neq\varnothing}{1,2}}
  \proofstep{}{X\in M_G \vdash X\neq\varnothing}{\rRI{2,3}}

  \proofstep{5}{X\in M_G}{\rA}
  \proofstep{6}{Y\in M_G}{\rA}
  \proofstep{7}{X\neq Y}{\rA}
  \proofstep{1,5,6,7}{X\cap Y=\varnothing}{%
    \FormulaRefAuto{G\colon B\to A \dsep X\in M_G \dsep Y\in M_G \dsep X\neq Y \vdash X\cap Y=\varnothing}{1,5,6,7}}
  \proofstep{}{X\in M_G \dsep Y\in M_G \dsep X\neq Y \vdash X\cap Y=\varnothing}{\rRI{5,\rRI{6,\rRI{7,8}}}}

  \proofstep{1}{\DisjFam(M_G)}{\rAI{4,9}}
\end{tabproof}

% ------------------------------------------------------------
% 8) Lemma: ACfam liefert eine Auswahlmenge L für M_G
% ------------------------------------------------------------
\FormulaThmDelta
{\ACfam \dsep G\colon B\sur A \vdash \exists L\,\forall X\in M_G\,\exists! a\,(a\in X \land a\in L)}
{
  \DeltaRow{Mengen}{A\dsep B\dsep X\dsep a}
  \DeltaRow{Surjektive Funktion}{G\colon B\sur A}
}
\begin{tabproof}
  \proofstep{1}{\ACfam}{\rA}
  \proofstep{2}{G\colon B\sur A}{\rA}
  \proofstep{2}{\DisjFam(M_G)}{\FormulaRefAuto{G\colon B\sur A \vdash \DisjFam(M_G)}{2}}
  \proofstep{1,2}{\exists L\,\forall X\in M_G\,\exists! a\,(a\in X \land a\in L)}{\rUE{1}}
\end{tabproof}

% ------------------------------------------------------------
% 9) Lemma: Aus der Auswahlmenge folgt eindeutige Wahl pro x∈A
% ------------------------------------------------------------
\FormulaThmDelta
{\forall X\in M_G\,\exists! a\,(a\in X \land a\in L) \dsep x\in A
 \vdash \exists! b\,\bigl(b\in \Fib(G,x)\land b\in L\bigr)}
{
  \DeltaRow{Mengen}{A\dsep B\dsep X\dsep x\dsep b}
  \DeltaRow{Funktion}{G\colon B\to A}
}
\begin{tabproof}
  \proofstep{1}{\forall X\in M_G\,\exists! a\,(a\in X \land a\in L)}{\rA}
  \proofstep{2}{x\in A}{\rA}

  \proofstep{2}{\exists X\in M_G\,(X=\Fib(G,x))}{%
    \FormulaRefAuto{x\in A \vdash \exists X\in M_G\,(X=\Fib(G,x))}{2}}

  \proofstep{4}{X\in M_G \land X=\Fib(G,x)}{\rA}
  \proofstep{4}{X\in M_G}{\rAEa{4}}
  \proofstep{1,5}{\exists! b\,(b\in X \land b\in L)}{\rUE{1}}
  \proofstep{4}{\exists! b\,(b\in \Fib(G,x) \land b\in L)}{\rIE{7, \rIE{6,\rAEb{4}}}}
  \proofstep{1,2}{\exists! b\,(b\in \Fib(G,x) \land b\in L)}{\rEE{3,4,8}}
\end{tabproof}
...
% ------------------------------------------------------------
\FormulaThmDelta
{\ACfam \dsep G\colon B\sur A \vdash \exists F\colon A\to B\,\bigl(G\circ F=\Id_A\bigr)}
{
  \DeltaRow{Mengen}{A\dsep B\dsep x}
  \DeltaRow{Surjektive Funktion}{G\colon B\sur A}
}
\begin{tabproof}
  ...
  \proofstep{3}{\exists L\,\forall X\in M_G\,\exists! a\,(a\in X \land a\in L)}{%
    \FormulaRefAuto{\ACfam \dsep G\colon B\sur A \vdash \exists L\,\forall X\in M_G\,\exists! a\,(a\in X \land a\in L)}{1,2}}

  \proofstep{4}{L \land \forall X\in M_G\,\exists! a\,(a\in X \land a\in L)}{\rA}
  \proofstep{5}{x\in A}{\rA}
  \proofstep{4,5}{\exists! b\,(b\in \Fib(G,x)\land b\in L)}{%
    \FormulaRefAuto{\forall X\in M_G\,\exists! a\,(a\in X \land a\in L) \dsep x\in A
     \vdash \exists! b\,(b\in \Fib(G,x)\land b\in L)}{4,5}}

  \proofstep{4}{\exists F\colon A\to B\,\forall x\in A\,\bigl(F(x)\in \Fib(G,x)\land F(x)\in L\bigr)}{%
    \FormulaRefAuto{\forall x\in A\,\exists! b\,P(x,b) \vdash \exists F\colon A\to B\,\forall x\in A\,P(x,F(x))}%
      {\rUI{\rRI{5,6}}}}

  \proofstep{7}{F\colon A\to B \land \forall x\in A\,\bigl(F(x)\in \Fib(G,x)\land F(x)\in L\bigr)}{\rA}
  \proofstep{8}{x\in A}{\rA}
  \proofstep{7,8}{F(x)\in \Fib(G,x)}{\rAEa{\rUE{7}}}
  \proofstep{7,8}{F(x)\in B \land G(F(x))=x}{%
    \FormulaRefAuto{z\in \{\,t\in B \mid P(t)\,\} \vdash z\in B \land P(z)}{9}}
  \proofstep{7,8}{G(F(x))=x}{\rAEb{10}}
  \proofstep{7}{\forall x\in A\,(G(F(x))=x)}{\rUI{\rRI{8,11}}}
  \proofstep{7}{G\circ F=\Id_A}{%
    \FormulaRefAuto{\forall x\in A\,(G(F(x))=x) \eqvdash G\circ F=\Id_A}{12}}

  \proofstep{1,2}{\exists F\colon A\to B\,\bigl(G\circ F=\Id_A\bigr)}{\rEE{3,4,\rEI{\rAI{13,\rAEa{7}}}}}
\end{tabproof}
