...
\[
\begin{array}{c}
\text{kein Tarski-Maximum in nichtleerem }B\subseteq\powerset(M)\\
\Downarrow\\
\forall X\in B\,\exists Y\in B\,\bigl(X\subset Y\bigr)\\
\Downarrow\\
\text{Frischdaten }b_0,S,D\text{ fuer eine wachsende Rekursion}\\
\Downarrow\\
...
Die langen Tabellen sind Detailnachweise; die eigentliche Argumentation
wird in den Bruecken und in der Hauptschlusskette wieder zusammengezogen.
Der Schlusssatz \(\Finite{M}\vdash \TarskiFinite(M)\) benutzt danach nur
noch die kurze Kette
\[
  \Finite{M}
  \Longrightarrow \neg\exists H\colon \mathbb{N}\inj M
...
\emph{Abhaengigkeitsmatrix.}
Die Beweise sind in drei Arten von Bausteinen getrennt: reine
Entfaltungen, konstruktive Auswahl- und Rekursionsschritte, und die
abschliessenden Bruecken zur Tarski-Endlichkeit.
\begin{center}
\small
\begin{tabularx}{\linewidth}{@{}p{0.18\linewidth}X X p{0.16\linewidth}@{}}
\textbf{Block} & \textbf{benutzt} & \textbf{liefert} & \textbf{Rolle}\\
\hline
Obermengenwahl &
echte Erweiterungen in \(B\) &
\(S\colon B\to B\) mit \(x\subset S(x)\) &
konstruktiv\\
Frischelementwahl &
\(S\) und \(X\subset S(X)\) &
\(D\colon B\to M\) mit frischen Werten &
konstruktiv\\
Abkuerzungen &
\(\mathsf{GrowRec}\), \(\mathsf{GrowFresh}\), \(\mathsf{FreshSeq}\) &
gezielt zitierbare Komponenten &
entfaltend\\
Technischer Kern &
\(\mathsf{GrowFresh}\)-Daten &
\(\exists H\colon\mathbb{N}\inj M\) &
konstruktiv\\
Tarski-Bruecke &
kein Tarski-Maximum &
echte Erweiterungen und damit eine Injektion &
kontrapositiv\\
Finale Bruecke &
\(\Finite{M}\) und Dedekind-Endlichkeit &
\(\TarskiFinite(M)\) &
abschliessend
\end{tabularx}
\end{center}
...
\paragraph{Entfaltungslemmata fuer Obermengenwahlen}

Dieser Detailblock fuehrt die erste Auswahl aus: Die Relation der echten
Obermengen wird entfaltet, ihre Totalitaet liefert per Auswahlfunktion
eine Schrittfunktion \(S\) mit \(x\subset S(x)\). Spaeter wird nur dieses
Ergebnis zitiert.
...
\paragraph{Konstruktives Auswahllemma fuer Obermengen}

Der folgende Satz ist das Ergebnis dieses Auswahlblocks.
...
\paragraph{Frischelementrelation}

Der zweite Auswahlblock wiederholt dasselbe Muster fuer die neu
hinzukommenden Elemente eines Schritts \(X\mapsto S(X)\).
...
\paragraph{Entfaltungslemmata der Frischelementrelation}

Die folgenden Hilfssaetze entpacken \(\FreshRel{S}\): Ein Paar
\((X,a)\) liegt genau dann in dieser Relation, wenn \(a\) in
\(S(X)\setminus X\) liegt.
...
\paragraph{Konstruktive Frischelementlemmata}

Aus \(X\subset S(X)\) folgt Totalitaet der Frischelementrelation; die
Auswahlfunktion liefert danach die konkrete Funktion \(D\).
...
\paragraph{Auswahl frischer Elemente}

Die letzte Auswahl in diesem Block macht aus der totalen
Frischelementrelation eine konkrete Funktion \(D\).
...
\paragraph{Abkuerzungen und Projektionen}

\FormulaDefDelta[Wachstumssituation]{%
  \begin{aligned}[t]
...
\FormulaDefDelta[Wachstumssituation mit Frischelementauswahl]{%
  \begin{aligned}[t]
...
Die Abkuerzungen entlasten die Anzeigeformeln. Einzelne Komponenten
werden bei Bedarf direkt ueber die Definitionssaetze entfaltet; eigene
Tabellen bleiben nur dort stehen, wo aus diesen Daten Rekursionsstufen,
Rekursionsgleichungen oder die Frischwertfolge gewonnen werden.
...
\begin{tabproof}
  ...
  \proofstep{1}{\GrowRec(B,b_0,S)}{%
    \FormulaRefAuto{Wachstumssituation mit Frischelementauswahl}{1}}
  \proofstep{1}{b_0\in B}{%
    \FormulaRefAuto{Wachstumssituation}{3}}
...
\FormulaThmDeltaR[Frischdaten typisieren die Frischwertfolge]{%
  ...
  \GrowFresh(M,B,b_0,S,D)\vdash
  D\circ \RecFun{b_0}{S}\colon \mathbb{N}\to M
}{%
...
\begin{tabproof}
  ...
  \proofstep{1}{\GrowRec(B,b_0,S)}{%
    \FormulaRefAuto{Wachstumssituation mit Frischelementauswahl}{1}}
  \proofstep{1}{b_0\in B}{%
    \FormulaRefAuto{Wachstumssituation}{2}}
  \proofstep{1}{S\colon B\to B}{%
    \FormulaRefAuto{Wachstumssituation}{2}}
...
\paragraph{Technischer Kern der Konstruktion}

Der technische Kern ist der Detailnachweis fuer einen einzigen
Mechanismus: Die Rekursion waechst monoton, die Frischwerte trennen
geordnete Indizes, und daraus entsteht eine Injektion von
\(\mathbb{N}\) nach \(M\).

\emph{Bausteintabelle.}
\begin{center}
\small
\begin{tabular}{@{}p{0.25\linewidth}p{0.32\linewidth}p{0.35\linewidth}@{}}
\textbf{Baustein} & \textbf{Eingang} & \textbf{Ausgabe}\\
\hline
Ein Schritt &
\(\GrowRec(B,b_0,S)\) &
jede Stufe liegt in ihrer Nachfolgerstufe\\
Monotonie &
Ein-Schritt-Wachstum und \(k<n\) &
die Stufe \(\Succ(k)\) liegt in der Stufe \(n\)\\
Frischwertlage &
\(\mathsf{GrowFresh}\)-Daten &
frisch in \(\Succ(u)\), aber nicht in \(u\)\\
Trennung &
Monotonie und Frischwertlage &
\(k<n\) liefert verschiedene Frischwerte\\
Injektion &
Ordnungstrennung auf \(\mathbb{N}\) &
eine Injektion \(\mathbb{N}\inj M\)
\end{tabular}
\end{center}

\emph{Invarianten des technischen Kerns.}
Die folgenden Tabellen beweisen nacheinander vier einfache Invarianten:
\[
\begin{array}{ll}
\text{(I)} &
  \RecFun{b_0}{S}(u)\in B,\\[2pt]
\text{(II)} &
  \RecFun{b_0}{S}(u)\subseteq \RecFun{b_0}{S}(\Succ(u)),\\[2pt]
\text{(III)} &
  D(\RecFun{b_0}{S}(u))\in \RecFun{b_0}{S}(\Succ(u))
  \text{, aber }
  D(\RecFun{b_0}{S}(u))\notin \RecFun{b_0}{S}(u),\\[2pt]
\text{(IV)} &
  k<n \text{ trennt die Frischwerte an den Stellen }k\text{ und }n.
\end{array}
\]
Damit ist die Injektivitaet nicht mehr der technische Teil des
Arguments, sondern nur noch eine Anwendung des allgemeinen
Ordnungstrennungssatzes auf \(\mathbb{N}\).

\emph{Wachsende Rekursion.}

Die ersten beiden Tabellen isolieren den Einzelschritt und heben ihn mit
dem allgemeinen Monotoniesatz auf \(k<n\): Spaetere Stufen enthalten die
frueher neu hinzugefuegten Elemente.
...
\begin{tabproof}
  ...
  \proofstep{2}{u\in\mathbb{N}}{\rA}
  \proofstep{1}{b_0\in B}{%
    \FormulaRefAuto{Wachstumssituation}{1}}
  \proofstep{1}{S\colon B\to B}{%
    \FormulaRefAuto{Wachstumssituation}{1}}
...
\begin{tabproof}
  ...
  \proofstep{1,5}{%
    ...
    \FormulaRefAuto{Ein-Schritt-Monotonie der wachsenden Rekursion}{1,5}}
  \proofstep{1}{%
  ...
  \proofstep{1,2,3,4}{%
    ...
    \FormulaRefAuto{Schrittweise Inklusionen induzieren Monotonie}{7,2,3,4}}
\end{tabproof}

\emph{Frischwertlage und Trennung.}

Die naechsten Tabellen transportieren nur die Lage der neuen Werte. Ein
Frischwert liegt im Zuwachs \(S(X)\setminus X\), wandert per Monotonie
weiter und trennt dort die Werte.
...
\begin{tabproof}
  ...
  \proofstep{1}{\forall X\in B\,\bigl(D(X)\in S(X)\setminus X\bigr)}{%
    \FormulaRefAuto{Wachstumssituation mit Frischelementauswahl}{1}}
  \proofstep{1,2}{\RecFun{b_0}{S}(u)\in B}{%
    \FormulaRefAuto{Frischdaten typisieren Rekursionsstufen}{1,2}}
  \proofstep{1,2}{%
    \begin{aligned}[t]
      D(\RecFun{b_0}{S}(u))&\in{}\\
  ...
  \proofstep{1,2}{%
    \begin{aligned}[t]
      ...
      &=S(\RecFun{b_0}{S}(u))
    \end{aligned}
  }{\FormulaRefAuto{Frischdaten liefern die Rekursionsgleichung}{1,2}}
  \proofstep{1,2}{%
    \begin{aligned}[t]
      S(\RecFun{b_0}{S}(u))
      &=\RecFun{b_0}{S}(\Succ(u))
...
\begin{tabproof}
  ...
  \proofstep{1,2}{D(\RecFun{b_0}{S}(k))\in \RecFun{b_0}{S}(\Succ(k))}{%
    \rAEa{\FormulaRefAuto{Frischwert liegt im Zuwachs}{1,2}}}
  \proofstep{1,2,3,4}{%
    \RecFun{b_0}{S}(\Succ(k))\subseteq \RecFun{b_0}{S}(n)
  }{%
    \FormulaRefAuto{Monotonie der wachsenden Rekursion}{5,2,3,4}}
  \proofstep{1,2,3,4}{D(\RecFun{b_0}{S}(k))\in \RecFun{b_0}{S}(n)}{%
    \FormulaRefAuto{A\subseteq B,\, x\in A \vdash x\in B}{7,6}}
\end{tabproof}

\emph{Uebergang zur Injektivitaet.}

Die Lageinformation wird nun direkt in Ungleichheit der Frischwerte
uebersetzt; anschliessend wird die punktweise Ordnungstrennung global
gebuendelt.
...
\begin{tabproof}
  ...
  \proofstep{4}{k < n}{\rA}
  \proofstep{1,2,3,4}{D(\RecFun{b_0}{S}(k))\in \RecFun{b_0}{S}(n)}{%
    \FormulaRefAuto{Frueher Frischwert liegt in spaeterer Stufe}{1,2,3,4}}
  \proofstep{1,3}{D(\RecFun{b_0}{S}(n))\notin \RecFun{b_0}{S}(n)}{%
    \rAEb{\FormulaRefAuto{Frischwert liegt im Zuwachs}{1,3}}}
  ...
  \proofstep{1,2,3,4}{%
    ...
    \FormulaRefAuto{Ungleichheit auf Kompositionswerte heben}{2,3,7}}
\end{tabproof}
...
\begin{tabproof}
  ...
  \proofstep{1,2,3,4}{%
    ...
    \FormulaRefAuto{\GrowFresh(M,B,b_0,S,D)\dsep k\in\mathbb{N}\dsep n\in\mathbb{N}\dsep k < n\vdash (D\circ \RecFun{b_0}{S})(k)\neq (D\circ \RecFun{b_0}{S})(n)}{1,2,3,4}}
  \proofstep{1,2,3}{%
    k<n\rightarrow
    (D\circ \RecFun{b_0}{S})(k)\neq
    (D\circ \RecFun{b_0}{S})(n)
...
Die globale Ordnungstrennung und die Typisierung der Frischwertfolge
liefern direkt die Injektivitaet.
...
\begin{tabproof}
  \proofstep{1}{\GrowFresh(M,B,b_0,S,D)}{\rA}
  \proofstep{1}{D\circ \RecFun{b_0}{S}\colon \mathbb{N}\to M}{%
    \FormulaRefAuto{\GrowFresh(M,B,b_0,S,D)\vdash D\circ \RecFun{b_0}{S}\colon \mathbb{N}\to M}{1}}
  \proofstep{1}{%
    \forall k,n\in\mathbb{N}\,\bigl(k<n\rightarrow
      (D\circ \RecFun{b_0}{S})(k)\neq
      (D\circ \RecFun{b_0}{S})(n)\bigr)
  }{%
    \FormulaRefAuto{\GrowFresh(M,B,b_0,S,D)\vdash \forall k,n\in\mathbb{N}\,\bigl(k<n\rightarrow (D\circ \RecFun{b_0}{S})(k)\neq (D\circ \RecFun{b_0}{S})(n)\bigr)}{1}}
  \proofstep{1}{D\circ \RecFun{b_0}{S}\colon \mathbb{N}\inj M}{%
...
\paragraph{Anwendung auf Tarski-Endlichkeit}

Die Anwendung wird direkt gebuendelt: Die Nichtleere liefert den
Start, die punktweisen echten Erweiterungen liefern den Schritt, und
die Frischelementauswahl liefert die Daten fuer den technischen Kern.
...
\emph{Lokale Abhaengigkeitstabelle.}
\begin{center}
\small
\begin{tabular}{@{}p{0.24\linewidth}p{0.66\linewidth}@{}}
\textbf{Etappe} & \textbf{Inhalt}\\
\hline
Wahlen &
Nichtleere und echte Obermengen liefern \(b_0,S\);
die Frischelementauswahl liefert \(D\).\\
Technischer Kern &
\(\GrowFresh(M,B,b_0,S,D)\) erzeugt eine injektive Folge in \(M\).\\
Sammelsatz &
Beides zusammen liefert \(\exists H\colon\mathbb{N}\inj M\).
\end{tabular}
\end{center}
...
\begin{tabproof}
  ...
  \proofstep{5}{D\circ \RecFun{b_0}{S}\colon \mathbb{N}\inj M}{%
    \FormulaRefAuto{\GrowFresh(M,B,b_0,S,D)\vdash D\circ \RecFun{b_0}{S}\colon \mathbb{N}\inj M}{5}}
  \proofstep{5}{\exists H\colon \mathbb{N}\inj M}{\rEI{6}}
  \proofstep{1,2,3}{\exists H\colon \mathbb{N}\inj M}{\rEE{4,5,7}}
\end{tabproof}

\paragraph{Tarski-Gegenbeispiele als Nichtmaximalitaet}

Der Gegenbeispielschritt wird direkt als Widerspruch gefuehrt: Haette
ein festes \(x\in B\) keine echte Erweiterung in \(B\), dann waere jedes
\(y\in B\) oberhalb von \(x\) bereits gleich \(x\), also waere \(x\)
ein Tarski-Maximum.
...
\emph{Hauptschlusskette.}
Alle technischen Tabellen sind hier zu ihren Ergebnissen
zusammengezogen; die Hauptargumentation besteht nur noch aus den
folgenden Implikationen.
\[
\begin{array}{c}
\text{kein Tarski-Maximum in einer nichtleeren }B\subseteq\powerset(M)\\
\Longrightarrow
\forall x\in B\,\exists y\in B\,\bigl(x\subset y\bigr)\\
\Longrightarrow
\exists b_0\in B\quad\text{und}\quad
\exists S\colon B\to B\,\forall x\in B\,\bigl(x\subset S(x)\bigr)\\
\Longrightarrow
\exists b_0\,\exists S\,\GrowRec(B,b_0,S)\\
\Longrightarrow
\exists b_0\,\exists S\,\exists D\,\GrowFresh(M,B,b_0,S,D)\\
\Longrightarrow
\exists H\colon\mathbb{N}\inj M.
