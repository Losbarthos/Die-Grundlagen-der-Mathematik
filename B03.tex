...
\subsection{Strikte Obermengenauswahl in Potenzmengenfamilien}

\subsubsection{Die strikte Obermengenrelation}
...
\begin{tabproof}
  ...
  \proofstep{1,2,3,4}{(x,y)\in \{(u,v)\in B\times B\mid u\subset v\}}{%
    \FormulaRefAuto{x \in A\dsep P(x)\vdash x \in \{x \in A \mid P(x)\}}{5,4}}
  \proofstep{1,2,3,4}{(x,y)\in \StrictSupRel{B}}{%
    \rIE{\FormulaRefAuto{B\subseteq \powerset(M)\vdash \StrictSupRel{B}\coloneqq \{(x,y)\in B\times B\mid x\subset y\}}{1},7}}
\end{tabproof}
...
\subsubsection{Die strikte Obermengenwahl als Abbildung}

\FormulaDefDelta[Typisierungsterm einer strikten Obermengenwahl]{%
  S\colon B\to B\dsep x\in B\vdash \tStrictSupSel{S}(x)\coloneqq S(x)%
}{%
  \DeltaRow{Mengen}{B\dsep x}
  \DeltaRow{Funktionen}{S}
  \DeltaRow{Funktionensymbol}{\tStrictSupSel{S}}
}

\FormulaThmDelta[Typisierungsaxiom für den Typisierungsterm der strikten Obermengenwahl]{%
  S\colon B\to B\dsep x\in B\vdash \tStrictSupSel{S}(x)\in B%
}{%
  \DeltaRow{Mengen}{B\dsep x}
  \DeltaRow{Funktionen}{S}
  \DeltaRow{Funktionensymbol}{\tStrictSupSel{S}}
...
\begin{tabproof}
  \proofstep{1}{S\colon B\to B}{\rA}
  \proofstep{2}{x\in B}{\rA}
  \proofstep{1,2}{\tStrictSupSel{S}(x)=S(x)}{%
    \FormulaRefAuto{S\colon B\to B\dsep x\in B\vdash \tStrictSupSel{S}(x)\coloneqq S(x)}{1,2}}
  \proofstep{1,2}{S(x)\in B}{%
    \FormulaRefAuto{F\colon A\to B\dsep x\in A\vdash F(x)\in B}{1,2}}
  \proofstep{1,2}{\tStrictSupSel{S}(x)\in B}{\rIE{3,4}}
\end{tabproof}
...
\begin{tabproof}
  ...
  \proofstep{5}{X\in B}{\rA}
  \proofstep{3,5}{X\subset S(X)}{\rRE{5,\rUE{3}}}
  \proofstep{3,5}{X\subseteq S(X)}{\FormulaRefAuto{A\subset B\vdash A\subseteq B}{6}}
  \proofstep{3,5}{X\neq S(X)}{\FormulaRefAuto{A\subset B \vdash A\neq B}{6}}
  \proofstep{3,5}{\exists a\in S(X)\setminus X}{%
    \FormulaRefAuto{A\subseteq B\dsep A\neq B\vdash \exists x\in B\setminus A}{7,8}}
  \proofstep{10}{a\in S(X)\setminus X}{\rA}
  \proofstep{2,5}{S(X)\in B}{%
    \FormulaRefAuto{F\colon A\to B\dsep x\in A\vdash F(x)\in B}{2,5}}
  \proofstep{1,11}{S(X)\in \powerset(M)}{%
    \FormulaRefAuto{A\subseteq B,\, x\in A \vdash x\in B}{1,11}}
  \proofstep{12}{S(X)\subseteq M}{%
    \FormulaRefAuto{x \in \powerset(A)\eqvdash x \subseteq A}{12}}
  \proofstep{10}{a\in S(X)}{\FormulaRefAuto{x\in A\setminus B\vdash x\in A}{10}}
  \proofstep{1,2,5,10}{a\in M}{%
    \FormulaRefAuto{A\subseteq B,\, x\in A \vdash x\in B}{13,14}}
  \proofstep{5,15}{(X,a)\in B\times M}{%
    \FormulaRefAuto{a\in A\dsep b\in B\vdash (a,b)\in A\times B}{5,15}}
  \proofstep{5,10,15}{(X,a)\in B\times M\land a\in S(X)\setminus X}{\rAI{16,10}}
  \proofstep{1,2,5,10}{(X,a)\in \FreshRel{S}}{%
    \rIE{\FormulaRefAuto{B\subseteq \powerset(M)\dsep S\colon B\to B \vdash \FreshRel{S}\coloneqq \{(X,a)\in B\times M\mid a\in S(X)\setminus X\}}{1,2},17}}
  \proofstep{1,2,3,5,10}{\exists a\,(X,a)\in \FreshRel{S}}{\rEI{18}}
  \proofstep{1,2,3,5}{\exists a\,(X,a)\in \FreshRel{S}}{\rEE{9,10,19}}
  \proofstep{1,2,3}{X\in B\rightarrow \exists a\,(X,a)\in \FreshRel{S}}{\rRI{5,20}}
  \proofstep{1,2,3}{\TotRel{\FreshRel{S},B,M}}{%
    \FormulaRefAuto{Totale Relation}{4,21}}
  \proofstep{1,2,3}{%
    \exists D\colon B\to M\,\forall X\in B\,\bigl((X,D(X))\in \FreshRel{S}\bigr)
  }{%
    \FormulaRefAuto{\exists F\colon A \to B\,\forall x\in A\,\bigl((x,F(x))\in R\bigr)}{22}}
  \proofstep{24}{D\colon B\to M\land \forall X\in B\,\bigl((X,D(X))\in \FreshRel{S}\bigr)}{\rA}
  \proofstep{24}{D\colon B\to M}{\rAEa{24}}
  \proofstep{24}{\forall X\in B\,\bigl((X,D(X))\in \FreshRel{S}\bigr)}{\rAEb{24}}
  \proofstep{27}{X\in B}{\rA}
  \proofstep{24,27}{(X,D(X))\in \FreshRel{S}}{\rRE{27,\rUE{26}}}
  \proofstep{1,2,24,27}{(X,D(X))\in \{(X,a)\in B\times M\mid a\in S(X)\setminus X\}}{%
    \rIE{\FormulaRefAuto{B\subseteq \powerset(M)\dsep S\colon B\to B \vdash \FreshRel{S}\coloneqq \{(X,a)\in B\times M\mid a\in S(X)\setminus X\}}{1,2},28}}
  \proofstep{1,2,24,27}{D(X)\in S(X)\setminus X}{%
    \FormulaRefAuto{x \in \{x \in A \mid P(x)\}\vdash P(x)}{29}}
  \proofstep{1,2,24}{\forall X\in B\,\bigl(D(X)\in S(X)\setminus X\bigr)}{\rUI{\rRI{27,30}}}
  \proofstep{1,2,24}{D\colon B\to M\land \forall X\in B\,\bigl(D(X)\in S(X)\setminus X\bigr)}{\rAI{25,31}}
  \proofstep{1,2,24}{\exists D\colon B\to M\,\forall X\in B\,\bigl(D(X)\in S(X)\setminus X\bigr)}{\rEI{32}}
  \proofstep{1,2,3}{\exists D\colon B\to M\,\forall X\in B\,\bigl(D(X)\in S(X)\setminus X\bigr)}{\rEE{23,24,33}}
\end{tabproof}
...
\begin{tabproof}
  ...
  \proofstep{1,2,3,4,5,6,9,10,11}{k=n}{%
    \begin{gathered}
      \text{Induktion auf }\mathbb{N}\text{ mit der Rekursionsgleichung;}\\
      \text{aus }X\subset S(X)\text{ und }D(X)\in S(X)\setminus X\\
      \text{liegt der neue Wert in der naechsten Stufe, aber nicht in der alten.}
    \end{gathered}}
