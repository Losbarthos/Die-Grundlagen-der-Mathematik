...
\begin{tabproof}
  ...
  \proofstep{3}{\exists X\in B\,\bigl(Z=M\setminus X\bigr)}{%
    \FormulaRefAuto{TarskiKomplementdualChar}{3}}
  ...
  \proofstep{16}{M\setminus X\in B^{\complement_M}}{%
    \FormulaRefAuto{TarskiKomplementdualChar}{\rEI{\rAI{16,\rII}}}}
...
\FormulaThmDeltaK[Rechenregeln für relative Komplemente]{%
  \begin{gathered}
...
\FormulaThmDeltaK[Komplementduale vertauscht Maxima und Minima]{%
  \begin{aligned}[t]
    &B\subseteq \powerset(M)\dsep
    ...
    &\exists X\in B\,\forall Y\in B\,
      \bigl(X\subseteq Y\rightarrow Y=X\bigr).
  \end{aligned}
}{TarskiKomplementdualMaxMin}{%
  \DeltaDecl{Mengen}{M\dsep B\dsep U\dsep V\dsep X\dsep Y}%
}
\begin{proof}
Sei zunächst \(U\) ein maximales Element von \(B^{\complement_M}\). Nach
\FormulaRefAuto{TarskiKomplementdualChar} gibt es ein \(X\in B\) mit
\(U=M\setminus X\). Ist \(Y\in B\) und \(Y\subseteq X\), so liegt
\(M\setminus Y\) wieder in \(B^{\complement_M}\), und nach
\FormulaRefAuto{TarskiRelCompRules} gilt
\(M\setminus X\subseteq M\setminus Y\). Die Maximalität von \(U\) liefert
\(M\setminus Y=U=M\setminus X\); wegen der Involutivität des relativen
Komplements folgt wieder mit \FormulaRefAuto{TarskiRelCompRules} die Gleichheit
\(Y=X\). Also ist \(X\) ein minimales Element von \(B\).

Die zweite Aussage ist derselbe Beweis mit umgekehrter Inklusionsrichtung:
Ein minimales Element der Komplementdualen entspricht einem maximalen Element
der Ausgangsfamilie.
\end{proof}
...
\begin{proof}
Dies ist die direkte Entfaltung der Definition
\FormulaRefAuto{minimal Tarski-endliche Menge}.
\end{proof}
...
\begin{proof}
Sei \(B\subseteq\powerset(M)\) nichtleer. Dann ist auch
\(B^{\complement_M}\) eine nichtleere Teilfamilie von \(\powerset(M)\) nach
\FormulaRefAuto{TarskiKomplementdualPowerFamily}. Aus \(\TarskiFinite(M)\)
erhält man dort nach
\FormulaRefAuto{\TarskiFinite(M)\dsep B\subseteq \powerset(M)\dsep B\neq\varnothing
\vdash \exists F\in B\,\forall G\in B\,\bigl(F\subseteq G\rightarrow G=F\bigr)}
ein maximales Element; das Dualitätslemma \FormulaRefAuto{TarskiKomplementdualMaxMin}
liefert daraus ein minimales Element von \(B\). Damit ist die Minimalitätsform
erfüllt.
\end{proof}
...
\begin{proof}
Sei \(B\subseteq\powerset(M)\) nichtleer. Nach
\FormulaRefAuto{TarskiKomplementdualPowerFamily} ist \(B^{\complement_M}\)
wieder eine nichtleere Teilfamilie von \(\powerset(M)\). Aus
\(\TarskiMinFinite(M)\) erhält man dort nach \FormulaRefAuto{TarskiMinFiniteElim}
ein minimales Element; nach \FormulaRefAuto{TarskiKomplementdualMaxMin}
entspricht dieses einem maximalen Element von \(B\). Das ist genau die
Maximalitätsform der Tarski-Endlichkeit.
\end{proof}
...
\begin{tabproofsplit}
  \proofpart{\(\vdash\)}
    ...
    \proofstep{1}{\TarskiMinFinite(M)}{%
      \FormulaRefAuto{TarskiFiniteToTarskiMinFinite}{1}}
  \closeproofpart

  \proofpart{\(\dashv\)}
    \proofstep{1}{\TarskiMinFinite(M)}{\rA}
    \proofstep{1}{\TarskiFinite(M)}{%
      \FormulaRefAuto{TarskiMinFiniteToTarskiFinite}{1}}
  \closeproofpart
\end{tabproofsplit}
...
\begin{tabproofsplit}
  \proofpart{\(\vdash\)}
    ...
    \proofstep{1}{\TarskiFinite(M)}{%
      \FormulaRefAuto{\Finite{M}\vdash \TarskiFinite(M)}{1}}
    \proofstep{1}{\TarskiMinFinite(M)}{%
      \FormulaRefAuto{TarskiFiniteToTarskiMinFinite}{2}}
  \closeproofpart
