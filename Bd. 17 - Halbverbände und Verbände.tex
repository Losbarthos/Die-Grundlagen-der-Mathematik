%============================================================
%  Bd. 17 - Halbverbände und Verbände
%============================================================
...
\title{Bd. 17 - Halbverbände und Verbände}
\author{Martin Kunze}
\date{}
\setcounter{file}{17}
...
Ausgehend von den Halbgruppen aus Band~15 und der allgemeinen Ordnungstheorie
...
\begin{tabproof}
  ...
  \proofstep{1}{%
    ...
    \Id_A(x\NeutralSemiOp{}y)
      =\Id_A(x)\NeutralSemiOp\Id_A(y)}{%
    \FormulaRefAuto{SemilatticeIdentityPreservesOperation}[thm]{1}}
  \proofstep{1}{%
    \SemilatticeHom{\Id_A;A,\NeutralSemiOp;A,\NeutralSemiOp}}{%
    \FormulaRefAuto{SemilatticeHomomorphism}[def]{1,1,2,3}}
\end{tabproof}
...
\paragraph{Beweis.}
Aus den beiden Homomorphismenvoraussetzungen folgen
\(f\colon A\to B\), \(g\colon B\to C\) und damit
\(g\circ f\colon A\to C\). Für \(x,y\in A\) gilt
\[
  \begin{aligned}
    (g\circ f)(x\NeutralSemiOp{}y)
      &=g\bigl(f(x\NeutralSemiOp{}y)\bigr)\\
      &=g\bigl(f(x)\mathbin{\diamond}f(y)\bigr)\\
      &=g(f(x))\mathbin{\bullet}g(f(y))\\
      &=(g\circ f)(x)\mathbin{\bullet}(g\circ f)(y).
  \end{aligned}
\]
Die mittleren Gleichheiten sind genau die Auswertungsaxiome der beiden
Homomorphismen. Zusammen mit den Quell- und Zielhalbverbänden liefert dies
nach \FormulaRefAuto{SemilatticeHomomorphism}[def] die Behauptung.
\hfill\(\square\)
...
\paragraph{Beweis.}
Nach dem Auswertungsaxiom ist
\[
  x\NeutralSemiOp{}y=\sup_{A,\leq}(\{x,y\}).
\]
Die Voraussetzungen \(x\leq u\) und \(y\leq u\) besagen, dass \(u\) eine
obere Schranke von \(\{x,y\}\) ist. Daher folgt aus der universellen
Eigenschaft des Paarsupremums
\(\sup_{A,\leq}(\{x,y\})\leq u\). Einsetzen der Auswertungsgleichung ergibt
die Behauptung.
\hfill\(\square\)
...
\paragraph{Beweis.}
Das Abgeschlossenheitsaxiom liefert
\(x\NeutralSemiOp{}y\in A\) und \(y\NeutralSemiOp{}z\in A\). Somit
existieren alle vier Paarsuprema, die in
\FormulaRefAuto{PairSupremumAssociative}[thm] auftreten. Nach diesem Satz
gilt
\[
  \begin{aligned}
    &\sup_{A,\leq}
  ...
  \end{aligned}
\]
Das Auswertungsaxiom identifiziert jedes dieser Paarsuprema mit der
entsprechenden Anwendung von \(\NeutralSemiOp\). Dadurch wird die linke
Seite zu \((x\NeutralSemiOp{}y)\NeutralSemiOp{}z\) und die rechte Seite zu
\(x\NeutralSemiOp(y\NeutralSemiOp{}z)\).
\hfill\(\square\)
...
\begin{tabproof}
  \proofstep{1}{\PartOrd{A,\leq}}{\rA}
  \proofstep{2}{\PairJoinOp{A,\NeutralSemiOp}}{\rA}
  \proofstep{2}{x,y\in A\vdash x\NeutralSemiOp{}y\in A}{%
  ...
  \proofstep{1,2}{x\in A\vdash x\NeutralSemiOp{}x=x}{%
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[1]{%
    x,y\in A\vdash f(x\MeetOp_A{}y)=f(x)\MeetOp_B f(y)}{%
    \FormulaRefAuto{LatticeHomMeetEvaluationAxiom}[ax]{1}}
  \proofstepwidestar[1]{\SemilatticeHom{f;A,\MeetOp_A;B,\MeetOp_B}}{%
    \FormulaRefAuto{SemilatticeHomomorphism}[def]{4,5,6,7}}
\end{tabproofwide}
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[1]{f\colon A\to B}{%
    \FormulaRefAuto{LatticeHomMapAxiom}[ax]{1}}
  \proofstepwidestar[1]{%
    x,y\in A\vdash f(x\JoinOp_A{}y)=f(x)\JoinOp_B f(y)}{%
    \FormulaRefAuto{LatticeHomJoinEvaluationAxiom}[ax]{1}}
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[1]{%
    ...
    \FormulaRefAuto{SemilatticeHomEvaluationAxiom}[ax]{4}}
  \proofstepwidestar[1]{%
    x,y\in A\vdash
    \Id_A(x\JoinOp_A{}y)=\Id_A(x)\JoinOp_A\Id_A(y)}{%
    \FormulaRefAuto{SemilatticeHomEvaluationAxiom}[ax]{5}}
  \proofstepwidestar[1]{%
    \LatticeHom{\Id_A;A,\MeetOp_A,\JoinOp_A;A,\MeetOp_A,\JoinOp_A}}{%
    \FormulaRefAuto{LatticeHomomorphism}[def]{1,1,6,7,8}}
\end{tabproofwide}
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[1,2]{%
    ...
    \FormulaRefAuto{SemilatticeHomEvaluationAxiom}[ax]{8}}
  \proofstepwidestar[1,2]{%
    \LatticeHom{g\circ f;A,\MeetOp_A,\JoinOp_A;C,\MeetOp_C,\JoinOp_C}}{%
    \FormulaRefAuto{LatticeHomomorphism}[def]{9,10,11,12,13}}
\end{tabproofwide}
...
\subsection{Homomorphismen und Isomorphismen der ausgezeichneten Strukturen}
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[2,4]{%
    \exists e\,\BoundedJoinSemi{A,\JoinOp,0,e}}{\rEI{8}}
  \proofstepwidestar[1,2]{%
    \exists e\,\BoundedJoinSemi{A,\JoinOp,0,e}}{\rEE{3,4,9}}
\end{tabproofwide}
...
\paragraph{Beweis.}
Setze \(L_{x,y}\coloneqq\LB_{A,\InducedLeq}(\{x,y\})\). Die von
\(\JoinOp\) induzierte Relation ist nach
\FormulaRefAuto{JoinSemilatticeInducesOrder}[thm] eine partielle Ordnung.
Ferner ist \(L_{x,y}\subseteq A\), also endlich, und wegen
\(0\in L_{x,y}\) nichtleer. Nach
\FormulaRefAuto{PairLowerBoundSetJoinClosed}[thm] ist \(L_{x,y}\) unter
\(\JoinOp\) abgeschlossen.

Der Satz über maximale Elemente endlicher geordneter Mengen liefert daher
ein maximales \(m\in L_{x,y}\). Wegen der Abschlussbedingung ist dieses
Element nach \FormulaRefAuto{JoinClosedMaximalIsGreatest}[thm] sogar das
größte Element von \(L_{x,y}\). Folglich gilt
\[
  \forall d\in L_{x,y}\;(d\InducedLeq m).
\]
Mit \FormulaRefAuto{InfimumCandidateEqualsInfimum}[thm] folgt
\(\PairInf{A}{\InducedLeq}{x}{y}{m}\); aus \(L_{x,y}\subseteq A\) folgt
außerdem \(m\in A\). Damit besitzt \(m\) sämtliche in der Behauptung
...
\paragraph{Beweis.}
Angenommen, es gälte \(u\not\InducedLeq y\). Aus
\(j\in\operatorname{Sep}_{A,\JoinOp}(x,y)\) folgt
\(j\InducedLeq x\). Mit \(u\InducedLeq j\) und der Transitivität der
induzierten Ordnung ergibt sich \(u\InducedLeq x\). Wegen \(u\in A\) wäre
damit \(u\in\operatorname{Sep}_{A,\JoinOp}(x,y)\). Die vorausgesetzte
Minimalität von \(j\) und \(u\InducedLeq j\) ergäben \(u=j\), im
Widerspruch zu \(u\neq j\). Also gilt \(u\InducedLeq y\).
\hfill\(\square\)
...
\paragraph{Beweis.}
Die Menge \(\operatorname{Sep}_{A,\JoinOp}(x,y)\) ist eine endliche
Teilmenge von \(A\). Sie ist nichtleer, denn aus
\(x\not\InducedLeq y\) folgt
\(x\in\operatorname{Sep}_{A,\JoinOp}(x,y)\). Die induzierte Relation ist
eine partielle Ordnung; daher besitzt die Separatormenge nach
\FormulaRefAuto{FinitePosetMinimalElement}[thm] ein minimales Element
\(j\).

Nach \FormulaRefAuto{MinimalSeparatorJoinIrreducible}[thm] ist dieses
\(j\) ein \(\JoinOp\)-irreduzibles Element. Aus
\(j\in\operatorname{Sep}_{A,\JoinOp}(x,y)\) folgen zugleich
\(j\InducedLeq x\) und \(j\not\InducedLeq y\). Somit erfüllt \(j\) die
behauptete Trennungseigenschaft.
\hfill\(\square\)
...
\paragraph{Beweis.}
Nach \FormulaRefAuto{JoinIrreducibleProperLowerSet}[def] bedeutet
\(Q\in D_L(P)\) insbesondere \(Q\in L\).
\hfill\(\square\)
