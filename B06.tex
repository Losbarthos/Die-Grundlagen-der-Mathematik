...
die zugrunde liegende Operation aus dem Kontext mitgeführt, aber in der
Strukturbezeichnung nicht eigens genannt. Das Zeichen \(\OmittedSemiOp\) in
den Axiomenschemata ist deshalb nur ein metasprachlicher Platzhalter und kein
Name dieser Operation. Muss die Operation bezeichnet werden, schreiben wir
beispielsweise \(\Semi{\NeutralSemiOp,A}\).
...
Axiomen steht \(\OmittedSemiOp\) für dieselbe, kontextuell festgelegte
Operation.
...
\FormulaAxiomDeltaK[Abgeschlossenheit einer Halbverbandsoperation]{%
  ...
  \DeltaRow{Funktionen}{\OmittedSemiOp\text{ (Platzhalter)}}
}
\par\smallskip
...
\FormulaDefDeltaK[Begriff des Halbverbandes]{%
  ...
  \DeltaRow{Mengen}{A\dsep x\dsep y\dsep z}
  \DeltaRow{Funktionen}{\OmittedSemiOp\text{ (kontextuell weggelassen)}}
  ...
  \DeltaPrem{Explizite Schreibweise}{\Semi{\NeutralSemiOp,A}}
}

Die folgenden Zugriffsaxiome verbinden das Strukturprädikat mit den vier
soeben registrierten Gesetzen. Wird die Operation später benannt, darf der
Platzhalter überall einheitlich durch ihren Namen ersetzt werden.
...
Band~03 eingeführten Funktionsauswertung. Beim Nachweis eines konkreten
...
Von nun an erhält die zuvor verborgene Operation beispielhaft den Namen
\(\NeutralSemiOp\). Die Abhängigkeit der induzierten Relation von
\(A\) und \(\Semi{\NeutralSemiOp,A}\) wird im Metablock festgehalten; in den
Formeln verwenden wir unmittelbar die natürliche Schreibweise \(\leq\).

\FormulaAxiomDeltaK[Auswertung der supremal induzierten Relation]{%
  \Semi{\NeutralSemiOp,A}\dsep x,y\in A\vdash
  \bigl(x\InducedLeq y\leftrightarrow x\NeutralSemiOp{}y=y\bigr)%
}{JoinOrderEvaluationAxiom}{%
  \DeltaRow{Mengen}{A\dsep x\dsep y}
  \DeltaRow{Funktionen}{\NeutralSemiOp}
  \DeltaPrem{Supremal induziert durch
    \(\Semi{\NeutralSemiOp,A}\)}{\InducedLeq}
}
...
\FormulaDefDeltaK[Supremal induzierte Relation und Schreibweise]{%
  ...
  \DeltaRow{Auswertung}{%
    \begin{aligned}[t]
      &\Semi{\NeutralSemiOp,A}\dsep x,y\in A\vdash{}\\[-2pt]
      &\bigl(x\InducedLeq y\leftrightarrow x\NeutralSemiOp{}y=y\bigr)
    \end{aligned}}
    [\FormulaRefAuto{JoinOrderEvaluationAxiom}[ax]]
  \DeltaRow{\textbf{Neues Symbol}}{}
  \DeltaPrem{\makecell[l]{Supremal induziert\\durch
    \(\Semi{\NeutralSemiOp,A}\)}}{\InducedLeq}
}

Die Formel \(x\InducedLeq y\) ist dabei die Infixschreibweise für die
Zugehörigkeit des Paares \((x,y)\) zur Relation \(\leq\). Für
\(x,y\in A\) gilt somit unmittelbar
\[
  x\InducedLeq y
...
\FormulaThmDeltaK[Erste Komponente der supremal induzierten Relation]{%
  \Semi{\NeutralSemiOp,A}\dsep x\InducedLeq y\vdash x\in A
}{JoinOrderFirstCarrier}{%
  \DeltaRow{Mengen}{A\dsep x\dsep y}
  \DeltaRow{Funktionen}{\NeutralSemiOp}
  \DeltaPrem{Supremal induzierte Relation aus
    \(A\) und \(\Semi{\NeutralSemiOp,A}\)}{\InducedLeq}
}
\begin{tabproof}
  \proofstep{1}{\Semi{\NeutralSemiOp,A}}{\rA}
...
\FormulaThmDeltaK[Zweite Komponente der supremal induzierten Relation]{%
  \Semi{\NeutralSemiOp,A}\dsep x\InducedLeq y\vdash y\in A
...
\FormulaThmDeltaK[Auswertung der supremal induzierten Relation]{%
  \Semi{\NeutralSemiOp,A}\dsep x\InducedLeq y
  \vdash x\NeutralSemiOp{}y=y
}{JoinOrderEvaluation}{%
  \DeltaRow{Mengen}{A\dsep x\dsep y}
  \DeltaRow{Funktionen}{\NeutralSemiOp}
  \DeltaPrem{Supremal induzierte Relation aus
...
\begin{tabproof}
  ...
  \proofstep{1,2}{x\InducedLeq y\leftrightarrow x\NeutralSemiOp{}y=y}{%
    \FormulaRefAuto{JoinOrderEvaluationAxiom}[ax]{1,3,4}}
  \proofstep{1,2}{x\NeutralSemiOp{}y=y}{%
...
\FormulaThmDeltaK[Reflexivität der supremal induzierten Relation]{%
  \Semi{\NeutralSemiOp,A}\dsep x\in A\vdash x\InducedLeq x
}{JoinOrderReflexive}{%
  \DeltaRow{Mengen}{A\dsep x}
  \DeltaRow{Funktionen}{\NeutralSemiOp}
  \DeltaPrem{Supremal induzierte Relation aus
...
\begin{tabproof}
  ...
  \proofstep{1,2}{x\NeutralSemiOp{}x=x}{%
    \FormulaRefAuto{SemilatticeIdempotenceAxiom}[ax]{1,2}}
  \proofstep{1,2}{x\InducedLeq x\leftrightarrow x\NeutralSemiOp{}x=x}{%
...
\begin{tabproof}
  \proofstep{1}{\Semi{\NeutralSemiOp,A}}{\rA}
  \proofstep{2}{x\InducedLeq y}{\rA}
  \proofstep{3}{y\InducedLeq z}{\rA}
  \proofstep{1,2}{x\in A}{\FormulaRefAuto{JoinOrderFirstCarrier}[thm]{1,2}}
  \proofstep{1,2}{y\in A}{\FormulaRefAuto{JoinOrderSecondCarrier}[thm]{1,2}}
  \proofstep{1,3}{z\in A}{\FormulaRefAuto{JoinOrderSecondCarrier}[thm]{1,3}}
  \proofstep{1,2}{x\NeutralSemiOp{}y=y}{\FormulaRefAuto{JoinOrderEvaluation}[thm]{1,2}}
  \proofstep{1,3}{y\NeutralSemiOp{}z=z}{\FormulaRefAuto{JoinOrderEvaluation}[thm]{1,3}}
  \proofstep{1,2,3}{%
  ...
  \proofstep{1,2,3}{\bigl(x\NeutralSemiOp{}y\bigr)\NeutralSemiOp{}z=z}{\rIE{7,8}}
  \proofstep{1,2,3}{%
    x\NeutralSemiOp\bigl(y\NeutralSemiOp{}z\bigr)=x\NeutralSemiOp{}z}{\rIE{8,\rII}}
  \proofstep{1,2,3}{z=\bigl(x\NeutralSemiOp{}y\bigr)\NeutralSemiOp{}z}{%
    \FormulaRefAuto{a=b\vdash b=a}{10}}
  \proofstep{1,2,3}{z=x\NeutralSemiOp\bigl(y\NeutralSemiOp{}z\bigr)}{%
    \FormulaRefAuto{a=b,\,b=c\vdash a=c}{12,9}}
  \proofstep{1,2,3}{x\NeutralSemiOp\bigl(y\NeutralSemiOp{}z\bigr)=z}{%
    \FormulaRefAuto{a=b\vdash b=a}{13}}
  \proofstep{1,2,3}{x\NeutralSemiOp{}z=z}{%
    \FormulaRefAuto{a=b,\,a=c\vdash b=c}{11,14}}
  \proofstep{1,2,3}{x\InducedLeq z\leftrightarrow x\NeutralSemiOp{}z=z}{%
    \FormulaRefAuto{JoinOrderEvaluationAxiom}[ax]{1,4,6}}
  \proofstep{1,2,3}{x\InducedLeq z}{%
...
\begin{tabproof}
  ...
  \proofstep{1,2}{y\in A}{\FormulaRefAuto{JoinOrderSecondCarrier}[thm]{1,2}}
  \proofstep{1,2}{x\NeutralSemiOp{}y=y}{\FormulaRefAuto{JoinOrderEvaluation}[thm]{1,2}}
  \proofstep{1,3}{y\NeutralSemiOp{}x=x}{\FormulaRefAuto{JoinOrderEvaluation}[thm]{1,3}}
  \proofstep{1,2}{x\NeutralSemiOp{}y=y\NeutralSemiOp{}x}{%
    \FormulaRefAuto{SemilatticeCommutativityAxiom}[ax]{1,4,5}}
  \proofstep{1,2,3}{x\NeutralSemiOp{}y=x}{\rIE{8,7}}
...
\FormulaThmDeltaK[Ein supremal gelesener Halbverband induziert eine partielle Ordnung]{%
  \Semi{\NeutralSemiOp,A}\vdash \PartOrd{\InducedLeq,A}
...
Band~03 definiert das Supremum bereits für jede Teilmenge \(T\subseteq A\).
...
\begin{tabproof}
  ...
  \proofstep{1,2,3}{x\NeutralSemiOp{}y\in A}{%
    \FormulaRefAuto{SemilatticeClosureAxiom}[ax]{1,2,3}}
  \proofstep{1,2}{x\NeutralSemiOp{}x=x}{%
    \FormulaRefAuto{SemilatticeIdempotenceAxiom}[ax]{1,2}}
  \proofstep{1,2,3}{%
    \bigl(x\NeutralSemiOp{}x\bigr)\NeutralSemiOp{}y=x\NeutralSemiOp{}y}{\rIE{5,\rII}}
  \proofstep{1,2,3}{%
    \bigl(x\NeutralSemiOp{}x\bigr)\NeutralSemiOp{}y
      =x\NeutralSemiOp\bigl(x\NeutralSemiOp{}y\bigr)}{%
    \FormulaRefAuto{SemilatticeAssociativityAxiom}[ax]{1,2,2,3}}
  \proofstep{1,2,3}{%
    x\NeutralSemiOp\bigl(x\NeutralSemiOp{}y\bigr)=x\NeutralSemiOp{}y}{%
    \FormulaRefAuto{a=b,\,a=c\vdash b=c}{7,6}}
  \proofstep{1,2,3}{x\InducedLeq x\NeutralSemiOp{}y
    \leftrightarrow
      x\NeutralSemiOp\bigl(x\NeutralSemiOp{}y\bigr)=x\NeutralSemiOp{}y}{%
...
\FormulaThmDeltaK[Der zweite Operand liegt unter dem algebraischen Supremum]{%
  \Semi{\NeutralSemiOp,A}\dsep x,y\in A
  \vdash y\InducedLeq x\NeutralSemiOp{}y
}{JoinUpperRight}{%
  \DeltaRow{Mengen}{A\dsep x\dsep y}
  \DeltaRow{Funktionen}{\NeutralSemiOp}
  \DeltaPrem{Supremal induzierte Relation aus
...
\begin{tabproof}
  ...
  \proofstep{2}{x\in A}{\rA}
  \proofstep{3}{y\in A}{\rA}
  \proofstep{1,2,3}{y\InducedLeq y\NeutralSemiOp{}x}{%
    \FormulaRefAuto{JoinUpperLeft}[thm]{1,3,2}}
  \proofstep{1,2,3}{y\NeutralSemiOp{}x=x\NeutralSemiOp{}y}{%
    \FormulaRefAuto{SemilatticeCommutativityAxiom}[ax]{1,3,2}}
  \proofstep{1,2,3}{y\InducedLeq x\NeutralSemiOp{}y}{\rIE{5,4}}
\end{tabproof}

\FormulaThmDeltaK[Das algebraische Supremum ist die kleinste obere Schranke]{%
  \begin{aligned}[t]
    &\Semi{\NeutralSemiOp,A}\dsep x,y,u\in A\dsep
      x\InducedLeq u\dsep y\InducedLeq u\vdash{}\\
    &x\NeutralSemiOp{}y\InducedLeq u
  \end{aligned}
}{JoinLeastUpper}{%
...
\begin{tabproof}
  ...
  \proofstep{5}{x\InducedLeq u}{\rA}
  \proofstep{6}{y\InducedLeq u}{\rA}
  \proofstep{1,5}{x\NeutralSemiOp{}u=u}{%
    \FormulaRefAuto{JoinOrderEvaluation}[thm]{1,5}}
  \proofstep{1,6}{y\NeutralSemiOp{}u=u}{%
    \FormulaRefAuto{JoinOrderEvaluation}[thm]{1,6}}
  \proofstep{1,2,3}{x\NeutralSemiOp{}y\in A}{%
    \FormulaRefAuto{SemilatticeClosureAxiom}[ax]{1,2,3}}
  \proofstep{1,2,3,4}{%
    \bigl(x\NeutralSemiOp{}y\bigr)\NeutralSemiOp{}u
      =x\NeutralSemiOp\bigl(y\NeutralSemiOp{}u\bigr)}{%
    \FormulaRefAuto{SemilatticeAssociativityAxiom}[ax]{1,2,3,4}}
  \proofstep{1,2,3,4,6}{%
    x\NeutralSemiOp\bigl(y\NeutralSemiOp{}u\bigr)=x\NeutralSemiOp{}u}{\rIE{8,\rII}}
  \proofstep{1,2,3,4,5,6}{%
    \bigl(x\NeutralSemiOp{}y\bigr)\NeutralSemiOp{}u=u}{%
    \FormulaRefAuto{a=b,\,b=c\vdash a=c}{10,\rIE{11,7}}}
  \proofstep{1,2,3,4}{x\NeutralSemiOp{}y\InducedLeq u
    \leftrightarrow
      \bigl(x\NeutralSemiOp{}y\bigr)\NeutralSemiOp{}u=u}{%
    \FormulaRefAuto{JoinOrderEvaluationAxiom}[ax]{1,9,4}}
  \proofstep{1,2,3,4,5,6}{x\NeutralSemiOp{}y\InducedLeq u}{%
    \FormulaRefAuto{P\leftrightarrow Q, Q\vdash P}{13,12}}
\end{tabproof}

\FormulaThmDeltaK[Die Halbverbandsoperation liefert das Paarsupremum]{%
  \Semi{\NeutralSemiOp,A}\dsep x,y\in A
  \vdash x\NeutralSemiOp{}y=\sup_{\InducedLeq,A}(\{x,y\})
}{JoinIsPairSupremum}{%
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[2,3]{\{x,y\}\subseteq A}{\rIE{5,4}}
  \proofstepwidestar[1,2,3]{x\NeutralSemiOp{}y\in A}{%
    \FormulaRefAuto{SemilatticeClosureAxiom}[ax]{1,2,3}}
  \proofstepwidestar[8]{z\in\{x,y\}}{\rA}
  \proofstepwidestar[8]{z=x\lor z=y}{%
    \FormulaRefAuto{x \in \{A,B\}\;\eqvdash\;(x = A \lor x = B)}{8}}
  \proofstepwidestar[10]{z=x}{\rA}
  \proofstepwidestar[1,2,3,10]{z\InducedLeq x\NeutralSemiOp{}y}{%
    \rIE{10,\FormulaRefAuto{JoinUpperLeft}[thm]{1,2,3}}}
  \proofstepwidestar[12]{z=y}{\rA}
  \proofstepwidestar[1,2,3,12]{z\InducedLeq x\NeutralSemiOp{}y}{%
    \rIE{12,\FormulaRefAuto{JoinUpperRight}[thm]{1,2,3}}}
  \proofstepwidestar[1,2,3,8]{z\InducedLeq x\NeutralSemiOp{}y}{%
    \rOE{9,10,11,12,13}}
  \proofstepwidestar[1,2,3]{%
    \forall z\in\{x,y\}\,(z\InducedLeq x\NeutralSemiOp{}y)}{%
    \rUI{\rRI{8,14}}}
  \proofstepwidestar[1,2,3]{%
    x\NeutralSemiOp{}y\in\UB_{\InducedLeq,A}(\{x,y\})}{%
    \FormulaRefAuto{P\leftrightarrow Q, Q\vdash P}{%
      \FormulaRefAuto{UpperBoundSetCharacterization}[thm]{6},\rAI{7,15}}}
  \proofstepwidestar[17]{u\in\UB_{\InducedLeq,A}(\{x,y\})}{\rA}
  \proofstepwidestar[2,3,17]{%
    u\in A\land\forall z\in\{x,y\}\,(z\InducedLeq u)}{%
    \FormulaRefAuto{P\leftrightarrow Q, P\vdash Q}{%
      \FormulaRefAuto{UpperBoundSetCharacterization}[thm]{6},17}}
  \proofstepwidestar[]{x\in\{x,y\}}{\FormulaRefAuto{a \in \{a,b\}}}
  \proofstepwidestar[]{y\in\{x,y\}}{\FormulaRefAuto{b \in \{a,b\}}}
  \proofstepwidestar[2,3,17]{x\InducedLeq u}{\rRE{19,\rUE{\rAEb{18}}}}
  \proofstepwidestar[2,3,17]{y\InducedLeq u}{\rRE{20,\rUE{\rAEb{18}}}}
  \proofstepwidestar[1,2,3,17]{x\NeutralSemiOp{}y\InducedLeq u}{%
    \FormulaRefAuto{JoinLeastUpper}[thm]{1,2,3,\rAEa{18},21,22}}
  \proofstepwidestar[1,2,3]{%
  ...
  \proofstepwidestar[1,2,3]{%
    x\NeutralSemiOp{}y=\sup_{\InducedLeq,A}(\{x,y\})}{%
    \FormulaRefAuto{SupremumCandidateEqualsSupremum}[thm]{25,6,16,24}}
\end{tabproofwide}
...
schreiben \(\PairJoinOp{R,A}\). In den Axiomenschemata bezeichnet
\(\OmittedSemiOp\) nur den Platz der kontextuell festgelegten Operation. Bei
Mehrdeutigkeit darf sie ausdrücklich notiert werden, etwa als
\(\PairJoinOp{\NeutralSemiOp,R,A}\).
...
\FormulaDefDeltaK[Durch Paarsuprema bestimmte Operation]{%
  ...
  \DeltaPrem{Kontextuelle Schreibweise}{%
    \PairJoinOp{R,A}}
  \DeltaPrem{Explizite Schreibweise}{%
    \PairJoinOp{\NeutralSemiOp,R,A}}
}
...
\begin{tabproof}
  ...
  \proofstep{3,4}{\{x,y\}\subseteq A\cup A}{%
    \FormulaRefAuto{a \in A,\, b \in B \vdash \{a,b\} \subseteq A \cup B}{3,4}}
  \proofstep{}{A\cup A=A}{\FormulaRefAuto{A\cup A=A}}
  \proofstep{3,4}{\{x,y\}\subseteq A}{\rIE{7,6}}
  \proofstep{}{x\in\{x,y\}}{\FormulaRefAuto{a \in \{a,b\}}}
  \proofstep{1,2,3,4}{(x,\sup_{R,A}(\{x,y\}))\in R}{%
    \FormulaRefAuto{SupremumDominatesMember}[thm]{1,8,5,9}}
  \proofstep{2,3,4}{x\OmittedSemiOp{}y=\sup_{R,A}(\{x,y\})}{%
    \FormulaRefAuto{PairJoinEvaluationAxiom}[ax]{2,3,4}}
  \proofstep{1,2,3,4}{(x,x\OmittedSemiOp{}y)\in R}{\rIE{11,10}}
\end{tabproof}
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[3,4]{\{x,y\}\subseteq A\cup A}{%
    \FormulaRefAuto{a \in A,\, b \in B \vdash \{a,b\} \subseteq A \cup B}{3,4}}
  \proofstepwidestar[]{A\cup A=A}{\FormulaRefAuto{A\cup A=A}}
  \proofstepwidestar[3,4]{\{x,y\}\subseteq A}{\rIE{9,8}}
  \proofstepwidestar[11]{z\in\{x,y\}}{\rA}
  \proofstepwidestar[11]{z=x\lor z=y}{%
    \FormulaRefAuto{x \in \{A,B\}\;\eqvdash\;(x = A \lor x = B)}{11}}
  \proofstepwidestar[13]{z=x}{\rA}
  \proofstepwidestar[6,13]{(z,u)\in R}{\rIE{13,6}}
  \proofstepwidestar[15]{z=y}{\rA}
  \proofstepwidestar[7,15]{(z,u)\in R}{\rIE{15,7}}
  \proofstepwidestar[6,7,11]{(z,u)\in R}{\rOE{12,13,14,15,16}}
  \proofstepwidestar[5,6,7]{%
    u\in A\land\forall z\in\{x,y\}\,((z,u)\in R)}{%
    \rAI{5,\rUI{\rRI{11,17}}}}
  \proofstepwidestar[3,4,5,6,7]{u\in\UB_{R,A}(\{x,y\})}{%
    \FormulaRefAuto{P\leftrightarrow Q, Q\vdash P}{%
      \FormulaRefAuto{UpperBoundSetCharacterization}[thm]{10},18}}
  ...
  \proofstepwidestar[1,2,3,4,5,6,7]{%
    (\sup_{R,A}(\{x,y\}),u)\in R}{%
    \FormulaRefAuto{SupremumLeastUpperBound}[thm]{1,10,20,19}}
  \proofstepwidestar[2,3,4]{x\OmittedSemiOp{}y=\sup_{R,A}(\{x,y\})}{%
    \FormulaRefAuto{PairJoinEvaluationAxiom}[ax]{2,3,4}}
  \proofstepwidestar[1,2,3,4,5,6,7]{(x\OmittedSemiOp{}y,u)\in R}{\rIE{22,21}}
\end{tabproofwide}
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[1,2,3,4,5]{%
    ...
    \makecell[l]{\FormulaRefAuto{PairJoinUpperLeft}[thm]\\
      \((1,2,3,7)\)}}
  \proofstepwidestar[1,2,3,4,5]{%
    (y\OmittedSemiOp{}z,x\OmittedSemiOp(y\OmittedSemiOp{}z))\in R}{%
    \makecell[l]{\FormulaRefAuto{PairJoinUpperRight}[thm]\\
      \((1,2,3,7)\)}}
  ...
  \proofstepwidestar[1,2,3,4,5]{%
    \begin{aligned}[t]
      &((x\OmittedSemiOp{}y)\OmittedSemiOp{}z,\\[-2pt]
      &\quad x\OmittedSemiOp(y\OmittedSemiOp{}z))\in R
    \end{aligned}}{%
    \makecell[l]{\FormulaRefAuto{PairJoinLeastUpper}[thm]\\
      \((1,2,6,5,8,14,13)\)}}
  ...
  \proofstepwidestar[1,2,3,4,5]{%
    (x,(x\OmittedSemiOp{}y)\OmittedSemiOp{}z)\in R}{%
    \FormulaRefAuto{EqRelTransitivity}[ax]{1,17,16}}
  \proofstepwidestar[1,2,3,4,5]{%
    (y,(x\OmittedSemiOp{}y)\OmittedSemiOp{}z)\in R}{%
    \FormulaRefAuto{EqRelTransitivity}[ax]{1,18,16}}
  \proofstepwidestar[1,2,3,4,5]{%
    (z,(x\OmittedSemiOp{}y)\OmittedSemiOp{}z)\in R}{%
  ...
  \proofstepwide[1,2,3,4,5]{%
    ...
    x\OmittedSemiOp(y\OmittedSemiOp{}z)}{%
    \makecell[l]{%
      \FormulaRefAuto{(x,y)\in R \dsep (y,x)\in R \vdash x=y}[ax]\\
      \((1,15,23)\)}}
\end{tabproofwide}
...
\(\Semi{\NeutralSemiOp,A}\) supremal induzierte Relation. Dann verwenden wir
für ihre in Band~03 definierte duale Relation unmittelbar die Schreibweise
\(\geq\). Es gilt also
\[
...
\FormulaThmDeltaK[Auswertung der infimalen Lesart]{%
  \Semi{\NeutralSemiOp,A}\dsep x,y\in A\vdash
  \bigl(x\InducedGeq y\leftrightarrow x\NeutralSemiOp{}y=x\bigr)
}{MeetOrderEvaluation}{%
  \DeltaRow{Mengen}{A\dsep x\dsep y}
  \DeltaRow{Funktionen}{\NeutralSemiOp}
  \DeltaPrem{Supremal induzierte Relation aus
...
\begin{tabproofwide}
  ...
  \proofstepwide[1,2,3]{}{\leftrightarrow}{y\NeutralSemiOp{}x=x}{%
    \FormulaRefAuto{JoinOrderEvaluationAxiom}[ax]{1,3,2}}
  \proofstepwide[1,2,3]{}{\leftrightarrow}{x\NeutralSemiOp{}y=x}{%
...
\FormulaThmDeltaK[Die infimale Lesart ist eine partielle Ordnung]{%
  \Semi{\NeutralSemiOp,A}\vdash \PartOrd{\InducedGeq,A}
...
Band~03 definiert das Infimum bereits für jede Teilmenge \(T\subseteq A\).
...
\FormulaThmDeltaK[Erste Untere-Schranken-Eigenschaft der infimalen Lesart]{%
  \Semi{\NeutralSemiOp,A}\dsep x,y\in A
  \vdash x\NeutralSemiOp{}y\InducedGeq x
}{MeetLowerLeft}{%
  \DeltaRow{Mengen}{A\dsep x\dsep y}
  \DeltaRow{Funktionen}{\NeutralSemiOp}
  \DeltaPrem{Supremal induzierte Relation}{\InducedLeq}
  \DeltaPrem{Duale, infimal gelesene Relation}{\InducedGeq}
}
\begin{tabproof}
  \proofstep{1}{\Semi{\NeutralSemiOp,A}}{\rA}
  ...
  \proofstep{1,2,3}{x\NeutralSemiOp{}y\InducedGeq x}{%
    \FormulaRefAuto{P\leftrightarrow Q, Q\vdash P}{%
...
\FormulaThmDeltaK[Zweite Untere-Schranken-Eigenschaft der infimalen Lesart]{%
  \Semi{\NeutralSemiOp,A}\dsep x,y\in A
  \vdash x\NeutralSemiOp{}y\InducedGeq y
}{MeetLowerRight}{%
  \DeltaRow{Mengen}{A\dsep x\dsep y}
  \DeltaRow{Funktionen}{\NeutralSemiOp}
  \DeltaPrem{Supremal induzierte Relation}{\InducedLeq}
...
\begin{tabproof}
  ...
  \proofstep{1,2,3}{x\NeutralSemiOp{}y\in A}{%
    \FormulaRefAuto{SemilatticeClosureAxiom}[ax]{1,2,3}}
  \proofstep{1,2,3}{y\InducedLeq x\NeutralSemiOp{}y}{%
    \FormulaRefAuto{JoinUpperRight}[thm]{1,2,3}}
  \proofstep{1,2,3}{x\NeutralSemiOp{}y\InducedGeq y}{%
    \FormulaRefAuto{P\leftrightarrow Q, Q\vdash P}{%
...
\FormulaThmDeltaK[Das algebraische Infimum ist die größte untere Schranke]{%
  \begin{aligned}[t]
    ...
    &u\InducedGeq x\NeutralSemiOp{}y
  \end{aligned}
}{MeetGreatestLower}{%
  \DeltaRow{Mengen}{A\dsep x\dsep y\dsep u}
  \DeltaRow{Funktionen}{\NeutralSemiOp}
...
\begin{tabproof}
  ...
  \proofstep{1,2,3,4,5,6}{u\InducedGeq x\NeutralSemiOp{}y}{%
    \FormulaRefAuto{P\leftrightarrow Q, Q\vdash P}{%
...
\FormulaThmDeltaK[Die Halbverbandsoperation liefert das Paarinfimum]{%
  \Semi{\NeutralSemiOp,A}\dsep x,y\in A
  \vdash x\NeutralSemiOp{}y=\inf_{\InducedGeq,A}(\{x,y\})
}{MeetIsPairInfimum}{%
  \DeltaRow{Mengen}{A\dsep x\dsep y\dsep u\dsep z}
  \DeltaRow{Funktionen}{\NeutralSemiOp}
  \DeltaPrem{Supremal induzierte Relation}{\InducedLeq}
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[2,3]{\{x,y\}\subseteq A\cup A}{%
    \FormulaRefAuto{a \in A,\, b \in B \vdash \{a,b\} \subseteq A \cup B}{2,3}}
  \proofstepwidestar[]{A\cup A=A}{\FormulaRefAuto{A\cup A=A}}
  \proofstepwidestar[2,3]{\{x,y\}\subseteq A}{\rIE{6,5}}
  \proofstepwidestar[1,2,3]{x\NeutralSemiOp{}y\in A}{%
    \FormulaRefAuto{SemilatticeClosureAxiom}[ax]{1,2,3}}
  ...
  \proofstepwidestar[1,2,3,11]{x\NeutralSemiOp{}y\InducedGeq z}{%
    \rIE{11,\FormulaRefAuto{MeetLowerLeft}[thm]{1,2,3}}}
  \proofstepwidestar[13]{z=y}{\rA}
  \proofstepwidestar[1,2,3,13]{x\NeutralSemiOp{}y\InducedGeq z}{%
    \rIE{13,\FormulaRefAuto{MeetLowerRight}[thm]{1,2,3}}}
  \proofstepwidestar[1,2,3,9]{x\NeutralSemiOp{}y\InducedGeq z}{%
    \rOE{10,11,12,13,14}}
  \proofstepwidestar[1,2,3]{%
    \forall z\in\{x,y\}\,(x\NeutralSemiOp{}y\InducedGeq z)}{%
    \rUI{\rRI{9,15}}}
  \proofstepwidestar[1,2,3]{%
    x\NeutralSemiOp{}y\in\LB_{\InducedGeq,A}(\{x,y\})}{%
    \FormulaRefAuto{P\leftrightarrow Q, Q\vdash P}{%
      \FormulaRefAuto{LowerBoundSetCharacterization}[thm]{7},\rAI{8,16}}}
  \proofstepwidestar[18]{u\in\LB_{\InducedGeq,A}(\{x,y\})}{\rA}
  \proofstepwidestar[2,3,18]{%
    u\in A\land\forall z\in\{x,y\}\,(u\InducedGeq z)}{%
    \FormulaRefAuto{P\leftrightarrow Q, P\vdash Q}{%
      \FormulaRefAuto{LowerBoundSetCharacterization}[thm]{7},18}}
  \proofstepwidestar[]{x\in\{x,y\}}{\FormulaRefAuto{a \in \{a,b\}}}
  \proofstepwidestar[]{y\in\{x,y\}}{\FormulaRefAuto{b \in \{a,b\}}}
  \proofstepwidestar[2,3,18]{u\InducedGeq x}{\rRE{20,\rUE{\rAEb{19}}}}
  \proofstepwidestar[2,3,18]{u\InducedGeq y}{\rRE{21,\rUE{\rAEb{19}}}}
  \proofstepwidestar[1,2,3,18]{u\InducedGeq x\NeutralSemiOp{}y}{%
    \FormulaRefAuto{MeetGreatestLower}[thm]{1,2,3,\rAEa{19},22,23}}
  \proofstepwidestar[1,2,3]{%
    \forall u\in\LB_{\InducedGeq,A}(\{x,y\})\,
      (u\InducedGeq x\NeutralSemiOp{}y)}{%
    \rUI{\rRI{18,24}}}
  \proofstepwidestar[1,2,3]{%
    x\NeutralSemiOp{}y=\inf_{\InducedGeq,A}(\{x,y\})}{%
    \FormulaRefAuto{InfimumCandidateEqualsInfimum}[thm]{4,7,17,25}}
\end{tabproofwide}
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[1,3,11]{j\InducedLeq y}{\rIE{12,10}}
  ...
  \proofstepwidestar[6,16,17]{%
    u\InducedLeq u\JoinOp{}v}{%
    \FormulaRefAuto{JoinUpperLeft}[thm]{6,16,17}}
  \proofstepwidestar[6,16,17,18]{u\InducedLeq j}{%
    \rIE{18,19}}
  \proofstepwidestar[6,16,17]{%
    v\InducedLeq u\JoinOp{}v}{%
    \FormulaRefAuto{JoinUpperRight}[thm]{6,16,17}}
  \proofstepwidestar[6,16,17,18]{v\InducedLeq j}{%
    \rIE{18,21}}
