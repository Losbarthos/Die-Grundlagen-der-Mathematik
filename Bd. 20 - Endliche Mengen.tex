...
\begin{tabproofwide}
  ...
  \proofstepwide[1,2]{\operatorname{Enum}_a(\Succ(n))}{=}
    {a[\NatSeg{\Succ(n)}]}{%
...
\(\NatSeg{\Succ(n)}=\NatSeg{n}\cup\{n\}\) gest\"utzte Rekursionsform der
...
\begin{tabproofsplitwide}
  ...
  \proofpartwideind[Induktionsschritt]{%
    ...
    \proofstepwidestar[1,2]{C\subseteq \NatSeg{\Succ(n)}\rightarrow \Finite{C}}{\rRI{3,4}}
    \proofstepwidestar[1,2]{%
      \forall C\,\bigl(C\subseteq \NatSeg{\Succ(n)} \rightarrow \Finite{C}\bigr)}{%
      \rUI{5}}
      \proofstepwidestarbreak[1,2]{%
      \forall C\,\bigl(C\subseteq \NatSeg{n+1} \rightarrow \Finite{C}\bigr)}{%
      \rIE{\FormulaRefAuto{PeanoAddOneSucc}[thm]{1},6}}
  \closeproofpartwideind
...
\FormulaDefDelta[Potenzmengenadjunktion]{%
  ...
  \begin{aligned}[t]
    &\PowAdj{n}\colon
      \powerset(\NatSeg{n})\to\powerset(\NatSeg{\Succ(n)}),\\[-2pt]
...
\begin{tabproof}
  ...
  \proofstep{1}{\NatSeg{n}\subseteq\NatSeg{\Succ(n)}}{%
    \FormulaRefAuto{n\in\mathbb{N}\vdash
      \NatSeg{n}\subseteq\NatSeg{\Succ(n)}}[thm]{1}}
  ...
  \proofstep{1,2}{X\cup\{n\}\in\powerset(\NatSeg{\Succ(n)})}{%
    \FormulaRefAuto{x\in\powerset(A)\eqvdash x\subseteq A}{8}}
  \proofstep{1}{%
    \forall X\in\powerset(\NatSeg{n})\,
      \bigl(X\cup\{n\}\in\powerset(\NatSeg{\Succ(n)})\bigr)}{%
    \rUI{\rRI{2,9}}}
\end{tabproof}
...
\FormulaThmDelta[Typisierung der Potenzmengenadjunktion]{%
  n\in\mathbb{N}\vdash
  \PowAdj{n}\colon
    \powerset(\NatSeg{n})\to\powerset(\NatSeg{\Succ(n)})
}{%
  \DeltaRow{Mengen}{n}
  \DeltaRow{Funktionensymbol}{\PowAdj{n}}
}
...
\begin{tabproofsplitwide}
  ...
  \proofpartwideindR[Induktionsschritt]{%
    ...
    \proofstepwidestar[1,2]{\Finite{\powerset(\NatSeg{\Succ(n)})}}{\rIE{3,7}}
      \proofstepwidestarbreak[1,2]{\Finite{\powerset(\NatSeg{n+1})}}{%
      \rIE{\FormulaRefAuto{PeanoAddOneSucc}[thm]{1},8}}
  \closeproofpartwideindR
...
\begin{tabproofsplitwide}
  \proofpartwideindR[Induktionsanfang]{%
    ...
    \proofstepwidestar[]{2^0=\Succ(0)}{%
      \FormulaRefAuto{PeanoPowerZero}[thm]{%
        \FormulaRefAuto{PeanoTwoInN}[thm]}}
    \proofstepwidestar[]{\Succ(0)=1}{%
      \FormulaRefAuto{a=b\vdash b=a}{\FormulaRefAuto{PeanoOne}[def]}}
    \proofstepwidestar[]{\NatSeg{2^0}=\{0\}}{%
      \rIE{3,4,\FormulaRefAuto{PeanoNatSegOneMembership}[thm]}}
  ...
  \proofpartwideindR[Induktionsschritt]{%
    ...
    \proofstepwidestar[1]{2^{\Succ(n)}=2^n\cdot2}{%
      \FormulaRefAuto{PeanoPowerSucc}[thm]{%
    ...
    \proofstepwidestar[1,2]{%
      ...
      c_{n+1}\colon
      \powerset(\NatSeg{n+1})\bij\NatSeg{2^{n+1}}}{%
      \rIE{\FormulaRefAuto{PeanoAddOneSucc}[thm]{1},5}}
  \closeproofpartwideindR
...
\]
Die Gleichung (1) und
\FormulaRefAuto{PeanoNatSegSucc}[thm] ergeben somit
\[
  \NatSeg{\Succ(p)}\EqCard\powerset(A).                     \tag{2}
\]
...
\(\Succ(p)\neq0\) nach \FormulaRefAuto{PeanoSuccNonzero}[ax]. Wegen (4)
ist daher auch \(2^n\neq0\). Zusammen mit \(2^n\in\mathbb N\) folgt
\(2^n\in\mathbb N\setminus\{0\}\), also liegt die rechte Seite von (4)
im Definitionsbereich der Vorgängerfunktion aus
\FormulaRefAuto{PeanoPredFunction}[thm]. Wenden wir nun \(\Pred\) auf (4)
an und benutzen \FormulaRefAuto{PeanoPredSucc}[thm], so erhalten wir
\[
  p=\Pred(2^n).
\]
Nach \FormulaRefAuto{PeanoAddOneSucc}[thm] ist (4) gleichbedeutend mit
\(p+1=2^n\).  Ferner liefert
\FormulaRefAuto{PeanoMinusOneNotation}[def] aus
\(2^n\in\mathbb N\) und \(2^n\neq0\) die Schreibweise
\(\Pred(2^n)=2^n-1\).  Wegen \(p=\tCard(\PowNE{A})\) sind dies die beiden
...
\begin{tabproofsplitwide}
  ...
  \proofpartwideindR[Induktionsschritt]{%
    ...
    \proofstepwidestar[1,2]{%
      ...
      \proofstepwidestarbreak[1,2]{%
      \forall M\,\bigl((n+1\EqCard M)\rightarrow\Finite{M}\bigr)}{%
      \rIE{\FormulaRefAuto{PeanoAddOneSucc}[thm]{1},15}}
  \closeproofpartwideindR
...
\[
  T_F(n,p,q)\coloneqq
  \begin{cases}
    \bigl(\Succ(n),v(p,q),q\bigr),&F(n)\leq u(p,q),\\
    \bigl(\Succ(n),p,u(p,q)\bigr),&u(p,q)<F(n).
  \end{cases}
\]
...
\FormulaRefAuto{DedekindRecursionTheorem}[thm] liefert, angewandt auf den
...
\[
  \begin{aligned}
    &\ell(n)<r(n),\\[-2pt]
    &\ell(n)\leq\ell(\Succ(n))
      <r(\Succ(n))\leq r(n),\\[-2pt]
    &F(n)\notin
      [\ell(\Succ(n)),r(\Succ(n))]_{\mathbb R}.
  \end{aligned}
\]
...
Nachfolgerschritt gilt aus \(m\leq\Succ(k)\) entweder \(m\leq k\) oder
\(m=\Succ(k)\); Transitivität zusammen mit den lokalen Ungleichungen liefert
...
Fixiere \(n\in\mathbb N\). Weil \(\ell(\Succ(n))\in L\), liefert
\FormulaRefAuto{SupremumDominatesMember}[thm]
\(\ell(\Succ(n))\leq x\). Außerdem ist \(r(\Succ(n))\) eine obere Schranke
von \(L\): Für jedes
\(m\in\mathbb N\) gilt entweder \(m\leq\Succ(n)\), und dann
\(\ell(m)\leq\ell(\Succ(n))<r(\Succ(n))\), oder
\(\Succ(n)\leq m\), und dann
\(\ell(m)<r(m)\leq r(\Succ(n))\). Der Satz
\FormulaRefAuto{SupremumLeastUpperBound}[thm] ergibt somit
\(x\leq r(\Succ(n))\). Also
\[
  x\in[\ell(\Succ(n)),r(\Succ(n))]_{\mathbb R}
