%============================================================
%  Bd. 40 - Gruppen und Ringe
%============================================================

\documentclass[main.tex]{subfiles}

\ifSubfilesClassLoaded{
  \BandSetupStandalone{B40}
}{
}

\title{Bd. 40 - Gruppen und Ringe}
\author{Martin Kunze}
\date{}
\setcounter{file}{40}

\hypersetup{
  pdftitle={Bd. 40 - Gruppen und Ringe},
...
Die Bände~28--37 entwickeln Halbgruppen und ihre Spezialisierungen,
Band~38 Monoide und Band~39 Halbringe.
...
Den Gruppenteil beschließt ein Rekonstruktionssatz: Ist die große
Potenzhalbgruppe einer Gruppe zur großen Potenzhalbgruppe einer beliebigen
Halbgruppe isomorph, so sind bereits die Grundhalbgruppen isomorph.
...
Wie in den Bänden~23--25 gehört jede Operation obligatorisch zur
...
\begin{proof}
Wähle \(a\in A\) und definiere die Linkstranslation
\(L_a(x)=a\star x\) sowie die Rechtstranslation
\(R_a(x)=x\star a\).  Links- beziehungsweise Rechtskürzbarkeit machen
\(L_a\) und \(R_a\) injektiv.  Eine injektive Selbstabbildung einer endlichen
Menge ist surjektiv: Ihr Bild hat dieselbe endliche Kardinalität wie der
Träger und kann deshalb keine echte Teilmenge sein.  Somit sind beide
Translationen surjektiv.

Wähle \(e,f\in A\) mit
\[
  e\star a=a,\qquad a\star f=a.
\]
Ist \(x\in A\), so existiert wegen der Surjektivität von \(L_a\) ein
\(y\in A\) mit \(x=a\star y\).  Daher gilt
\[
  e\star x=e\star(a\star y)=(e\star a)\star y=a\star y=x.
\]
Entsprechend liefert die Surjektivität von \(R_a\) eine Darstellung
\(x=y\star a\), und damit
\[
  x\star f=(y\star a)\star f=y\star(a\star f)=y\star a=x.
\]
Also ist \(e\) linksneutral und \(f\) rechtsneutral.  Aus
\(e=e\star f=f\) folgt \(e=f\); dieses Element ist beidseitig neutral.

Sei nun \(b\in A\).  Die Surjektivität von \(L_b\) und \(R_b\) liefert
\(c,d\in A\) mit
\[
  b\star c=e,\qquad d\star b=e.
\]
Dann ist
\[
  d=d\star e=d\star(b\star c)=(d\star b)\star c=e\star c=c.
\]
Somit besitzt jedes \(b\in A\) ein beidseitiges Inverses.  Zusammen mit der
Halbgruppenstruktur erfüllt \((A,\star,e)\) daher
...
Für ein Monoid \((A,\star,e_A)\) bezeichne
\[
  A^\times
  \coloneqq
  \bigl\{u\in A\bigm|
    \MonoidUnit{A,\star,e_A}{u}\bigr\}
\]
die Menge seiner Einheiten.  Sie ist nichtleer, denn \(e_A\) ist nach
\FormulaRefAuto{MonoidIdentityIsUnit}[thm] eine Einheit; insbesondere ist
\(e_A\in A^\times\).  Sind \(u,v\) mit beidseitigen Inversen \(u',v'\)
Einheiten, so zeigen die Monoidgesetze
\[
\begin{aligned}
  (u\star v)\star(v'\star u')
    &=u\star(v\star v')\star u'=e_A,\\
  (v'\star u')\star(u\star v)
    &=v'\star(u'\star u)\star v=e_A,
\end{aligned}
\]
dass \(v'\star u'\) ein beidseitiges Inverses von \(u\star v\) ist.
Außerdem ist \(u'\) selbst eine Einheit, weil \(u\) ein beidseitiges
Inverses von \(u'\) ist.  Assoziativität und neutrales Element werden vom
Monoid geerbt.  Damit erfüllt die Einschränkung von \(\star\) auf
\(A^\times\) alle Bedingungen von \FormulaRefAuto{GroupDef}[def].  Diese
Gruppe heißt die \emph{Einheitengruppe} des Monoids.

Allgemein heißt ein Element \(z\) eines Monoids \(M\)
\emph{einheitenstabil}, wenn
\[
  uz=z=zu
  \qquad\text{für jede Einheit }u\in M.
\]
Auf das Potenzmonoid von \(A\) angewandt bedeutet dies für
\(X\in\PowNE A\), dass
\[
  U\star_{\mathcal P}X=X=X\star_{\mathcal P}U
\]
für jede Einheit \(U\) des Potenzmonoids gilt.  Nach
\FormulaRefAuto{PowerMonoidUnitCharacterization}[thm] sind diese Einheiten
genau die Einermengen \(\{u\}\) mit \(u\in A^\times\).  Folglich gilt
\[
  X\text{ ist einheitenstabil}
  \quad\Longleftrightarrow\quad
  A^\times\star_{\mathcal P}X=X
  =X\star_{\mathcal P}A^\times.                 \tag{1}
\]
Für die nichttriviale Rückrichtung sei etwa
\(A^\times\star_{\mathcal P}X=X\), und sei \(u\in A^\times\) mit einem
beidseitigen Inversen \(v\).  Dann sind
\(\{u\}\star_{\mathcal P}X\) und
\(\{v\}\star_{\mathcal P}X\) in \(X\) enthalten.  Daher gilt auch
\[
  X=\{e_A\}\star_{\mathcal P}X
    =\{u\}\star_{\mathcal P}
       \bigl(\{v\}\star_{\mathcal P}X\bigr)
    \subseteq\{u\}\star_{\mathcal P}X,
\]
während die umgekehrte Inklusion bereits feststeht.  Also fixiert jede
linke Einheit \(X\); rechts gilt dieselbe Rechnung.  Die Hinrichtung von
(1) folgt durch Vereinigung über alle Einheiten.

Außerdem gilt für jede nichtleere Teilmenge \(Y\subseteq A\)
\[
  Y\subseteq A^\times
  \quad\Longleftrightarrow\quad
  Y\star_{\mathcal P}A^\times=A^\times
  \quad\Longleftrightarrow\quad
  A^\times\star_{\mathcal P}Y=A^\times.        \tag{2}
\]
Ist etwa \(Y\subseteq A^\times\), sei \(y\in Y\), und wähle ein
beidseitiges Inverses \(y'\in A^\times\).  Der Produktabschluss der
Einheitengruppe liefert
\(y\star A^\times\subseteq A^\times\).  Ist umgekehrt
\(z\in A^\times\), so ist \(y'\star z\in A^\times\), und
\[
  z=y\star(y'\star z)\in y\star A^\times.
\]
Also gilt \(y\star A^\times=A^\times\), und wegen der Nichtleerheit von
\(Y\) folgt
\(Y\star_{\mathcal P}A^\times=A^\times\).  Umgekehrt folgt aus
\(Y\star_{\mathcal P}A^\times=A^\times\) wegen \(e_A\in A^\times\), dass
jedes \(y=y\star e_A\) zu \(A^\times\) gehört.  Die linke Variante folgt
durch dieselbe Rechnung mit vertauschten Faktoren.
...
\begin{proof}
Hier bezeichnet \(F(A^\times)\) den Wert von \(F\) an der nichtleeren
Teilmenge \(A^\times\), während
\(F[\PowNE{A^\times}]\) das direkte Bild der ganzen angegebenen
Teilmengenfamilie bezeichnet.

Wir benötigen zunächst den Transport der Einheitenstabilität.  Sei dazu
\(h\colon M\to N\) ein Isomorphismus zwischen multiplikativ geschriebenen
Monoiden mit neutralen Elementen \(e_M,e_N\).  Sei \(y\in N\).  Wegen der
Surjektivität von \(h\) gibt es ein \(x\in M\) mit \(h(x)=y\), und es gilt
\[
  h(e_M)y=h(e_Mx)=h(x)=y,
  \qquad
  yh(e_M)=h(xe_M)=h(x)=y.
\]
Also ist \(h(e_M)=e_N\).  Besitzt \(u\in M\) ein beidseitiges Inverses
\(v\), so zeigen
\[
  h(u)h(v)=h(e_M)=e_N=h(v)h(u),
\]
dass \(h(u)\) eine Einheit ist.  Dasselbe Argument für \(h^{-1}\) zeigt,
dass \(h\) die Einheiten bijektiv auf die Einheiten abbildet.  Ist nun
\(z\in M\) einheitenstabil und \(w\in N\) eine Einheit, so gibt es eine
Einheit \(u\in M\) mit \(h(u)=w\).  Daher
\[
  wh(z)=h(uz)=h(z)=h(zu)=h(z)w.
\]
Somit bleiben einheitenstabile Elemente unter einem Monoidisomorphismus und
seiner Umkehrabbildung erhalten.  Wir wenden dies auf die Potenzmonoide und
den Isomorphismus \(F\) an.

Die Teilmenge \(A^\times\) ist nach (1)
einheitenstabil.  Somit ist \(F(A^\times)\) einheitenstabil in
\(\PowNE B\), und erneut nach (1) gilt
\[
  F(A^\times)\diamond_{\mathcal P}B^\times=F(A^\times).       \tag{3}
\]
Setze \(X=F^{-1}(B^\times)\).  Da auch \(B^\times\) einheitenstabil ist,
ist \(X\) einheitenstabil; folglich
\(A^\times\star_{\mathcal P}X=X\).  Mit (3) erhält man
\[
\begin{aligned}
  B^\times
    &=F(X)
      =F(A^\times\star_{\mathcal P}X)\\
    &=F(A^\times)\diamond_{\mathcal P}F(X)
      =F(A^\times)\diamond_{\mathcal P}B^\times
      =F(A^\times).
\end{aligned}                                                   \tag{4}
\]

Sei nun \(Y\in\PowNE{A^\times}\).  Nach (2) gilt
\(Y\star_{\mathcal P}A^\times=A^\times\).  Anwenden von \(F\) und (4)
liefert
\[
  F(Y)\diamond_{\mathcal P}B^\times=B^\times.
\]
Nach (2) folgt \(F(Y)\subseteq B^\times\).  Also ist
\(F[\PowNE{A^\times}]\subseteq\PowNE{B^\times}\).  Dasselbe Argument für
\(F^{-1}\) liefert die umgekehrte Inklusion.  Damit bildet die
Einschränkung von \(F\) die große Potenzhalbgruppe von \(A^\times\)
isomorph auf diejenige von \(B^\times\) ab.
\end{proof}
...
\begin{proof}
Nach \FormulaRefAuto{GroupBaseMonoidAxiom}[ax] ist
\((A,\star,e_A)\) ein Monoid, und nach
\FormulaRefAuto{PowerSemigroupMonoid}[thm] ist \(\{e_A\}\) das neutrale
Element seiner Potenzhalbgruppe.  Setze \(E=F(\{e_A\})\).  Für jedes
\(Y\in\PowNE B\) gibt es wegen der Surjektivität von \(F\) ein
\(X\in\PowNE A\) mit \(F(X)=Y\).  Die Operationserhaltung ergibt
\[
\begin{aligned}
  E\diamond_{\mathcal P}Y
    &=F(\{e_A\})\diamond_{\mathcal P}F(X)
      =F(\{e_A\}\star_{\mathcal P}X)=F(X)=Y,\\
  Y\diamond_{\mathcal P}E
    &=F(X)\diamond_{\mathcal P}F(\{e_A\})
      =F(X\star_{\mathcal P}\{e_A\})=F(X)=Y.
\end{aligned}
\]
Somit ist \(E\) das neutrale Element der Potenzhalbgruppe von \(B\).
Nach
\FormulaRefAuto{PowerSemigroupIdentityCharacterization}[thm] existiert ein
\(e_B\in B\) mit \(E=\{e_B\}\), und \((B,\diamond,e_B)\) ist ein Monoid.

Jedes Element der Gruppe \(A\) ist eine Einheit, also \(A^\times=A\).
Der vorangehende Rekonstruktionssatz liefert daher
\[
  F[\PowNE A]=\PowNE{B^\times}.
\]
Andererseits liefert die Surjektivität von \(F\) die Gleichheit
\(F[\PowNE A]=\PowNE B\).  Folglich gilt
\(\PowNE B=\PowNE{B^\times}\).  Für jedes \(b\in B\) gehört damit die
Einermenge \(\{b\}\) zu \(\PowNE{B^\times}\), also
\(b\in B^\times\).  Daher ist \(B=B^\times\): Jedes Element von \(B\)
besitzt ein beidseitiges Inverses, und
\FormulaRefAuto{GroupDef}[def] macht \((B,\diamond,e_B)\) zu einer Gruppe.

Nach \FormulaRefAuto{PowerMonoidUnitCharacterization}[thm] sind nun die
Einheiten der beiden Potenzmonoide genau
\(\operatorname{Sing}(A)\) beziehungsweise \(\operatorname{Sing}(B)\).
Da \(F\) Einheiten bijektiv auf Einheiten abbildet, gilt
\[
  F[\operatorname{Sing}(A)]=\operatorname{Sing}(B).
\]
Der Rekonstruktionssatz für die Einermengenschicht
\FormulaRefAuto{PowerSemigroupSingletonLayerReconstruction}[thm]
liefert einen Halbgruppenisomorphismus \(f\colon A\to B\).  Zwischen den
beiden soeben nachgewiesenen Gruppen ist er nach
\FormulaRefAuto{GroupHomDef}[def] ein Gruppenhomomorphismus; seine
Bijektivität und \FormulaRefAuto{GroupIsoDef}[def] liefern den behaupteten
...
Der Gruppenstarrheitssatz liefert zunächst die Isomorphie der Grundgruppen.
Für spätere Faserargumente halten wir zusätzlich fest, welche Einermenge
ein gegebener Potenzisomorphismus wohin schickt.
...
\begin{tabproofwide}
  ...
  \proofstepwidestarbreakkeep[5]{g\in G}{\rA}
  \proofstepwidestarbreakkeep[1,5]{\MonoidUnit{G,\star,e}{g}}{\FormulaRefAuto{GroupElementIsMonoidUnit}[thm]{1,5}}
  \proofstepwidestarbreakkeep[1]{\forall g\in G\,\MonoidUnit{G,\star,e}{g}}{\rUI{\rRI{5,6}}}
  ...
  \end{aligned}}{\textnormal{Existenzregeln und Konjunktionsregeln, }7}
  \proofstepwidestarbreakkeep[1]{X\in\operatorname{Sing}(G)\leftrightarrow\exists g\in G\,(X=\{g\})}{%
    \FormulaRefAuto{SemigroupSingletonCopyDef}[def]{\FormulaRefAuto{GroupBaseSemigroup}[thm]{1}}}
  \proofstepwidestarbreakkeep[1,2]{\begin{aligned}[t]
    &\MonoidUnit{\PowNE G,\star_{\mathcal P},\{e\}}{X}\\[-2pt]
    &\quad\leftrightarrow X\in\operatorname{Sing}(G)
  \end{aligned}}{%
    \textnormal{Äquivalenzersetzung, }4,8,9}
\end{tabproofwide}
\endgroup

\paragraph{Beweisidee.}
Einheiten sind rein durch die Multiplikation und die Identität bestimmt.
In der Potenzhalbgruppe einer Gruppe sind sie genau die Einermengen.
Der Einheitentransport aus Band~38 erhält daher die gesamte Einermengenschicht.
...
\begin{tabproofwide}
  ...
  \proofstepwidestarbreakkeep[1]{\MonoidAlg{\PowNE G}{\star_{\mathcal P}}{\{e\}}}{%
    \FormulaRefAuto{PowerSemigroupMonoid}[thm]{\FormulaRefAuto{GroupBaseMonoidAxiom}[ax]{1}}}
  \proofstepwidestarbreakkeep[2]{\MonoidAlg{\PowNE H}{\diamond_{\mathcal P}}{\{u\}}}{%
    \FormulaRefAuto{PowerSemigroupMonoid}[thm]{\FormulaRefAuto{GroupBaseMonoidAxiom}[ax]{2}}}
  \proofstepwidestarbreakkeep[3]{F\colon\PowNE G\bij\PowNE H}{\FormulaRefAuto{SemigroupIsoBijectiveAxiom}[ax]{3}}
  \proofstepwidestarbreakkeep[7]{X\in\PowNE G}{\rA}
  \proofstepwidestarbreakkeep[3,7]{F(X)\in\PowNE H}{\FormulaRefAuto{F\colon A\bij B\dsep x\in A\vdash F(x)\in B}[thm]{6,7}}
  \proofstepwidestarbreakkeep[1,2,3,7]{\begin{aligned}[t]
    &\MonoidUnit{\PowNE G,\star_{\mathcal P},\{e\}}{X}\\[-2pt]
    &\quad\leftrightarrow\MonoidUnit{\PowNE H,\diamond_{\mathcal P},\{u\}}{F(X)}
  \end{aligned}}{%
    \FormulaRefAuto{SemigroupIsoBetweenMonoidsUnits}[thm]{4,5,3,7}}
  \proofstepwidestarbreakkeep[1,2,3,7]{X\in\operatorname{Sing}(G)\leftrightarrow F(X)\in\operatorname{Sing}(H)}{%
    \FormulaRefAuto{PowerGroupUnitsAreSingletons}[thm]{1,7};\
    \FormulaRefAuto{PowerGroupUnitsAreSingletons}[thm]{2,8};\ \textnormal{Äquivalenzersetzung, }9}
  \proofstepwidestarbreakkeep[1,2,3]{\forall X\in\PowNE G\,\left(
    ...
    \end{aligned}\right)}{\rUI{\rRI{7,10}}}
  \proofstepwidestarbreakkeep[1]{\operatorname{Sing}(G)\subseteq\PowNE G}{%
    \FormulaRefAuto{SemigroupSingletonCopySubset}[thm]{\FormulaRefAuto{GroupBaseSemigroup}[thm]{1}}}
  \proofstepwidestarbreakkeep[2]{\operatorname{Sing}(H)\subseteq\PowNE H}{%
    \FormulaRefAuto{SemigroupSingletonCopySubset}[thm]{\FormulaRefAuto{GroupBaseSemigroup}[thm]{2}}}
  \proofstepwidestarbreakkeep[1,2,3]{R\colon\operatorname{Sing}(G)\bij\operatorname{Sing}(H)}{%
    \FormulaRefAuto{PointwiseCompatibilityImpliesBijectiveRestriction}[thm]{6,12,13,11}}
  ...
  \proofstepwidestarbreakkeep[1,3]{R[\operatorname{Sing}(G)]=F[\operatorname{Sing}(G)]}{%
    \FormulaRefAuto{C\subseteq A\dsep F\colon A\to B\vdash (F\restriction_C)[C]=F[C]}[thm]{12,\FormulaRefAuto{Bijektive Funktion}[def]{6}}}
  \proofstepwidestarbreakkeep[1,2,3]{F[\operatorname{Sing}(G)]=\operatorname{Sing}(H)}{%
    \FormulaRefAuto{a=b\dsep a=c\vdash b=c}[thm]{16,15}}
\end{tabproofwide}
\endgroup
...
\begin{tabproofwide}
  ...
  \end{aligned}\right)}{%
    \FormulaRefAuto{PowerSemigroupSingletonLayerReconstructionWitness}[thm]{%
      \FormulaRefAuto{GroupBaseSemigroup}[thm]{1},\FormulaRefAuto{GroupBaseSemigroup}[thm]{2},3,4}}
  \proofstepwidestarbreakkeep[6]{\begin{aligned}[t]
    &\SemigroupIso{\varphi}{G,\star}{H,\diamond}\\[-2pt]
    &\land\forall g\in G\,(F(\{g\})=\{\varphi(g)\})
  \end{aligned}}{\rA}
  \proofstepwidestarbreakkeep[6]{\SemigroupHom{\varphi}{G,\star}{H,\diamond}}{\FormulaRefAuto{SemigroupIsoHomAxiom}[ax]{\rAEa{6}}}
  \proofstepwidestarbreakkeep[6]{\varphi\colon G\bij H}{\FormulaRefAuto{SemigroupIsoBijectiveAxiom}[ax]{\rAEa{6}}}
  \proofstepwidestarbreakkeep[1,2,6]{\GroupHom{\varphi}{G,\star,e}{H,\diamond,u}}{\FormulaRefAuto{GroupHomDef}[def]{1,2,7}}
  \proofstepwidestarbreakkeep[1,2,6]{\GroupIso{\varphi}{G,\star,e}{H,\diamond,u}}{\FormulaRefAuto{GroupIsoDef}[def]{9,8}}
  \proofstepwidestarbreakkeep[1,2,6]{\exists\varphi\,\left(\begin{aligned}[t]
    &\GroupIso{\varphi}{G,\star,e}{H,\diamond,u}\\[-2pt]
    &\land\forall g\in G\,(F(\{g\})=\{\varphi(g)\})
  \end{aligned}\right)}{\rEI{\rAI{10,\rAEb{6}}}}
  \proofstepwidestarbreakkeep[1,2,3]{\exists\varphi\,\left(\begin{aligned}[t]
    &\GroupIso{\varphi}{G,\star,e}{H,\diamond,u}\\[-2pt]
    &\land\forall g\in G\,(F(\{g\})=\{\varphi(g)\})
  \end{aligned}\right)}{\rEE{5,6,11}}
\end{tabproofwide}
\endgroup
  ...
  \proofpartwideindR[Eindeutigkeit auf der natürlichen Einbettung]{%
    ...
    \proofstepwidestar[1,8]{%
      ...
      \BandXXIXProofRefStack{%
        ...
        \left(
          \FormulaRefAuto{NaturalSemiringMapAddOne}[thm]
    ...
    \proofstepwidestar[1,2,17]{%
      \begin{aligned}[t]
        &f(\IntEmbed(n))={}\\[-2pt]
        &\quad\NaturalSemiringMap{R,\oplus,\odot,0_R,1_R}(n)
      \end{aligned}}{%
      \FormulaRefAuto{PeanoInductionRuleAddOne}[thm]{7,16,17}}
  \closeproofpartwideindR
