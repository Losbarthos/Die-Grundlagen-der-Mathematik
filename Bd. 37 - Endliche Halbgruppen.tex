%============================================================
%  Bd. 37 - Endliche Halbgruppen
%============================================================

\documentclass[main.tex]{subfiles}

\ifSubfilesClassLoaded{
  \BandSetupStandalone{B37}
}{
}

\title{Bd. 37 - Endliche Halbgruppen}
\author{Martin Kunze}
\date{}
\setcounter{file}{37}

\hypersetup{
  pdftitle={Bd. 37 - Endliche Halbgruppen},
...
Potenzen und die Existenz von Idempotenten.  Die Idealtheorie aus Band~28
...
\FormulaThmDeltaK[Anzahl der nichtleeren Teilmengen]{%
  ...
  \vdash \FiniteCard\bigl(\PowNE{A}\bigr)=2^N-1%
...
\begin{tabproofwide}
  ...
  \proofstepwidestarbreak[1,2,3]{%
    \FiniteCard\bigl(\PowNE{A}\bigr)+1=2^N\land
  ...
  \proofstepwidestarforcebreak[1,2,3]{%
    \FiniteCard\bigl(\PowNE{A}\bigr)=2^N-1}{\rAEb{4}}
\end{tabproofwide}

Für \(\FiniteCard(A)=N\) gilt also
...
Band~28 zur Verfügung.  Die Endlichkeit wird nun nur noch an den Stellen
...
Die Potenzfolge aus Band~28 beginnt beim Folgenindex \(0\), während positive
...
\begin{tabproofwide}
  ...
  \proofstepwidestarbreak[8]{k+1\neq0}{%
    \FormulaRefAuto{PeanoAddOneNonzero}[thm]{10}}
  \proofstepwidestarbreak[8]{n+1\neq0}{%
    \FormulaRefAuto{PeanoAddOneNonzero}[thm]{11}}
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[3]{1\in\mathbb N}{%
      \FormulaRefAuto{PeanoOneInN}[thm]}
  ...
  \proofstepwidestarbreak[3,9,16]{s<j}{%
    \rIE{9,\rAEb{\rAEb{16}}}}
  \proofstepwidestarbreak[1,2,3,8,14,15,16]{a^r\neq a^s}{%
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Existenz.}\par
Wähle nach dem Existenzsatz ein Index--Perioden-Paar
\((\mu,\lambda)\) und setze \(z=(\mu,\lambda)\).  \par\medskip\noindent\textbf{(ii) Eindeutigkeit.}\par
Seien ferner
...
\FormulaThmDeltaK[Größe der von einem Element erzeugten Unterhalbgruppe]{%
  \begin{aligned}[t]
    ...
    &\Finite{\langle a\rangle_{A,\star}}\land
      \FiniteCard\!\left(\langle a\rangle_{A,\star}\right)
        =\mu+\lambda-1
  \end{aligned}%
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Darstellung durch beschränkt viele Potenzen.}\par
Nach Band~28 besteht \(\langle a\rangle_{A,\star}\) aus allen positiven
...
\(0<r<\mu+\lambda\) erfasst.  \par\medskip\noindent\textbf{(ii) Endlichkeit und Kardinalität.}\par
Diese Potenzen sind nach der
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Natürliche Zahl und Positivität.}\par
Die Abgeschlossenheit der Multiplikation liefert
\(\lambda\cdot\mu\in\mathbb N\).  Aus der Nullteilerfreiheit und
\(\lambda,\mu\neq0\) folgt außerdem
\(\lambda\cdot\mu\neq0\).  \par\medskip\noindent\textbf{(ii) Vergleich mit dem Index.}\par
Schreibe wegen \(\lambda\neq0\)
\(\lambda=k+1\).  Dann ist
\[
  \lambda\cdot\mu
  =(k+1)\cdot\mu
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Existenz eines minimalen Ideals.}\par
...
\(\subseteq\) ein minimales Mitglied \(K\).  \par\medskip\noindent\textbf{(ii) Kleinheit unter allen nichtleeren Idealen.}\par
Ist \(I\) ein nichtleeres
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[1,2,3,4]{e^{-1}\colon A\to\NatSeg{n}}{%
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Transportgleichung.}\par
...
\]
\par\medskip\noindent\textbf{(ii) Assoziativität.}\par
...
von \(e\) ergibt deshalb die Assoziativität von \(m_{\star,e}\).  \par\medskip\noindent\textbf{(iii) Endlichkeit und Isomorphismus.}\par
Der
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Typisierung der Operation und der Permutationen.}\par
...
\par\medskip\noindent\textbf{(ii) Erhaltung der Assoziativität.}\par
...
\((m^\pi)^\sigma\) definiert.  \par\medskip\noindent\textbf{(iii) Identitäts- und Kompositionsgesetz.}\par
Für alle \(u,v\in S\) gilt punktweise
\[
  m^{\Id_S}(u,v)=m(u,v)
\]
und wegen
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Von der Umbenennungsrelation zur Wirkungsformel.}\par
...
\]
\par\medskip\noindent\textbf{(ii) Von der Wirkungsformel zur Umbenennungsrelation.}\par
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Vom Orbit zur Isomorphie.}\par
...
endlicher Halbgruppen.  \par\medskip\noindent\textbf{(ii) Von der Isomorphie zum Orbit.}\par
Umgekehrt ist jeder solche Isomorphismus \(f\) eine
  ...
  \begin{aligned}[t]
    ...
    &\operatorname{can}(A,\star)\coloneqq
      \left(
        \FiniteCard(A),
        \left\{
          w_{\FiniteCard(A)}\!\left(m_{\star,e}\right)
          \;\middle|\;
          e\colon\NatSeg{\FiniteCard(A)}\bij A
        \right\}
      \right)
  \end{aligned}%
}{FiniteSemigroupCanonicalTableCodeDef}{%
  \DeltaRow{Mengen}{A}
  \DeltaRow{Funktionen}{\star\dsep e\dsep m_{\star,e}\dsep w_{\FiniteCard(A)}}
  \DeltaRow{\textbf{Neues Symbol}}{}
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Isomorphie erhält den Tafelcode.}\par
Sei zunächst \(f\) ein Isomorphismus von \((A,\star)\) nach
\((B,\diamond)\).  Dann ist \(\FiniteCard(A)=\FiniteCard(B)=n\), und die Zuordnung
...
\par\medskip\noindent\textbf{(ii) Gleicher Tafelcode liefert Isomorphie.}\par
...
Kürzbarkeit und die in Band~28 eingeführten Teilbarkeitsrelationen
...
\begin{proof}
\par\medskip\noindent\textbf{(i) Konstruktion der Bijektion.}\par
...
\[
  ...
  \colon A\bij B
\]
eine Bijektion mit \(F(\{a\})=\{f(a)\}\) für alle \(a\in A\).  \par\medskip\noindent\textbf{(ii) Operationserhaltung.}\par
Für
\(a,b\in A\) folgt
\[
\begin{aligned}
...
\[
  ...
  \quad\Longrightarrow\quad
  \FiniteCard(A)=\FiniteCard(B).
\]
Dabei gilt
\(\FiniteCard(\PowNE A)+1=2^{\FiniteCard(A)}\), und die
...
\FormulaThmDeltaK[Potenzhalbgruppen bestimmen Halbgruppen der Ordnung zwei]{%
  \begin{aligned}[t]
    &\FiniteSemigroupAlg{A}{\star}\dsep
      ...
      \FiniteCard(A)=2\dsep\FiniteCard(B)=2\dsep{}\\[-2pt]
    &\SemigroupIso{F}
      {\PowNE{A},\star_{\mathcal P}}
...
Die in Band~28 bewiesene umgekehrte Richtung gilt sogar ohne Endlichkeit:
...
Für die in den Bänden~29--31 eingeführten Kürzbarkeitsklassen lässt sich die
...
Für den folgenden Gruppenschluss werden die in den Bänden~38 und~40
systematisch behandelten Begriffe bereits durch ihre definierenden
Bedingungen festgelegt.

\FormulaDefDeltaR[Monoidbegriff für den endlichen Gruppenschluss]{%
  \begin{aligned}[t]
    &\MonoidAlg{A}{\star}{e}\coloneqq{}\\[-2pt]
    &\SemigroupAlg{A}{\star}\land
      \bigl(e\in A\land\bigl(
      (\forall x\in A\,(e\star x=x))\land
      (\forall x\in A\,(x\star e=x))\bigr)\bigr)
  \end{aligned}
}{FiniteSemigroupMonoidConventionDef}{%
  \DeltaRow{Mengen}{A\dsep e\dsep x}
  \DeltaRow{Funktionen}{\star}
}

\FormulaDefDeltaR[Gruppenbegriff für den endlichen Gruppenschluss]{%
  \begin{aligned}[t]
    &\GroupAlg{A}{\star}{e}\coloneqq{}\\[-2pt]
    &\MonoidAlg{A}{\star}{e}\land
      \forall x\in A\,\exists y\in A\,
        (x\star y=e\land y\star x=e)
  \end{aligned}
}{FiniteSemigroupGroupConventionDef}{%
...
\FormulaThmDeltaK[Eine nichtleere endliche kürzbare Halbgruppe ist eine Gruppe]{%
  \FiniteSemigroupAlg{A}{\star}\dsep
  \CancellativeSemigroupAlg{A}{\star}\dsep A\neq\varnothing
  \vdash\exists e\in A\,\GroupAlg{A}{\star}{e}
}{FiniteCancellativeSemigroupGroupExistence}{%
  \DeltaRow{Mengen}{A\dsep a\dsep e\dsep x\dsep y}
  \DeltaRow{Funktionen}{\star\dsep\rho_{A,\star,a}}
}
\begingroup
\ProofWideReasonsNearFormula
\begin{tabproofsplitwide}
  \proofpartwideindR[Bijektivität der Rechtstranslation]{%
    \FiniteSemigroupAlg{A}{\star}\dsep\CancellativeSemigroupAlg{A}{\star}
    \dsep a\in A\vdash\rho_{A,\star,a}\colon A\bij A
  }{%
    \FiniteSemigroupAlg{A}{\star}\dsep\CancellativeSemigroupAlg{A}{\star}
    \dsep a\in A\vdash\rho_{A,\star,a}\colon A\bij A
  }[FiniteCancellativeRightTranslationBijective]
    \proofstepwidestar[1]{\FiniteSemigroupAlg{A}{\star}}{\rA}
    \proofstepwidestar[2]{\CancellativeSemigroupAlg{A}{\star}}{\rA}
    \proofstepwidestar[3]{a\in A}{\rA}
    \proofstepwidestar[1]{\SemigroupAlg{A}{\star}}{%
      \FormulaRefAuto{FiniteSemigroupBaseAxiom}[ax]{1}}
    \proofstepwidestar[1]{\Finite{A}}{%
      \FormulaRefAuto{FiniteSemigroupFinitenessAxiom}[ax]{1}}
    \proofstepwidestar[1,3]{\rho_{A,\star,a}\colon A\to A}{%
      \FormulaRefAuto{SemigroupRightTranslationFunction}[thm]{4,3}}
    \proofstepwidestar[7]{x\in A}{\rA}
    \proofstepwidestar[8]{y\in A}{\rA}
    \proofstepwidestar[9]{\rho_{A,\star,a}(x)=\rho_{A,\star,a}(y)}{\rA}
    \proofstepwidestar[1,3,7]{\rho_{A,\star,a}(x)=x\star a}{%
      \FormulaRefAuto{SemigroupRightTranslationEvaluation}[thm]{4,3,7}}
    \proofstepwidestar[1,3,8]{\rho_{A,\star,a}(y)=y\star a}{%
      \FormulaRefAuto{SemigroupRightTranslationEvaluation}[thm]{4,3,8}}
    \proofstepwidestar[1,3,7,8,9]{x\star a=y\star a}{\rIE{10,11,9}}
    \proofstepwidestar[1,2,3,7,8,9]{x=y}{%
      \FormulaRefAuto{CancellativeSemigroupRightCancellationAxiom}[ax]{2,3,7,8,12}}
    \proofstepwidestar[1,2,3,7,8]{%
      \rho_{A,\star,a}(x)=\rho_{A,\star,a}(y)\rightarrow x=y}{\rRI{9,13}}
    \proofstepwidestar[1,2,3,7]{\forall y\in A\,
      (\rho_{A,\star,a}(x)=\rho_{A,\star,a}(y)\rightarrow x=y)}{\rUI{\rRI{8,14}}}
    \proofstepwidestar[1,2,3]{\forall x,y\in A\,
      (\rho_{A,\star,a}(x)=\rho_{A,\star,a}(y)\rightarrow x=y)}{\rUI{\rRI{7,15}}}
    \proofstepwidestar[1,2,3]{\rho_{A,\star,a}\colon A\inj A}{%
      \FormulaRefAuto{Injektive Funktion}[def]{6,16}}
    \proofstepwidestar[1]{\forall F\colon A\inj A\,(F\colon A\bij A)}{%
      \FormulaRefAuto{\Finite{M}\eqvdash
        \forall F\colon M\inj M\,\bigl(F\colon M\bij M\bigr)}[thm]{5}}
    \proofstepwidestar[1,2,3]{\rho_{A,\star,a}\colon A\bij A}{\rRE{\rUE{18},17}}
  \closeproofpartwideindR

  \proofpartwideindR[Ein Idempotentes ist beidseitig neutral]{%
    \CancellativeSemigroupAlg{A}{\star}\dsep e\in E_{A,\star}
    \vdash\MonoidAlg{A}{\star}{e}
  }{%
    \CancellativeSemigroupAlg{A}{\star}\dsep e\in E_{A,\star}
    \vdash\MonoidAlg{A}{\star}{e}
  }[CancellativeIdempotentGivesMonoid]
    \proofstepwidestar[1]{\CancellativeSemigroupAlg{A}{\star}}{\rA}
    ...
    \proofstepwidestar[1,2]{e\in A\land e\star e=e}{%
      \rRE{\rLREa{\FormulaRefAuto{SemigroupIdempotentMembership}[thm]{3}},2}}
    \proofstepwidestar[1,2]{e\in A}{\rAEa{4}}
    \proofstepwidestar[1,2]{e\star e=e}{\rAEb{4}}
    \proofstepwidestar[7]{x\in A}{\rA}
    \proofstepwidestar[1,2,7]{e\star x\in A}{%
      \FormulaRefAuto{SemigroupClosureAxiom}[ax]{3,5,7}}
    \proofstepwidestar[1,2,7]{x\star e\in A}{%
      \FormulaRefAuto{SemigroupClosureAxiom}[ax]{3,7,5}}
    \proofstepwidestar[1,2,7]{e\star(e\star x)=(e\star e)\star x}{%
      \FormulaRefAuto{a=b\vdash b=a}[thm]{\FormulaRefAuto{SemigroupAssociativityAxiom}[ax]{3,5,5,7}}}
    \proofstepwidestar[1,2,7]{e\star(e\star x)=e\star x}{\rIE{6,10}}
    \proofstepwidestar[1,2,7]{e\star x=x}{%
      \FormulaRefAuto{CancellativeSemigroupLeftCancellationAxiom}[ax]{1,5,8,7,11}}
    \proofstepwidestar[1,2]{\forall x\in A\,(e\star x=x)}{\rUI{\rRI{7,12}}}
    \proofstepwidestar[1,2,7]{(x\star e)\star e=x\star(e\star e)}{%
      \FormulaRefAuto{SemigroupAssociativityAxiom}[ax]{3,7,5,5}}
    \proofstepwidestar[1,2,7]{(x\star e)\star e=x\star e}{\rIE{6,14}}
    \proofstepwidestar[1,2,7]{x\star e=x}{%
      \FormulaRefAuto{CancellativeSemigroupRightCancellationAxiom}[ax]{1,5,9,7,15}}
    \proofstepwidestar[1,2]{\forall x\in A\,(x\star e=x)}{\rUI{\rRI{7,16}}}
    \proofstepwidestar[1,2]{\MonoidAlg{A}{\star}{e}}{%
      \FormulaRefAuto{FiniteSemigroupMonoidConventionDef}[def]{\rAI{3,\rAI{5,\rAI{13,17}}}}}
  \closeproofpartwideindR

  \proofpartwideindR[Beidseitige Inverse]{%
    \FiniteSemigroupAlg{A}{\star}\dsep\CancellativeSemigroupAlg{A}{\star}
    \dsep\MonoidAlg{A}{\star}{e}\vdash\GroupAlg{A}{\star}{e}
  }{%
    \FiniteSemigroupAlg{A}{\star}\dsep\CancellativeSemigroupAlg{A}{\star}
    \dsep\MonoidAlg{A}{\star}{e}\vdash\GroupAlg{A}{\star}{e}
  }[FiniteCancellativeMonoidIsGroup]
    \proofstepwidestar[1]{\FiniteSemigroupAlg{A}{\star}}{\rA}
    \proofstepwidestar[2]{\CancellativeSemigroupAlg{A}{\star}}{\rA}
    \proofstepwidestar[3]{\MonoidAlg{A}{\star}{e}}{\rA}
    \proofstepwidestar[3]{e\in A}{%
      \rAEa{\rAEb{\FormulaRefAuto{FiniteSemigroupMonoidConventionDef}[def]{3}}}}
    \proofstepwidestar[1]{\SemigroupAlg{A}{\star}}{%
      \FormulaRefAuto{FiniteSemigroupBaseAxiom}[ax]{1}}
    \proofstepwidestar[6]{x\in A}{\rA}
    \proofstepwidestar[1,2,6]{\rho_{A,\star,x}\colon A\bij A}{%
      \FormulaRefAuto{FiniteCancellativeRightTranslationBijective}[thm]{1,2,6}}
    \proofstepwidestar[1,2,6]{\forall z\in A\,\exists y\in A\,
      (\rho_{A,\star,x}(y)=z)}{\FormulaRefAuto{Bijektive Funktion}[def]{7}}
    \proofstepwidestar[1,2,3,6]{\exists y\in A\,(\rho_{A,\star,x}(y)=e)}{\rRE{\rUE{8},4}}
    \proofstepwidestar[10]{y\in A\land\rho_{A,\star,x}(y)=e}{\rA}
    \proofstepwidestar[10]{y\in A}{\rAEa{10}}
    \proofstepwidestar[1,6,10]{\rho_{A,\star,x}(y)=y\star x}{%
      \FormulaRefAuto{SemigroupRightTranslationEvaluation}[thm]{5,6,11}}
    \proofstepwidestar[1,6,10]{y\star x=e}{\rIE{12,\rAEb{10}}}
    \proofstepwidestar[1,6,10]{(x\star y)\star x=x\star(y\star x)}{%
      \FormulaRefAuto{SemigroupAssociativityAxiom}[ax]{5,6,11,6}}
    \proofstepwidestar[1,6,10]{(x\star y)\star x=x\star e}{\rIE{13,14}}
    \proofstepwidestar[3,6]{x\star e=x}{%
      \rRE{\rUE{\rAEb{\rAEb{\rAEb{\FormulaRefAuto{FiniteSemigroupMonoidConventionDef}[def]{3}}}}},6}}
    \proofstepwidestar[3,6]{e\star x=x}{%
      \rRE{\rUE{\rAEa{\rAEb{\rAEb{\FormulaRefAuto{FiniteSemigroupMonoidConventionDef}[def]{3}}}}},6}}
    \proofstepwidestar[1,3,6,10]{(x\star y)\star x=e\star x}{\rIE{16,\FormulaRefAuto{a=b\vdash b=a}[thm]{17},15}}
    \proofstepwidestar[1,6,10]{x\star y\in A}{\FormulaRefAuto{SemigroupClosureAxiom}[ax]{5,6,11}}
    \proofstepwidestar[1,2,3,6,10]{x\star y=e}{%
      \FormulaRefAuto{CancellativeSemigroupRightCancellationAxiom}[ax]{2,6,19,4,18}}
    \proofstepwidestar[1,2,3,6,10]{\exists y\in A\,(x\star y=e\land y\star x=e)}{%
      \rEI{\rAI{11,\rAI{20,13}}}}
    \proofstepwidestar[1,2,3,6]{\exists y\in A\,(x\star y=e\land y\star x=e)}{\rEE{9,10,21}}
    \proofstepwidestar[1,2,3]{\forall x\in A\,\exists y\in A\,
      (x\star y=e\land y\star x=e)}{\rUI{\rRI{6,22}}}
    \proofstepwidestar[1,2,3]{\GroupAlg{A}{\star}{e}}{%
      \FormulaRefAuto{FiniteSemigroupGroupConventionDef}[def]{\rAI{3,23}}}
  \closeproofpartwideindR

  \proofpartwide{Existenz des neutralen Elements und Gruppenschluss}
    \proofstepwidestar[1]{\FiniteSemigroupAlg{A}{\star}}{\rA}
    \proofstepwidestar[2]{\CancellativeSemigroupAlg{A}{\star}}{\rA}
    \proofstepwidestar[3]{A\neq\varnothing}{\rA}
    \proofstepwidestar[1,3]{E_{A,\star}\neq\varnothing}{%
      \FormulaRefAuto{FiniteSemigroupIdempotentExists}[thm]{1,3}}
    \proofstepwidestar[1,3]{\exists e\,(e\in E_{A,\star})}{%
      \FormulaRefAuto{A\neq\varnothing\eqvdash\exists x\,(x\in A)}[thm]{4}}
    \proofstepwidestar[6]{e\in E_{A,\star}}{\rA}
    \proofstepwidestar[2,6]{\MonoidAlg{A}{\star}{e}}{%
      \FormulaRefAuto{CancellativeIdempotentGivesMonoid}[thm]{2,6}}
    \proofstepwidestar[2,6]{e\in A}{%
      \rAEa{\rAEb{\FormulaRefAuto{FiniteSemigroupMonoidConventionDef}[def]{7}}}}
    \proofstepwidestar[1,2,6]{\GroupAlg{A}{\star}{e}}{%
      \FormulaRefAuto{FiniteCancellativeMonoidIsGroup}[thm]{1,2,7}}
    \proofstepwidestar[1,2,6]{\exists e\in A\,\GroupAlg{A}{\star}{e}}{\rEI{\rAI{8,9}}}
    \proofstepwidestar[1,2,3]{\exists e\in A\,\GroupAlg{A}{\star}{e}}{\rEE{5,6,10}}
  \closeproofpartwide
...
\FormulaThmDeltaK[Monogene Unterhalbgruppen im endlichen einseitig kürzbaren Fall]{%
  \begin{aligned}[t]
    &\FiniteSemigroupAlg{A}{\star}\dsep
    \bigl(\LeftCancellativeSemigroupAlg{A}{\star}\lor
      \RightCancellativeSemigroupAlg{A}{\star}\bigr)\vdash{}\\[-2pt]
    &\forall a\in A\,\exists e\in\langle a\rangle_{A,\star}\,
      \GroupAlg{\langle a\rangle_{A,\star}}{\star}{e}
  \end{aligned}
}{FiniteOneSidedCancellativeMonogenicGroups}{%
  \DeltaRow{Mengen}{A\dsep a\dsep U\dsep x\dsep y\dsep z\dsep m\dsep n\dsep e}
  \DeltaRow{Funktionen}{\star}
}
\begingroup
\ProofWideReasonsNearFormula
\begin{tabproofsplitwide}
  \proofpartwideindR[Kommutativität der monogenen Unterhalbgruppe]{%
    \SemigroupAlg{A}{\star}\dsep a\in A
    \vdash\CommutativeSemigroupAlg{\langle a\rangle_{A,\star}}{\star}
  }{%
    \SemigroupAlg{A}{\star}\dsep a\in A
    \vdash\CommutativeSemigroupAlg{\langle a\rangle_{A,\star}}{\star}
  }[MonogenicSubsemigroupIsCommutative]
    \proofstepwidestar[1]{\SemigroupAlg{A}{\star}}{\rA}
    \proofstepwidestar[2]{a\in A}{\rA}
    \proofstepwidestar[1,2]{\mathsf{UHGr}(\langle a\rangle_{A,\star};A,\star)}{%
      \FormulaRefAuto{MonogenicSubsemigroupIsSubsemigroup}[thm]{1,2}}
    \proofstepwidestar[1,2]{\SemigroupAlg{\langle a\rangle_{A,\star}}{\star}}{%
      \FormulaRefAuto{SubsemigroupFormsSemigroup}[thm]{1,3}}
    \proofstepwidestar[1,2]{\langle a\rangle_{A,\star}=P_{A,\star}(a)}{%
      \FormulaRefAuto{SemigroupMonogenicPowerDescription}[thm]{1,2}}
    \proofstepwidestar[6]{x\in\langle a\rangle_{A,\star}}{\rA}
    \proofstepwidestar[7]{y\in\langle a\rangle_{A,\star}}{\rA}
    \proofstepwidestar[1,2,6]{x\in P_{A,\star}(a)}{\rIE{5,6}}
    \proofstepwidestar[1,2,7]{y\in P_{A,\star}(a)}{\rIE{5,7}}
    \proofstepwidestar[1,2,6]{\exists m\in\mathbb N\,(m\neq0\land x=a^m)}{%
      \rAEb{\rRE{\rLREa{\FormulaRefAuto{SemigroupPositivePowerSetMembership}[thm]{1,2}},8}}}
    \proofstepwidestar[1,2,7]{\exists n\in\mathbb N\,(n\neq0\land y=a^n)}{%
      \rAEb{\rRE{\rLREa{\FormulaRefAuto{SemigroupPositivePowerSetMembership}[thm]{1,2}},9}}}
    \proofstepwidestar[12]{m\in\mathbb N\land(m\neq0\land x=a^m)}{\rA}
    \proofstepwidestar[13]{n\in\mathbb N\land(n\neq0\land y=a^n)}{\rA}
    \proofstepwidestar[1,2,12,13]{a^m\star a^n=a^n\star a^m}{%
      \FormulaRefAuto{SemigroupPowersCommute}[thm]{1,2,
        \rAEa{12},\rAEa{13},\rAEa{\rAEb{12}},\rAEa{\rAEb{13}}}}
    \proofstepwidestar[12]{a^m=x}{%
      \FormulaRefAuto{a=b\vdash b=a}[thm]{\rAEb{\rAEb{12}}}}
    \proofstepwidestar[13]{a^n=y}{%
      \FormulaRefAuto{a=b\vdash b=a}[thm]{\rAEb{\rAEb{13}}}}
    \proofstepwidestar[1,2,12,13]{x\star y=y\star x}{\rIE{15,16,14}}
    \proofstepwidestar[1,2,7,12]{x\star y=y\star x}{\rEE{11,13,17}}
    \proofstepwidestar[1,2,6,7]{x\star y=y\star x}{\rEE{10,12,18}}
    \proofstepwidestar[1,2,6]{\forall y\in\langle a\rangle_{A,\star}\,(x\star y=y\star x)}{%
      \rUI{\rRI{7,19}}}
    \proofstepwidestar[1,2]{\forall x,y\in\langle a\rangle_{A,\star}\,(x\star y=y\star x)}{%
      \rUI{\rRI{6,20}}}
    \proofstepwidestar[1,2]{\CommutativeSemigroupAlg{\langle a\rangle_{A,\star}}{\star}}{%
      \FormulaRefAuto{CommutativeSemigroupDef}[def]{4,21}}
  \closeproofpartwideindR

  \proofpartwideindR[Kürzung in einer kommutativen Unterhalbgruppe]{%
    \begin{aligned}[t]
      &\CommutativeSemigroupAlg{U}{\star}\dsep U\subseteq A\dsep{}\\[-2pt]
      &\bigl(\LeftCancellativeSemigroupAlg{A}{\star}\lor
        \RightCancellativeSemigroupAlg{A}{\star}\bigr)
      \vdash\CancellativeSemigroupAlg{U}{\star}
    \end{aligned}
  }{%
    \CommutativeSemigroupAlg{U}{\star}\dsep U\subseteq A\dsep
    \bigl(\LeftCancellativeSemigroupAlg{A}{\star}\lor
      \RightCancellativeSemigroupAlg{A}{\star}\bigr)
    \vdash\CancellativeSemigroupAlg{U}{\star}
  }[CommutativeSubsemigroupOneSidedCancellation]
    \proofstepwidestar[1]{\CommutativeSemigroupAlg{U}{\star}}{\rA}
    \proofstepwidestar[2]{U\subseteq A}{\rA}
    \proofstepwidestar[3]{\LeftCancellativeSemigroupAlg{A}{\star}\lor
      \RightCancellativeSemigroupAlg{A}{\star}}{\rA}
    \proofstepwidestar[4]{z\in U}{\rA}
    \proofstepwidestar[5]{x\in U}{\rA}
    \proofstepwidestar[6]{y\in U}{\rA}
    \proofstepwidestar[7]{z\star x=z\star y}{\rA}
    \proofstepwidestar[2,4]{z\in A}{\FormulaRefAuto{A\subseteq B,\,x\in A\vdash x\in B}[thm]{2,4}}
    \proofstepwidestar[2,5]{x\in A}{\FormulaRefAuto{A\subseteq B,\,x\in A\vdash x\in B}[thm]{2,5}}
    \proofstepwidestar[2,6]{y\in A}{\FormulaRefAuto{A\subseteq B,\,x\in A\vdash x\in B}[thm]{2,6}}
    \proofstepwidestar[11]{\LeftCancellativeSemigroupAlg{A}{\star}}{\rA}
    \proofstepwidestar[2,4,5,6,7,11]{x=y}{%
      \FormulaRefAuto{LeftCancellativeSemigroupCancellationAxiom}[ax]{11,8,9,10,7}}
    \proofstepwidestar[13]{\RightCancellativeSemigroupAlg{A}{\star}}{\rA}
    \proofstepwidestar[1,4,5]{z\star x=x\star z}{%
      \FormulaRefAuto{CommutativeSemigroupCommutativityAxiom}[ax]{1,4,5}}
    \proofstepwidestar[1,4,6]{z\star y=y\star z}{%
      \FormulaRefAuto{CommutativeSemigroupCommutativityAxiom}[ax]{1,4,6}}
    \proofstepwidestar[1,4,5,6,7]{x\star z=y\star z}{\rIE{14,15,7}}
    \proofstepwidestar[1,2,4,5,6,7,13]{x=y}{%
      \FormulaRefAuto{RightCancellativeSemigroupCancellationAxiom}[ax]{13,8,9,10,16}}
    \proofstepwidestar[1,2,3,4,5,6,7]{x=y}{\rOE{3,11,12,13,17}}
    \proofstepwidestar[1,2,3,4,5,6]{z\star x=z\star y\rightarrow x=y}{\rRI{7,18}}
    \proofstepwidestar[1,2,3]{\forall z,x,y\in U\,(z\star x=z\star y\rightarrow x=y)}{%
      \rUI{\rRI{4,\rUI{\rRI{5,\rUI{\rRI{6,19}}}}}}}
    \proofstepwidestar[1,2,3]{\mathsf{LK\ddot{u}rz}\!\left(U,\star\right)}{%
      \FormulaRefAuto{LeftCancellativeDef}[def]{20}}
    \proofstepwidestar[1]{\SemigroupAlg{U}{\star}}{\FormulaRefAuto{CommutativeSemigroupBaseAxiom}[ax]{1}}
    \proofstepwidestar[1,2,3]{\LeftCancellativeSemigroupAlg{U}{\star}}{%
      \FormulaRefAuto{LeftCancellativeSemigroupDef}[def]{22,21}}
    \proofstepwidestar[1,2,3]{\RightCancellativeSemigroupAlg{U}{\star}}{%
      \FormulaRefAuto{CommutativeLeftCancellativeSemigroupImpliesRightCancellativeSemigroup}[thm]{1,23}}
    \proofstepwidestar[1,2,3]{\CancellativeSemigroupAlg{U}{\star}}{%
      \FormulaRefAuto{CancellativeSemigroupDef}[def]{23,24}}
  \closeproofpartwideindR

  \proofpartwide{Monogene Unterhalbgruppen sind Gruppen}
    \proofstepwidestar[1]{\FiniteSemigroupAlg{A}{\star}}{\rA}
    \proofstepwidestar[2]{\LeftCancellativeSemigroupAlg{A}{\star}\lor
      \RightCancellativeSemigroupAlg{A}{\star}}{\rA}
    \proofstepwidestar[3]{a\in A}{\rA}
    ...
    \proofstepwidestar[1,3]{\mathsf{UHGr}(\langle a\rangle_{A,\star};A,\star)}{%
      \FormulaRefAuto{MonogenicSubsemigroupIsSubsemigroup}[thm]{4,3}}
    \proofstepwidestar[1,3]{\langle a\rangle_{A,\star}\subseteq A}{%
      \FormulaRefAuto{SemigroupSubsemigroupSubsetAxiom}[ax]{4,6}}
    \proofstepwidestar[1,3]{\SemigroupAlg{\langle a\rangle_{A,\star}}{\star}}{%
      \FormulaRefAuto{SubsemigroupFormsSemigroup}[thm]{4,6}}
    \proofstepwidestar[1,3]{\Finite{\langle a\rangle_{A,\star}}}{\FormulaRefAuto{FiniteSubset}[thm]{5,7}}
    \proofstepwidestar[1,3]{\FiniteSemigroupAlg{\langle a\rangle_{A,\star}}{\star}}{%
      \FormulaRefAuto{FiniteSemigroupDef}[def]{8,9}}
    \proofstepwidestar[1,3]{\CommutativeSemigroupAlg{\langle a\rangle_{A,\star}}{\star}}{%
      \FormulaRefAuto{MonogenicSubsemigroupIsCommutative}[thm]{4,3}}
    \proofstepwidestar[1,2,3]{\CancellativeSemigroupAlg{\langle a\rangle_{A,\star}}{\star}}{%
      \FormulaRefAuto{CommutativeSubsemigroupOneSidedCancellation}[thm]{11,7,2}}
    \proofstepwidestar[1,3]{\langle a\rangle_{A,\star}\neq\varnothing}{%
      \FormulaRefAuto{SemigroupSubsemigroupNonemptyAxiom}[ax]{4,6}}
    \proofstepwidestar[1,2,3]{\exists e\in\langle a\rangle_{A,\star}\,
      \GroupAlg{\langle a\rangle_{A,\star}}{\star}{e}}{%
      \FormulaRefAuto{FiniteCancellativeSemigroupGroupExistence}[thm]{10,12,13}}
    \proofstepwidestar[1,2]{\forall a\in A\,\exists e\in\langle a\rangle_{A,\star}\,
      \GroupAlg{\langle a\rangle_{A,\star}}{\star}{e}}{\rUI{\rRI{3,14}}}
  \closeproofpartwide
\end{tabproofsplitwide}
\endgroup

Jedes Element \(a\in A\) liegt nach
\FormulaRefAuto{GeneratedSubsemigroupContainsGenerators}[thm] und
\FormulaRefAuto{SemigroupMonogenicSubsemigroupDef}[def] in seinem Monogen.
Die soeben nachgewiesenen Untergruppen überdecken daher \(A\); dies ist die
hier verwendete Bedeutung von vollständig regulär.
...
In der folgenden Tabelle sind die beiden nicht elementaren
Rekonstruktionsschritte ausdrücklich als Literaturresultate markiert.
...
\ProofWideReasonsNearFormula
\begin{tabproofsplitwide}
  \proofpartwide{Nichtleere Träger in den Fällen (1) und (2)}
    \proofstepwidestar[1]{\FiniteSemigroupAlg{A}{\star}}{\rA}
    ...
    \proofstepwidestar[5]{%
      \substack{\text{beide Halbgruppen sind linkskürzbar}\\
        \text{oder beide Halbgruppen sind rechtskürzbar}}}{\rA}
    \proofstepwidestar[1,4,5]{%
      \text{Die Halbgruppe }(A,\star)\text{ ist vollständig regulär}}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright Struktursatz über einseitige Kürzbarkeit}}}
    \proofstepwidestar[2,4,5]{%
      \text{Die Halbgruppe }(B,\diamond)\text{ ist vollständig regulär}}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright Struktursatz über einseitige Kürzbarkeit}}}
    \proofstepwidestar[1,2,3]{%
      \begin{aligned}[t]
        &C\subseteq A\dsep D\subseteq B\dsep
          \exists H\colon\PowNE{C}\bij\PowNE{D}
      \end{aligned}}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright im Rekonstruktionsbeweis auftretende Klassen}}}
    \proofstepwidestar[1,2,8]{\Finite{C}\land\Finite{D}}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright Teilmengen endlicher Träger}}}
    \proofstepwidestar[1,2,8,9]{2^{|C|}-1=2^{|D|}-1}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright Bijektivität in 8 und Anzahl nichtleerer Teilmengen}}}
    \proofstepwidestar[1,2,8,9]{|C|=|D|}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright strenge Monotonie von \(n\mapsto2^n-1\), 10; kein GCH nötig}}}
    \proofstepwidestar[1,2,3,4,5]{%
      (A,\star)\cong(B,\diamond)}{%
      \textnormal{\textbf{Literaturresultat: }}%
      \href{https://doi.org/10.1080/927872.2026.2659846}%
        {Yu--Zhao (2026), Satz~5.4}}
  \closeproofpartwide

  \proofpartwide{Nichtleere Träger im Fall (3)}
    \proofstepwidestar[1]{\FiniteSemigroupAlg{A}{\star}}{\rA}
    \proofstepwidestar[2]{\FiniteSemigroupAlg{B}{\diamond}}{\rA}
    \proofstepwidestar[3]{%
      \SemigroupIso{F}
    ...
    \proofstepwidestar[4]{A\neq\varnothing\land B\neq\varnothing}{\rA}
    \proofstepwidestar[5]{%
      \substack{\text{mindestens eine der beiden Halbgruppen ist}\\
        \text{links- und rechtskürzbar}}}{\rA}
    \proofstepwidestar[1,2,5]{%
      \begin{aligned}[t]
        &\{(G,\circ),(S,\bullet)\}
          =\{(A,\star),(B,\diamond)\},\qquad
          \CancellativeSemigroupAlg{G}{\circ}
      \end{aligned}}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright Bezeichnung der kürzbaren und der anderen Halbgruppe}}}
    \proofstepwidestar[1,2,4,5,6]{%
      \FiniteSemigroupAlg{G}{\circ}\land G\neq\varnothing}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright 6 und Endlichkeit der beiden Ausgangshalbgruppen}}}
    \proofstepwidestar[1,2,4,5,6]{%
      e_G\in G\land\GroupAlg{G}{\circ}{e_G}}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright Wähle \(e_G\) nach dem Hilfssatz, 7}}}
    \proofstepwidestar[1,2,3,6]{%
      \exists F'\,
        \SemigroupIso{F'}
          {\PowNE{G},\circ_{\mathcal P}}
          {\PowNE{S},\bullet_{\mathcal P}}}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright \(F\) oder \(F^{-1}\), gemäß der Bezeichnung in 6}}}
    \proofstepwidestarforcebreak[1,2,3,4,5]{%
      \exists e_S\in S\,\exists f\,
        \bigl(\GroupAlg{S}{\bullet}{e_S}\land
          \GroupIso{f}{G,\circ,e_G}{S,\bullet,e_S}\bigr)}{%
      \begin{aligned}[t]
        &\text{\textbf{Literaturresultat:} }
          \href{https://doi.org/10.48550/arXiv.2606.1917}%
            {\text{Liu--Tringali (2026)}},\\[-2pt]
        &\text{in Band~40 vollständig bewiesen; Schritte 8--9}
      \end{aligned}}
    \proofstepwidestar[1,2,3,4,5]{(A,\star)\cong(B,\diamond)}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright Rückkehr von \(G,S\) zu \(A,B\), 6,10}}}
  \closeproofpartwide

  \proofpartwide{Leere Träger und Schluss}
    \proofstepwidestar[1]{\FiniteSemigroupAlg{A}{\star}}{\rA}
    \proofstepwidestar[2]{\FiniteSemigroupAlg{B}{\diamond}}{\rA}
    \proofstepwidestar[3]{%
      \SemigroupIso{F}
        {\PowNE{A},\star_{\mathcal P}}
        {\PowNE{B},\diamond_{\mathcal P}}}{\rA}
    \proofstepwidestar[4]{\text{Einer der Fälle (1)--(3) gilt}}{\rA}
    \proofstepwidestar[]{%
      \PowNE{A}=\varnothing\leftrightarrow A=\varnothing}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright Definition von \(\PowNE{A}\)}}}
    \proofstepwidestar[]{%
      \PowNE{B}=\varnothing\leftrightarrow B=\varnothing}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright Definition von \(\PowNE{B}\)}}}
    \proofstepwidestar[3]{%
      \PowNE{A}=\varnothing\leftrightarrow\PowNE{B}=\varnothing}{%
      \textnormal{Bijektivität von \(F\)}}
    \proofstepwidestar[3]{A=\varnothing\leftrightarrow B=\varnothing}{%
      \rLRS{6,\rLRS{5,7}}}
    \proofstepwidestar[1,2,3]{%
      (A=\varnothing\land B=\varnothing)\rightarrow(A,\star)\cong(B,\diamond)}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright die leere Abbildung ist ein Isomorphismus}}}
    \proofstepwidestar[1,2,3,4]{%
      A\neq\varnothing\land B\neq\varnothing
        \rightarrow(A,\star)\cong(B,\diamond)}{%
      \textnormal{\parbox[t]{\linewidth}{\raggedright die beiden vorangehenden Teile}}}
    \proofstepwidestar[]{A=\varnothing\lor A\neq\varnothing}{%
      \FormulaRefAuto{P\lor\neg P}[thm]}
    \proofstepwidestar[12]{A=\varnothing}{\rA}
    \proofstepwidestar[3,12]{B=\varnothing}{\rRE{\rLREa{8},12}}
    \proofstepwidestar[1,2,3,12]{(A,\star)\cong(B,\diamond)}{\rRE{9,\rAI{12,13}}}
    \proofstepwidestar[15]{A\neq\varnothing}{\rA}
    \proofstepwidestar[16]{B=\varnothing}{\rA}
    \proofstepwidestar[3,16]{A=\varnothing}{\rRE{\rLREb{8},16}}
    \proofstepwidestar[3,15,16]{\bot}{\rBI{15,17}}
    \proofstepwidestar[3,15]{B\neq\varnothing}{\rCI{16,18}}
    \proofstepwidestar[1,2,3,4,15]{(A,\star)\cong(B,\diamond)}{\rRE{10,\rAI{15,19}}}
    \proofstepwidestar[1,2,3,4]{(A,\star)\cong(B,\diamond)}{\rOE{11,12,14,15,20}}
  \closeproofpartwide
\end{tabproofsplitwide}
\endgroup
...
\begin{itemize}
  \item Ohne Endlichkeitsvoraussetzung ist die allgemeine Aussage falsch.
    Das in Band~28 vollständig ausgeführte Gegenbeispiel geht zurück auf
  ...
  \item Sind beide endlichen Halbgruppen idempotent, so folgt ihre
    ...
    Band~31 enthält den Rekonstruktionsbeweis und erläutert auch, weshalb
  ...
  \item Ist eine Grundhalbgruppe eine Gruppe, darf die andere zunächst eine
    ...
    \href{https://doi.org/10.48550/arXiv.2606.01917}%
      {(Liu--Tringali, 2026)}.  Der vollständige Beweis steht in Band~40 im
...
Band~42 ergänzt nach Einführung der Gruppen einen weiteren vollständig
bewiesenen Fall: Ist die Produktmenge einer endlichen Halbgruppe eine
Gruppe, so ist die Halbgruppe bereits durch ihre Potenzhalbgruppe bestimmt.
Die dortige Rekonstruktion zählt die Fasern einer Inflation und verlangt
keine Erhaltung der Einermengen außerhalb des Gruppenkerns.
\end{remark}
...
positive Halbgruppenpotenzen aus Band~28 liefert unmittelbar
\[
...
Unterhalbgruppe aus Band~28 gilt
\[
...
beidseitiges Inverses.  In der erst in Band~40 formal eingeführten
