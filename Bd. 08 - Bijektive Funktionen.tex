...
% In diesem Abschnitt treten viele lange Mengenfamilien- und Funktionsterme
% auf.  Feste, oben ausgerichtete Absatzspalten halten Formel und Begründung
% dennoch in jeder Beweiszeile an derselben Stelle.  Die Anpassung wird am
% Abschnittsende wieder zurückgenommen.
\let\BandEightFamiliesOriginalProofStepWideStar\proofstepwidestar
\let\BandEightFamiliesOriginalProofStepWideStarBreakKeep\proofstepwidestarbreakkeep
\ExplSyntaxOn
...
\RenewDocumentCommand{\proofstepwidestar}{O{} m m}{%
  ...
  \BandEightFamiliesBreakArgs{#1} & \theproofstepnr
     & \multicolumn{3}{@{}>{\raggedright\arraybackslash}p{.59\linewidth}@{}}{%
         \ensuremath{#2}}%
     & \multicolumn{1}{@{\hspace{.4em}}l@{}}{%
         \makebox[.23\linewidth][l]{\scriptsize #3}}\\%
}
\RenewDocumentCommand{\proofstepwidestarbreakkeep}{O{} m m}{%
  \proofstepwidestar[#1]{#2}{#3}%
}
...
\begin{tabproofsplitwide}
  \proofpartwideindR[Zerlegung]{%
    ...
    \proofstepwidestar[25]{H\in\CoreFamily{P}{A\cup\{c\}}}{%
      \FormulaRefAuto{CoreFamilyMembership}[thm]{\rAI{\rAEa{26},28}}}
    \proofstepwidestar[30]{H\in\CoreFamily{P\cup\{c\}}{A}}{\rA}
    \proofstepwidestar[30]{P\cup\{c\}\subseteq H\land
      H\subseteq(P\cup\{c\})\cup A}{%
      \FormulaRefAuto{CoreFamilyMembership}[thm]{30}}
    \proofstepwidestar[]{P\subseteq P\cup\{c\}}{%
      \FormulaRefAuto{A\subseteq A\cup B}[thm]}
    \proofstepwidestar[30]{P\subseteq H}{%
      \FormulaRefAuto{A\subseteq B\dsep B\subseteq C\vdash A\subseteq C}[thm]{%
        32,\rAEa{31}}}
    \proofstepwidestar[]{(P\cup\{c\})\cup A=P\cup(\{c\}\cup A)}{%
      \FormulaRefAuto{(A\cup B)\cup C=A\cup(B\cup C)}[thm]}
    ...
    \proofstepwidestar[]{(P\cup\{c\})\cup A=P\cup(A\cup\{c\})}{%
      \rChain{34,36}}
    \proofstepwidestar[30]{H\subseteq P\cup(A\cup\{c\})}{%
      \rIE{37,\rAEb{31}}}
    \proofstepwidestar[30]{H\in\CoreFamily{P}{A\cup\{c\}}}{%
      \FormulaRefAuto{CoreFamilyMembership}[thm]{\rAI{33,38}}}
    ...
    \proofstepwidestar[]{%
      ...
      \FormulaRefAuto{A\subseteq B\coloneq
        \forall x\,(x\in A\rightarrow x\in B)}[def]{\rUI{\rRI{23,40}}}}
    \proofstepwidestar[1]{\CoreFamily{P}{A\cup\{c\}}
      =\CoreFamily{P}{A}\cup\CoreFamily{P\cup\{c\}}{A}}{%
      \FormulaRefAuto{A\subseteq B\dsep B\subseteq A\vdash A=B}[thm]{22,41}}
  \closeproofpartwideindR
  ...
  \proofpartwideindR[Disjunktheit]{%
    ...
    \proofstepwidestar[2]{H\subseteq P\cup A}{%
      \rAEb{\FormulaRefAuto{CoreFamilyMembership}[thm]{\rAEa{3}}}}
    \proofstepwidestar[2]{P\cup\{c\}\subseteq H}{%
      \rAEa{\FormulaRefAuto{CoreFamilyMembership}[thm]{\rAEb{3}}}}
    ...
    \proofstepwidestar[1,2]{\bot}{\rBI{1,8}}
    \proofstepwidestar[1]{%
      \CoreFamily{P}{A}\cap\CoreFamily{P\cup\{c\}}{A}=\varnothing}{%
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[1,2,3,4,5,6]{%
    ...
    \end{aligned}}{%
    \FormulaRefAuto{CaseFunBijectiveDisjointTargets}[thm]{15,6,16,17}}
  \proofstepwidestar[1,2,3,4,5,6]{%
    \begin{aligned}[t]
      &\CaseFun
        {\CoreFamily{P}{A}}
  ...
  \proofpartwideindR[Fixierung der leeren Menge]{%
    ...
    \proofstepwidestar[1,2,3,4,5,6]{%
      \varnothing\notin\mathcal C\rightarrow
        \eta(\varnothing)=\varnothing}{\rRE{\rUE{7},8}}
    \proofstepwidestar[1,2,3,4,5,6]{%
      \eta(\varnothing)=\varnothing}{\rRE{9,3}}
  \closeproofpartwideindR
\end{tabproofsplitwide}
...
\begin{tabproofwide}
  ...
  \proofstepwidestar[3,6]{\LayerPowerMap{A}{D}{E}{g}(X)
    \in\powerset(A\cup E)}{%
    \FormulaRefAuto{F\colon A\to B\dsep x\in A\vdash F(x)\in B}[thm]{%
      \FormulaRefAuto{LayerPowerMapMapping}[thm]{3},6}}
  ...
  \proofstepwidestarbreakkeep[4,7]{%
    ...
    \end{aligned}}{%
    \FormulaRefAuto{LayerPowerMapEvaluation}[thm]{4,7}}
  \proofstepwidestar[]{X\cap D\in\powerset(D)}{%
    \FormulaRefAuto{x \in \powerset(A)\eqvdash x \subseteq A}[thm]{%
      \FormulaRefAuto{A\cap B\subseteq B}[thm]}}
...
\begin{tabproofsplitwide}
  ...
  \proofpartwide{Schluss}
    ...
    \proofstepwidestarbreakkeep[3]{\LayerPowerMap{A}{D}{E}{g}\colon
      \powerset(A\cup D)\to\powerset(A\cup E)}{%
      \FormulaRefAuto{LayerPowerMapMapping}[thm]{\FormulaRefAuto{Bijektive Funktion}[def]{3}}}
...
% Die abschnittslokalen Tabellenanpassungen dürfen nicht in die folgenden
% Abschnitte und beim Gesamtsatz nicht in spätere Bände hineinwirken.
\let\proofstepwidestar\BandEightFamiliesOriginalProofStepWideStar
\let\proofstepwidestarbreakkeep\BandEightFamiliesOriginalProofStepWideStarBreakKeep
\makeatletter
...
Im Beweis bezeichnet \(t(y)\) den bereits mengenwertigen Fallterm
\[
  t(y)=
  \begin{cases}
    b,&y=c,\\
  ...
  \end{cases}
\]
Auch \(b=c\) ist zugelassen; dann ist \(t(y)=y\).
\begin{tabproofsplitwide}
  \proofpartwide{Konstruktion und Vertauschungswerte}
    \proofstepwidestar[1]{b\in B\land c\in B}{\rA}
    \proofstepwidestar[2]{y\in B}{\rA}
    \proofstepwidestar[]{%
      y=c\lor(y\neq c\land y=b)\lor(y\neq c\land y\neq b)}{%
      \textnormal{zweimal ausgeschlossener Dritter}}
    \proofstepwidestar[1,2]{t(y)\in B}{%
      \textnormal{Fallterm; Fallunterscheidung 3 mit 1--2}}
    \proofstepwidestar[1]{\forall y\in B\,t(y)\in B}{%
      \rUI{\rRI{2,4}}}
    \proofstepwidestar[1]{%
      \exists\tau\,\bigl(\tau\colon B\to B\land
        \forall y\in B\,\tau(y)=t(y)\bigr)}{%
      \FormulaRefAuto{TypedFunctionDefinitionPrinciple}[thm]{5}}
  \closeproofpartwide

  \proofpartwideindR[Die Fallabbildung ist bijektiv]{%
    b\in B\dsep c\in B\dsep\tau\colon B\to B\dsep
    \forall y\in B\,\tau(y)=t(y)\vdash
    \tau\colon B\bij B\land\tau(c)=b
  }{%
    b\in B\dsep c\in B\dsep\tau\colon B\to B\dsep
    \forall y\in B\,\tau(y)=t(y)\vdash
    \tau\colon B\bij B\land\tau(c)=b
    ...
    \proofstepwidestar[1]{b\in B\land c\in B}{\rA}
    \proofstepwidestar[2]{\tau\colon B\to B}{\rA}
    \proofstepwidestar[3]{\forall y\in B\,\tau(y)=t(y)}{\rA}
    \proofstepwidestar[1,3]{\tau(c)=b}{%
      \textnormal{Spezialisierung von 3; erster Fall}}
    \proofstepwidestar[1,3]{\tau(b)=c}{%
      \textnormal{3; Fälle \(b=c\) und \(b\neq c\)}}
    \proofstepwidestar[3]{%
      \forall y\in B\,(y\neq c\land y\neq b\rightarrow\tau(y)=y)}{%
      \textnormal{Spezialisierung von 3; dritter Fall}}
    \proofstepwidestar[7]{y\in B}{\rA}
    \proofstepwidestar[1,2,3,7]{y=c\rightarrow\tau(\tau(y))=y}{%
      \textnormal{Gleichheitsersetzung mit 4--5}}
    \proofstepwidestar[1,2,3,7]{y=b\rightarrow\tau(\tau(y))=y}{%
      \textnormal{Gleichheitsersetzung mit 5 und 4}}
    \proofstepwidestar[2,3,7]{%
      y\neq c\land y\neq b\rightarrow\tau(\tau(y))=y}{%
      \textnormal{zweimal Gleichheitsersetzung mit 6}}
    \proofstepwidestar[1,2,3,7]{\tau(\tau(y))=y}{%
      \textnormal{ausgeschlossener Dritter; 8--10}}
    \proofstepwidestar[1,2,3]{\forall y\in B\,\tau(\tau(y))=y}{%
      \rUI{\rRI{7,11}}}
    \proofstepwidestar[13]{y\in B\land z\in B\land\tau(y)=\tau(z)}{\rA}
    \proofstepwidestar[1,2,3,13]{y=\tau(\tau(y))=\tau(\tau(z))=z}{%
      \textnormal{12; Gleichheitsersetzung mit 13}}
    \proofstepwidestar[1,2,3]{\tau\colon B\inj B}{%
      \FormulaRefAuto{Injektive Funktion}[def]{2,13,14}}
    \proofstepwidestar[2,7]{\tau(y)\in B}{%
      \FormulaRefAuto{F\colon A\to B\dsep x\in A\vdash F(x)\in B}[thm]{2,7}}
    \proofstepwidestar[1,2,3,7]{\exists z\in B\,\tau(z)=y}{%
      \rEI{\rAI{16,11}}}
    \proofstepwidestar[1,2,3]{\tau\colon B\sur B}{%
      \FormulaRefAuto{Surjektive Funktion}[def]{2,7,17}}
    \proofstepwidestar[1,2,3]{\tau\colon B\bij B}{%
      \FormulaRefAuto{F\colon A\inj B\dsep F\colon A\sur B\vdash
        F\colon A\bij B}[thm]{15,18}}
    \proofstepwidestar[1,2,3]{\tau\colon B\bij B\land\tau(c)=b}{\rAI{19,4}}
  \closeproofpartwideindR
  ...
  \proofpartwide{Existenzschluss}
    ...
    \proofstepwidestar[1]{%
      \exists\tau\,\bigl(\tau\colon B\to B\land
        \forall y\in B\,\tau(y)=t(y)\bigr)}{%
      \textnormal{Konstruktion, Zeile 6}}
    \proofstepwidestar[3]{%
      \tau\colon B\to B\land\forall y\in B\,\tau(y)=t(y)}{\rA}
    \proofstepwidestar[1,3]{\tau\colon B\bij B\land\tau(c)=b}{%
      \FormulaRefAuto{TranspositionTermBijective}[thm]{1,3}}
    \proofstepwidestar[1,3]{%
      \exists\tau\,\bigl(\tau\colon B\bij B\land\tau(c)=b\bigr)}{\rEI{4}}
    \proofstepwidestar[1]{%
      \exists\tau\,\bigl(\tau\colon B\bij B\land\tau(c)=b\bigr)}{\rEE{2,3,5}}
  \closeproofpartwide
\end{tabproofsplitwide}
